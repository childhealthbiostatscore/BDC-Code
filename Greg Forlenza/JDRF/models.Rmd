...
mod = glm(no_int_model,family = "binomial",df)
kable(summary(mod)$coefficients,caption = "Bad model summary")
```
...
summary(t1,labelTranslations = 
          list(am_final = "AM Use at Final Visit",hba1c = "Baseline HbA1c",
               c_paid_total = "PAID PEDS Baseline Score",
               ya_paid_total = "YA PAID Baseline Score"))
```

### Final model

```{r final model,echo=FALSE}
final_model = as.formula("success ~ hba1c + m1_sensor_wear + m1_tir + demographics_age + demographics_sex.factor")

mod = glm(final_model,family = "binomial",df)
t = broom::tidy(mod,exponentiate=T)
t$term[2:nrow(t)] = c("Baseline HbA1c","M1 Sensor Wear","M1 TIR","Baseline Age 14 - 18","Baseline Age 18+","Male")
kable(t)
```
