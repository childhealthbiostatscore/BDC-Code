---
title: "Markdown"
author: "Greg"
date: "12/20/2019"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
library(skimr)
library(tidyverse)
library(table1)
library(lme4)
library(nlme)
```

```{r data import,include=FALSE}
source("C:/Users/forlenzg/Dropbox/JDRF ECPODRA - HCL Modeling and Interventions/Study Analysis Fall 2019 (Pyle and Vigers)/AdvancedClosedLoopCl_R_2019-12-12_1535.r")
```

```{r eda}
demographics <- data %>%
  filter(redcap_event_name == "baseline_arm_1" | redcap_event_name == "baseline_arm_2") %>%
  select(record_id,demographics_age.factor:demographics_t1d_duration.factor)

table1::table1(~demographics_age.factor + demographics_pumphx.factor + 
                 demographics_cgmhx.factor | demographics_sex.factor,demographics)
```