...
m1 = data %>% filter(gyl_timepoint == 1) %>% 
  select(record_id,sensor_wear,sensor_70_180,dailybolus,time_am)
...
Lasso parameter $\lambda$ were selected based on 5-fold cross validation. There are two recommended ways to looks at lasso selection: picking the model with the lowest CV error, or the smallest model with CV error within 1 standard deviation of the minimum.
