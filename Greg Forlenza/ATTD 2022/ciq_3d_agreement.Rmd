...
# HbA1c cutoff
roc_df$hba1c_group = cut(roc_df$hba1c,c(-Inf,7,Inf),labels = c("<7%",">=7%"),right = F)
# LOO CV
cv = groupKFold(roc_df$record_id,k = 5)
aucs = sapply(cv, function(f){
  # Split data
  train = roc_df[f,]
  test = roc_df[-f,]
  # Mixed model
  mod = glmer(hba1c_group ~ sensor_70_180 + (1|record_id),data = train,family = "binomial")
  # Predict on test
  probs = predict(mod, test, type = "response", allow.new.levels = TRUE)
  # ROC
  r = roc(response = test$hba1c_group, predictor = probs,quiet = T)
  # Return AUC
  r$auc[[1]]
})
```
