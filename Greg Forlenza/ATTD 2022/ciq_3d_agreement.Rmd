...
roc_df = data %>% select(record_id,sensor_mean,sensor_70_180,hba1c)
# HbA1c cutoff
roc_df$hba1c_group = cut(roc_df$hba1c,c(-Inf,7,Inf),labels = c("<=7%",">7%"))
roc_df$hba1c_group = relevel(roc_df$hba1c_group,ref = ">7%")
# Mixed model
mod = geeglm(hba1c_group ~ sensor_70_180,id = record_id,data = roc_df,family = binomial)
kable(tidy(mod,"fixed",exponentiate = T),digits = 3)
# Predict on test
probs = predict(mod, type = "response")
# ROC
r = roc(response = mod@frame$hba1c_group, predictor = probs,ci=T)
# Plot
plot(r,print.thres = T)
```

We fit a mixed logistic model with random intercept for each participant. The results above are interpreted on the odds ratio scale, so for each 1 unit increase in TIR the odds of achieving an HbA1c of <=7% increase by `r round((exp(summary(mod)$coefficients[2,1])-1)*100,2)`% on average.
