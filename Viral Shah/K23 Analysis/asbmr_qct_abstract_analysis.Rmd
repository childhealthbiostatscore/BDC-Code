...
fit_mod = function(outcome,df){
    ...
    # Fit p1np model
  f = as.formula(paste0("p1np_baseline~",outcome, "*", "t1d_diagnosis"))
  mod = lm(f,
            data = df,
            na.action = na.omit)
   cat("\n")
  ...
  # Fit ctx model
  f2 = as.formula(paste0("ctx_bl~",outcome,"*","t1d_diagnosis"))
  mod2 = lm(f2,
            data = df,
            na.action = na.omit)
   cat("\n")
...
# Interactions
## HBA1C
P1NP: Hba1c levels affect p1np differences between t1d and controls (p = 0.04)
CTX: Hba1c levels do not affect ctx differences between groups (p = 0.15)
...
# Code Appendix
```{r qct interactions}
# any interaction between QCT variables and age of onset t1d, diabetes duration, hba1c
