...
ggplot(plot_df,aes(x = tir_70_140,y = pred,color = time)) + 
  geom_line() + theme_bw() + geom_point(aes(y = a1c),size = 1) +
  xlab("TTIR (%)") + ylab("A1C (%)") + 
...
ggplot(plot_df,aes(x = mbg,y = pred,color = time)) + 
  geom_line() + theme_bw() + geom_point(aes(y = a1c),size = 1) +
  annotate(geom = "text",x=250,y=5.5,parse=T,label=paste0("R^{2}",": ",r)) +
  annotate(geom = "text",x=250,y=5.25,parse=T,label=paste0("RMSE: ",rms)) +
