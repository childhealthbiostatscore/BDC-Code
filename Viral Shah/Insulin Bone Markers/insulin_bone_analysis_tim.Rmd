...
cpep <- cpep %>%
  ...
  select(PtID,Visit,TimePoint,Analyte,Value) %>%
  group_by(PtID) %>%
...
alldata$cpep_group <- as.factor(ifelse(alldata$cpep_group == "medium","high",as.character(alldata$cpep_group)))
# Remove missing
...
# Nicer rownames
rownames(t1a) <- c("n","Gender = M (%)","InsDeliveryMethod = Pump (%)","CurrUseCGM = Yes (%)","AgeAtConsent Years (median [IQR])","T1DDuration Years (median [IQR])","UnitsInsPerKg (mean (SD))","HbA1c % (mean (SD))","BMI kg/(cm^2) (mean (SD))","Vitamin D ng/mL (mean (SD))","calcium mg/dL (median [IQR])","ALPO4 (median [IQR])")
...
```{r echo=FALSE,dpi=600,eval=FALSE}
...
df1$Timepoint <- 0
plotvars120 <- c("PtID","cpep_group",corrvars120[-c(which(corrvars120 == "INS_TOSOH_120"))])
