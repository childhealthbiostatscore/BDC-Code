...
for (i in 1:length(NN_vars)) {
  mod <- lm(data = data,data[,NN_vars[i]]~T1DStatus+Age+as.factor(Sex)+BMI + NN_BMD)
...
for (i in 1:length(FS_vars)) {
  mod <- lm(data = data,data[,FS_vars[i]]~T1DStatus+Age+as.factor(Sex)+BMI + FS_BMD)
...
for (i in 1:length(IT_vars)) {
  mod <- lm(data = data,data[,IT_vars[i]]~T1DStatus+Age+as.factor(Sex)+BMI + IT_BMD)
