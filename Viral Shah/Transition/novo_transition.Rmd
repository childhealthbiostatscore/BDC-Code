...
cgm_data = left_join(cgm_data,cgm_data72,by = "subject_id") %>% 
  select(subject_id,randomization_group.factor,
...
# Table 1
t1 <- 
  tableby(randomization_group.factor~age+a1c+diabetes_duration+bmi+fe(gender.factor)+
...
t = 
  tableby(randomization_group.factor ~ kwt(percent_time_70_180.x,"median","q1q3") + 
            kwt(percent_time_70_180.y,"median","q1q3") + 
            kwt(percent_time_under_70.x,"median","q1q3") + 
            kwt(percent_time_under_70.y,"median","q1q3") + 
            kwt(percent_time_over_180.x,"median","q1q3") + 
            kwt(percent_time_over_180.y,"median","q1q3") + 
            kwt(percent_time_over_250.x,"median","q1q3") + 
            kwt(percent_time_over_250.y,"median","q1q3"),
            cgm_data)
summary(t,pfootnote=T)
```

```{r plots}
boxplot(percent_time_70_180.x~randomization_group.factor,cgm_data,
        ylab = "% TIR",xlab = "Randomization Group",ylim = c(0,90))

boxplot(percent_time_70_180.y~randomization_group.factor,cgm_data,
        ylab = "% TIR - First 72 Hours",xlab = "Randomization Group",ylim = c(0,90))

boxplot(percent_time_under_70.x~randomization_group.factor,cgm_data,
        ylab = "% Time < 70",xlab = "Randomization Group",ylim = c(0,90))

boxplot(percent_time_under_70.y~randomization_group.factor,cgm_data,
        ylab = "% Time < 70 - First 72 Hours",xlab = "Randomization Group",ylim = c(0,90))
```
