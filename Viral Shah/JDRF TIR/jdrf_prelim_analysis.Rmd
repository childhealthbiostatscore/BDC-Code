...
# plots
a1c_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_a1c, group = Group, color = Group)) + scale_x_reverse(breaks = seq(0,8,1))  + scale_y_continuous(breaks = seq(6,11,1))  + theme_classic()
tir_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_tir, group = Group, color = Group)) + scale_x_reverse(breaks = seq(0,8,1)) + theme_classic()
...
a1c_YEAR_p + geom_line() + geom_errorbar(aes(ymin = mean_a1c - sd_a1c, ymax = mean_a1c + sd_a1c), alpha = 0.25) +
  geom_text(aes(label =paste0("n = ",n_grp)),size = 3, nudge_y = 0.52, show.legend = F, nudge_x = -0.25) +
  geom_text(aes(label = round(mean_a1c, 2)), nudge_y = .35, nudge_x = -0.25, show.legend = F)
