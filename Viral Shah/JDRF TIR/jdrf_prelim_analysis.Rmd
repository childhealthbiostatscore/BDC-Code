...
# annualize data
jdrf_a1c = jdrf_a1c %>% group_by(`Study ID`) %>% filter(!is.na(Date)) %>% mutate(visit = row_number(),
                                                                                 visit1 = min(Date),
                                                                                 yrs_since_v1 = time_length(difftime(Date, visit1),"years"),
                                                                                 YEAR = floor(yrs_since_v1) + 1)
...
a1c_tir_by_YEAR = jdrf_a1c %>% group_by(YEAR, Group) %>% summarise(mean_a1c = mean(a1c, na.rm = T),
                                                                     ...
                                                                     n_grp = n())
# plots
a1c_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_a1c, group = Group, color = Group)) + theme_classic()
tir_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_tir, group = Group, color = Group)) + theme_classic()
tar_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_tar, group = Group, color = Group)) + theme_classic()
tbr_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_tbr, group = Group, color = Group)) + theme_classic()
ttir_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_ttir, group = Group, color = Group)) + theme_classic()
mg_YEAR_p = ggplot(data = a1c_tir_by_YEAR, aes(x = YEAR, y = mean_mg, group = Group, color = Group)) + theme_classic()
...
* NEW: When annualizing, some sbjects have visit numbers, but no corresponding dates. these not included when annualizing
...
# tar
Retinopathy status is associated with tar (p = 0.002)
```{r tar}
...
# ttir
Retinopathy status is associated with ttir (p = 0.002)
```{r ttir}
...
# mean glucose
Retinopathy status is associated with mean glucose (p = 0.003)
```{r mg}
