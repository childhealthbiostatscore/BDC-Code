...
cgm_cases <- read_csv("S:/Laura/BDC/Projects/Viral Shah/JDRF/Data_Cleaned/cases.csv")
cgm_controls <- read_csv("S:/Laura/BDC/Projects/Viral Shah/JDRF/Data_Cleaned/controls.csv")
cgm = rbind(cgm_cases, cgm_controls); rm(cgm_cases, cgm_controls)
cgm = cgm %>% mutate(Date = date(ymd_hms(date_cgm_placement)),
                     tbr54 = 100*percent_time_under_54,
                     tar250 = 100*percent_time_over_250) %>% select(Date, standard_deviation, cv, tbr54, tar250, excursions_under_54, estimated_a1c)
```
...
Retinopathy status is not associated with tbr (p = 0.01)
