...
if(Sys.info()["sysname"] == "Windows"){
  home_dir = "B:/Projects/Kim Driscoll/Bring BG Down"
...
fig1a = ggplot(means,aes(x = studyvisit,y = emmean,linetype = factor(treatment_group))) +
  ...
  geom_errorbar(aes(x = studyvisit,y = emmean,
                    ymin=emmean-SE, ymax=emmean+SE),width = 0.1,inherit.aes = F) + 
  scale_linetype_manual(values = c("dashed","solid"),name = "Group",
...
mod = lmer(update(plot_form,Parent.GAD.7.Score ~ .+(1|participant_id)),
           data = df)
means = data.frame(emmeans(mod,~studyvisit+factor(treatment_group)))
fig1d = ggplot(means,aes(x = studyvisit,y = emmean,linetype = factor(treatment_group))) +
  geom_point() + geom_line() + 
  geom_errorbar(aes(x = studyvisit,y = emmean,
                    ymin=emmean-SE, ymax=emmean+SE),width = 0.1,inherit.aes = F) + 
  scale_linetype_manual(values = c("dashed","solid"),name = "Group",
                        labels = c("Control","Treatment")) + 
  ggtitle("Caregiver GAD-7") +
  theme_bw() +
  theme(axis.title.y = element_blank(),axis.title.x = element_blank(),
        plot.title = element_text(hjust = 0.5))
fig1d
...
grid.arrange(arrangeGrob(fig1a + theme(legend.position="none"), 
                         ...
                         nrow = 2,
                         left = textGrob("Mean Score", rot = 90, vjust = 1,
                                         gp = gpar(fontface = "bold",cex=1.5))),
                         bottom = textGrob("Visit", hjust = 1, 
                                           gp = gpar(fontface = "bold",cex=1.5)))
```
