...
# Data management
indir <- paste(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Kim Driscoll/Baseline Pump Paper/Data_Cleaned/Pump Files Original",sep="")
outdir <- paste(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Kim Driscoll/Baseline Pump Paper/Data_Cleaned/Pump Files Cleaned/",sep = "")
...
# Add a midnight row to calculate last bolus before midnight
  six_hour_bolus <- setDT(bolus_dat)[, indx := date][, .SD[1:(.N+1)], indx][,indx := NULL][!.N]
...
# Make a list of times when BG readings occur. Add 15 minutes to each element so
# it's a list of time ranges. Collapse the list and keep unique times. 
  lowBGs <- unique(unlist(lapply((as.list(dat$timestamp[which(dat$bgreading < 70)])),function(x) x:(x+900))))
  intargetBGs <- unique(unlist(lapply((as.list(dat$timestamp[which(dat$bgreading %in% 70:180)])),function(x) x:(x+900))))
  highBGs <- unique(unlist(lapply((as.list(dat$timestamp[which(dat$bgreading %in% 181:250)])),function(x) x:(x+900))))
  veryhighBGs <- unique(unlist(lapply((as.list(dat$timestamp[which(dat$bgreading %in% 251:400)])),function(x) x:(x+900))))
  extremeBGs <- unique(unlist(lapply((as.list(dat$timestamp[which(dat$bgreading > 400)])),function(x) x:(x+900))))
# Count BG readings
  ...
  veryhighBGcount <- length(which(dat$bgreading %in% 250:400))
...
# Percentages 
  summary[f,"perc_lowBG_followed_by_carb_bolus"] <- 
    (min(c(sum(carbtimes %in% lowBGs)),sum(bolus.times %in% lowBGs))/lowBGcount)*100
  summary[f,"perc_lowBG_followed_by_bolus"] <- 
    (sum(bolus.times %in% lowBGs)/lowBGcount)*100
  
  summary[f,"perc_intBG_followed_by_carb_bolus"] <- 
    (min(c(sum(carbtimes %in% intargetBGs)),sum(bolus.times %in% intargetBGs))/intargetBGcount)*100
  summary[f,"perc_intBG_followed_by_bolus"] <- 
    (sum(bolus.times %in% intargetBGs)/intargetBGcount)*100
  
  summary[f,"perc_highBG_followed_by_carb_bolus"] <- 
    (min(c(sum(carbtimes %in% highBGs)),sum(bolus.times %in% highBGs))/highBGcount)*100
  summary[f,"perc_highBG_followed_by_bolus"] <- 
    (sum(bolus.times %in% highBGs)/highBGcount)*100
  
  summary[f,"perc_veryhighBG_followed_by_carb_bolus"] <- 
    (min(c(sum(carbtimes %in% veryhighBGs)),sum(bolus.times %in% veryhighBGs))/veryhighBGcount)*100
  summary[f,"perc_veryhighBG_followed_by_bolus"] <- 
    (sum(bolus.times %in% veryhighBGs)/veryhighBGcount)*100
  
  summary[f,"perc_extremeBG_followed_by_carb_bolus"] <- 
    (min(c(sum(carbtimes %in% extremeBGs)),sum(bolus.times %in% extremeBGs))/extremeBGcount)*100
  summary[f,"perc_extremeBG_followed_by_bolus"] <- 
    (sum(bolus.times %in% extremeBGs)/extremeBGcount)*100
