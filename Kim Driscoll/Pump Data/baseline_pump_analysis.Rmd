...
files <- "/Volumes/som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Kim Driscoll/Baseline Pump Paper/Data_Cleaned/T1_330_test.csv"
...
for (f in 1:length(files)) {
  dat <- read.csv(files,header = T,stringsAsFactors = FALSE,na.strings = "")
  dat$timestamp <- lubridate::mdy_hm(dat$timestamp)
  id <- sub(".csv","",basename(files))
  summary[f,"subject_id"] <- id
  start <- lubridate::mdy("1/19/17")
  end <- lubridate::mdy("1/27/17")
