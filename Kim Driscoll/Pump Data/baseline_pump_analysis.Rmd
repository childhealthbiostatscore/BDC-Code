...
# Counting variables and placeholder
  ...
  bolus.lower <- 0
  bolus.higher <- 0
  if (bolus_dat$bolustype[1] == "Normal") {
    if (bolus_dat$bwzestimate[1] == bolus_dat$bolusvoldelivered[1]) {
      bolus.correct <- bolus.correct + 1
    } else if (bolus_dat$bwzestimate[1] < bolus_dat$bolusvoldelivered[1]) {
      bolus.lower <- bolus.lower + 1
    } else if (bolus_dat$bwzestimate[1] > bolus_dat$bolusvoldelivered[1]) {
      bolus.higher <- bolus.higher + 1
    }
  } else {
    if (bolus_dat$bwzestimate[1] == (bolus_dat$bolusvoldelivered[1] + 
                                     bolus_dat$bolusvoldelivered[2])) {
      bolus.correct <- bolus.correct + 1
                                     } 
    else if (bolus_dat$bwzestimate[1] < (bolus_dat$bolusvoldelivered[1] + 
                                     bolus_dat$bolusvoldelivered[2])) {
...
# Iterate through data rows. Count total boluses
  for (r in 2:nrow(bolus_dat)) {
    ...
    if (time.diff > 20) {
      ...
      double.bolus <- 1
      total.bolus.vol <- 
        bolus_dat$bolusvoldelivered[r] + bolus_dat$bolusvoldelivered[r-1]
      bolus.wiz <- sum(bolus_dat$bwzestimate[r],bolus_dat$bwzestimate[r-1],
                       na.rm = T)
      if (total.bolus.vol == bolus.wiz) {bolus.correct <- bolus.correct + 1} 
      else if (total.bolus.vol < bolus.wiz) {bolus.lower <- bolus.lower + 1}
      else if (total.bolus.vol > bolus.wiz) {bolus.higher <- bolus.higher + 1}
