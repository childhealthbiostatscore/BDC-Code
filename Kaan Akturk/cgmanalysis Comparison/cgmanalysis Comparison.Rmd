...
knitr::opts_knit$set(root.dir = "C:/Users/timbv/Dropbox/Work/Kaan Akturk/cgmanalysis Comparisons/Medtronic, Dexcom, Libre")
library(arsenal)
library(skimr)
library(knitr)
library(cgmanalysis)
...
```{r echo=FALSE}
# TOST function - takes two columns and a difference cutoff on the raw scale
tost = function(vec1,vec2,max_diff,plot = T,verbose = F){
  d = vec2 - vec1
  m = mean(d,na.rm = T)
  sd = sd(d,na.rm = T)
  n = sum(!is.na(d))
  TOSTone.raw(m,0,sd,n,-max_diff,max_diff,
              alpha = 0.05,plot = plot,verbose = verbose)
}
```

For each CGM metric, we tested equivalence using the two one-sided test approach (TOST). This approach does not simply test whether the difference between the two metrics is statistically different from 0, but whether the difference is functionally equivalent to 0 based on the provided bounds. For example, it is possible that the the difference between the two measures is statistically not 0 due to low standard deviation, but that the measures are still practically equivalent in the sense that the average difference is small.

### Percent Time < 54 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(dexcom_package$percent_time_under_54,dexcom_package$X.54,1)
```

The R package and Dexcom software are statistically equivalent (p `r format.pval(max(r$TOST_p1,r$TOST_p2),digits = 3,eps = 0.001)`).

### Percent Time < 70 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(dexcom_package$percent_time_under_70,dexcom_package$X.70,1)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_package$percent_time_70_180,dexcom_package$X70.180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_package$percent_time_over_180,dexcom_package$X.180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_package$percent_time_over_250,dexcom_package$X.250,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_package$standard_deviation,dexcom_package$SD,2)
```
...
```{r echo=FALSE,results='asis'}
#r = tost(dexcom_package$cv,dexcom_package$CV,2)
```

The R package and Dexcom software are exactly the same.
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$dex_u54,dexcom_tandem_package$tandem_u54,1)
```

The Tandem and Dexcom software are statistically equivalent (p `r format.pval(max(r$TOST_p1,r$TOST_p2),digits = 3,eps = 0.001)`).

### Percent Time < 70 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$dex_u70,dexcom_tandem_package$tandem_u70,1)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$dex_70_180,dexcom_tandem_package$tandem_70_180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$dex_a180,dexcom_tandem_package$tandem_a180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$dex_a250,dexcom_tandem_package$tandem_a250,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$dex_sd,dexcom_tandem_package$tandem_sd,2)
```
...
```{r tandem lm plots,echo=FALSE,fig.width=8,fig.height=6}
# Make plots
...
# Make plots
under_54 <- blandr.draw(dexcom_tandem_package$percent_time_under_54_tandem,dexcom_tandem_package$tandem_u54,
                        ciDisplay = F) + theme_bw() + ggtitle("% Time Under 54 mg/dL")
under_70 <- blandr.draw(dexcom_tandem_package$percent_time_under_70_tandem,dexcom_tandem_package$tandem_u70,
                        ciDisplay = F) + theme_bw() + ggtitle("% Time Under 70 mg/dL")
tir_70_180 <- blandr.draw(dexcom_tandem_package$percent_time_70_180_tandem,dexcom_tandem_package$tandem_70_180,
                        ciDisplay = F) + theme_bw() + ggtitle("% Time 70 - 180 mg/dL")
over_180 <- blandr.draw(dexcom_tandem_package$percent_time_over_180_tandem,dexcom_tandem_package$tandem_a180,
                        ciDisplay = F) + theme_bw() + ggtitle("% Time Over 180 mg/dL")
over_250 <- blandr.draw(dexcom_tandem_package$percent_time_over_250_tandem,dexcom_tandem_package$tandem_a250,
                        ciDisplay = F) + theme_bw() + ggtitle("% Time Over 250 mg/dL")
sd <- blandr.draw(dexcom_tandem_package$sd_tandem,
                  dexcom_tandem_package$sd_tandem,
...
```{r echo=FALSE,results='asis'}
# r = tost(dexcom_tandem_package$percent_time_under_54_tandem,dexcom_tandem_package$tandem_u54,1)
```

The R package and Tandem software are exactly the same.

### Percent Time < 70 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$percent_time_under_70_tandem,dexcom_tandem_package$tandem_u70,1)
```

The R package and Tandem software are statistically equivalent (p `r format.pval(max(r$TOST_p1,r$TOST_p2),digits = 3,eps = 0.001)`).

### Percent Time 70-180 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$percent_time_70_180_tandem,dexcom_tandem_package$tandem_70_180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$percent_time_over_180_tandem,dexcom_tandem_package$tandem_a180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$percent_time_over_250_tandem,dexcom_tandem_package$tandem_a250,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(dexcom_tandem_package$sd_tandem,dexcom_tandem_package$tandem_sd,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(libre_package$percent_time_under_54,libre_package$X.54,1)
```

The R package and Libre software are statistically equivalent (p `r format.pval(max(r$TOST_p1,r$TOST_p2),digits = 3,eps = 0.001)`).

### Percent Time < 70 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(libre_package$percent_time_under_70,libre_package$X.70,1)
```
...
```{r echo=FALSE,results='asis'}
r = tost(libre_package$percent_time_70_180,libre_package$X70.180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(libre_package$percent_time_over_180,libre_package$X.180,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(libre_package$percent_time_over_250,libre_package$X.250,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(libre_package$standard_deviation,libre_package$SD,2)
```
...
```{r echo=FALSE,results='asis'}
#r = tost(libre_package$cv,libre_package$CV,2)
```

The R package and Libre software are exactly the same.
...
### Percent Time 40-50 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(medtronic_package$percent_time_40_50,medtronic_package$X.40.50.,1)
```

The R package and Medtronic software are statistically equivalent (p `r format.pval(max(r$TOST_p1,r$TOST_p2),digits = 3,eps = 0.001)`).

### Percent Time < 70 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(medtronic_package$percent_time_50_70,medtronic_package$X.50.70.,1)
```
...
```{r echo=FALSE,results='asis'}
r = tost(medtronic_package$percent_time_70_180,medtronic_package$X.70.180.TIR,2)
```

The R package and Medtronic software are statistically equivalent (p `r format.pval(max(r$TOST_p1,r$TOST_p2),digits = 3,eps = 0.001)`).

### Percent Time 180 - 250 mg/dL

```{r echo=FALSE,results='asis'}
r = tost(medtronic_package$percent_time_180_250,medtronic_package$X.180.250.,2)
```
...
```{r echo=FALSE,results='asis'}
r = tost(medtronic_package$percent_time_250_400,medtronic_package$X.250.400.,2)
```
