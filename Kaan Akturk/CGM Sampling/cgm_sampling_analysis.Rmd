...
```{r analysis}
m3 <- subjects %>% filter(timepoint == "m3")
m3_a1cs 
temp <- m3 %>% pivot_wider(id_cols = "subject_id",names_from = "cutoff",values_from = all_of(c(vars,"a1c")))
c <- cor(temp[,2:ncol(temp)],use="pairwise.complete.obs")
```