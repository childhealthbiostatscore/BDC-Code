...
#knitr::opts_knit$set(root.dir = "/Volumes/som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Kaan Akturk/CGM Sampling")
...
for (d in days) {
  cgmvariables(paste0("S:/Laura/BDC/Projects/Kaan Akturk/CGM Sampling/Data_Cleaned/cgm","/",paste0("day",d)),
               "S:/Laura/BDC/Projects/Kaan Akturk/CGM Sampling/Data_Cleaned/",
...
for (d in days) {
  ...
  dat <- read.csv(paste0("S:/Laura/BDC/Projects/Kaan Akturk/CGM Sampling//Data_Cleaned/",filename))
...
levels(plot$name) <- c("Mean glucose","CV","TIR and HbA1c",
...
p <- ggplot(plot1[plot1$name == "percent_time_70_180",],
            aes(x=sample_days,y=value,color=percent_wear_group)) + geom_line() +
  theme_bw() + ylab("R^2") + xlab("Days in sampling period") +
...
levels(plot1$name) <- c("Mean glucose","CV","TIR and HbA1c",
                        "% 70-180 mg/dL","% >180 mg/dL","% >250 mg/dL",
                        "% <54 mg/dL","% <70 mg/dL")
...
c <- lapply(c, function(x){
  ...
  names(all) <- c("sample_days","n",vars,"mean_hba1c")
  return(all)
}) 
c <- as.data.frame(do.call(rbind,c))
# Plot
...
levels(plot2$name) <- c("Mean glucose","CV","TIR and HbA1c",
                        "% 70-180 mg/dL","% >180 mg/dL","% >250 mg/dL",
                        "% <54 mg/dL","% <70 mg/dL")
p3 <- 
  ggplot(plot2,aes(x=sample_days,y=value,color=name)) + 
  geom_line() +  
  theme_bw() + ylab("R^2") + xlab("Days in sampling period") +
  scale_color_discrete(name="") +
  scale_x_continuous(breaks = days[seq(1,length(days),2)]) + 
