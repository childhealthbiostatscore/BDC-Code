...
These are pretty nice looking residuals! Not exactly normal but I don't think they violate any assumptions, so this model is good to go.
...
Technically, the model without insurance is the best again, but we need to keep it in there (mostly so reviewers don't ask why you didn't control for it). Also the difference in AIC is small, so we could really pick either the model without insurance or without income. Let's check the models with and without interaction terms as well, just to be sure:
...
There was an overall effect of time on % high boluses without carb, but neither visit on its own was significantly different from baseline (I know this is a little odd, so we can chat about it when we go over the results). There were also overall effects of HbA1c and sex, but nothing else was significant overall. 

There was a significant difference between treatment groups at study visit 2.
