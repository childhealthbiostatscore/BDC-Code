...
```{r}
full_mod <- lme(readings_per_day ~ Treatment + StudyVisit + Sex + HbA1c + Treatment:HbA1c + 
                     age_years + T1D_dur_years + IncomeLevel + Treatment:IncomeLevel +
...
In general, VIF > 10 is a concern so it looks like we don't need to worry about multicollinearity here. You can ignore the high values for treatment, since it's involved in interaction terms and obviously the interactions term will be correlated with the direct effect term.
...
```{r}
reduced_mod <- lme(readings_per_day ~ Treatment + StudyVisit + HbA1c + Treatment:HbA1c + 
                     age_years + readings_per_day_b,random =~1|Site/ID, data = dat,na.action = na.omit) 
```
...
```{r}
t1d_dur_mod <- lme(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                     Treatment:HbA1c + age_years + readings_per_day_b + T1D_dur_years,
                   random =~1|Site/ID, data = dat,na.action = na.omit)
income_mod <- lme(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                    Treatment:HbA1c + age_years + readings_per_day_b + IncomeLevel,
                  random =~1|Site/ID, data = dat,na.action = na.omit)
sex_mod <- lme(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                    Treatment:HbA1c + age_years + readings_per_day_b + Sex,
                  random =~1|Site/ID, data = dat,na.action = na.omit)
insurance_mod <- lme(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                    Treatment:HbA1c + age_years + readings_per_day_b + Insurance,
                  random =~1|Site/ID, data = dat,na.action = na.omit)
```

```{r echo=FALSE}
t1d_dur_mod <- lmer(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                     Treatment:HbA1c + age_years + readings_per_day_b + T1D_dur_years + (1|Site/ID), 
                    data = dat,na.action = na.omit)
income_mod <- lmer(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                    Treatment:HbA1c + age_years + readings_per_day_b + IncomeLevel + (1|Site/ID), 
                   data = dat,na.action = na.omit)
sex_mod <- lmer(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                    Treatment:HbA1c + age_years + readings_per_day_b + Sex + (1|Site/ID), data = dat,na.action = na.omit)
insurance_mod <- lmer(readings_per_day ~ Treatment + StudyVisit + HbA1c + 
                    Treatment:HbA1c + age_years + readings_per_day_b + Insurance + (1|Site/ID), data = dat,na.action = na.omit)
```

### Compare AIC

```{r echo=FALSE,warning=FALSE}
kable(AIC(full_mod,t1d_dur_mod,sex_mod,insurance_mod,reduced_mod))
```

The model including income did not converge with a treatment:income interaction term, so the interaction was omitted for this comparison. Regardless, the full model is still the best by AIC.
...
glm1 <- try(glmmLasso(readings_per_day ~ as.factor(Treatment) + as.factor(StudyVisit) + 
                        as.factor(Sex) + HbA1c + as.factor(Treatment):HbA1c + age_years + 
                        T1D_dur_years + as.factor(IncomeLevel) + as.factor(Treatment):as.factor(IncomeLevel) + 
                        as.factor(Insurance) + readings_per_day_b,
            data = lasso_data,rnd = list(ID=~1 + Site),lambda = 1,final.re=TRUE))  
...
lasso <- glmmLasso(readings_per_day ~ as.factor(Treatment) + as.factor(StudyVisit) + 
                        as.factor(Sex) + HbA1c + as.factor(Treatment):HbA1c + age_years + 
                        T1D_dur_years + as.factor(IncomeLevel) + Treatment:as.factor(IncomeLevel) + 
                        Insurance + readings_per_day_b,data = lasso_data,rnd = list(ID=~1),lambda = lambda[opt],final.re=TRUE)
```
