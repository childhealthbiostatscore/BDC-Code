...
# Use AIC to find optimal tuning parameter lambda (from glmmLasso demo)
...
as the final model for readings per day.
...
bolus_mod_full <- lme(highBG_without_carb_with_bolus ~ Treatment*HbA1c + StudyVisit + Sex + 
                        age_years + T1D_dur_years + IncomeLevel + Treatment:IncomeLevel + 
                        Insurance + highBG_without_carb_with_bolus_b,
                     random =~1|Site/ID, data = dat,na.action = na.omit)

no_t1d_or_sex_bolus_mod <- lme(highBG_without_carb_with_bolus ~ Treatment*HbA1c + StudyVisit + 
                                      age_years + IncomeLevel + Treatment:IncomeLevel + 
                                      Insurance + highBG_without_carb_with_bolus_b,
                     random =~1|Site/ID, data = dat,na.action = na.omit)

bolus_mod_reduced <- lme(highBG_without_carb_with_bolus ~ Treatment*HbA1c + 
                           StudyVisit + age_years + highBG_without_carb_with_bolus_b,
                     random =~1|Site/ID, data = dat,na.action = na.omit)
```

```{r echo=FALSE,warning=FALSE}
kable(AIC(bolus_mod_full,no_t1d_or_sex_bolus_mod,bolus_mod_reduced))
```

Technically the full model is best by AIC, but the general rule of thumb is that a difference of 2 or more is significant so it would be okay to use the model without sex or T1D duration here as well. Let's try the lasso approach again, just for fun. 
...
AIC_vec<-rep(Inf,length(lambda))
for(j in 1:length(lambda)){
glm1 <- try(glmmLasso(highBG_without_carb_with_bolus ~ as.factor(Treatment) + as.factor(StudyVisit) + 
                        as.factor(Sex) + HbA1c + age_years + 
                        T1D_dur_years + as.factor(IncomeLevel) + 
                        as.factor(Insurance) + highBG_without_carb_with_bolus_b,
            data = lasso_data,rnd = list(ID=~1),lambda = lambda[j],final.re=TRUE))  
if(class(glm1)!="try-error"){AIC_vec[j]<-glm1$aic}}
opt<-which.min(AIC_vec)
```
...
```{r}
lasso <- glmmLasso(highBG_without_carb_with_bolus ~ as.factor(Treatment) + as.factor(StudyVisit) + 
                        as.factor(Sex) + HbA1c + age_years + 
                        T1D_dur_years + as.factor(IncomeLevel) + 
                        as.factor(Insurance) + highBG_without_carb_with_bolus_b,
            data = lasso_data,rnd = list(ID=~1),lambda = lambda[opt],final.re=TRUE)
...
Lasso removes sex and T1D duration as well, so I think it's safe to use this model structure for every outcome. 