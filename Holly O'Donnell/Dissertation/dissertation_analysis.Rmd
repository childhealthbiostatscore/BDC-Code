...
# Import data
dat <- 
  read.csv("/Users/timvigers/Desktop/Dissertation/Data_Raw/diss_data_7.22.19.csv",
           na.strings = "")
...
dat$income_three[dat$income_three == "1" | dat$income_three == "2"] <- "Low"
dat$income_three[dat$income_three == "3" | dat$income_three == "4"] <- "Medium"
dat$income_three[dat$income_three == "5" | dat$income_three == "6"] <- "High"
dat$income_three <- as.factor(dat$income_three)
...
kable(summary(bg_mod)$coefficients,caption = "Fixed Effects")
```
...
```{r}
highBG_mod <- lmer(highBG_without_carb_with_bolus_piu ~ Treatment*StudyVisit + 
                   HbA1c + Sex + age_years + Insurance + IncomeLevel + 
                   highBG_without_carb_with_bolus_b + (1|Site/ID), data = dat)
```
...
kable(summary(highBG_mod)$coefficients,caption = "Fixed Effects")
```
...
```{r}
carb_mod <- lmer(carb_inputs_per_day ~ Treatment*StudyVisit + HbA1c + Sex + 
                  age_years + income_three + carb_inputs_per_day_b + (1|Site/ID),
                 data = dat)
```
...
kable(summary(carb_mod)$coefficients,caption = "Fixed Effects")
```
...
kable(summary(bolus_day_mod)$coefficients,caption = "Fixed Effects")
```
...
extreme_bolus_mod <- lmer(extremeBG_without_carb_with_bolus_piu ~ 
                            ...
                            income_three + extremeBG_without_carb_with_bolus_b +
                            (1|Site/ID), data = dat)
```
...
kable(summary(extreme_bolus_mod)$coefficients,caption = "Fixed Effects")
```
...
<!-- # Models with income replacing insurance -->

<!-- ```{r echo=FALSE} -->
<!-- # Power Analysis -->
<!-- # PA done with GLIMMPSE, cross tabs for group sizes page -->
<!-- cross_tabs <- xtabs(formula = ~Insurance+Sex+Treatment,data = dat) -->
<!-- cross_tabs -->
<!-- means <- plyr::ddply(dat,~Insurance+Sex+Treatment+StudyVisit,summarise,mean(extremeBG_without_carb_with_bolus,na.rm=T)) %>% -->
<!--   arrange(StudyVisit,Treatment,Sex) -->
<!-- means -->
<!-- ``` -->