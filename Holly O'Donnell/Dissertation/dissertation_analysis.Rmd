...
bg_mod <- lme(readings_per_day ~ Treatment*StudyVisit + HbA1c + Sex + 
                age_years + income_three + readings_per_day_b,random = ~1|Site/ID,
...
highBG_mod <- lme(highBG_without_carb_with_bolus_piu ~ Treatment*StudyVisit + 
                   ...
                   highBG_without_carb_with_bolus_b,random = ~1|Site/ID, data = dat,
                   na.action = na.omit)
```
...
carb_mod <- lme(carb_inputs_per_day ~ Treatment*StudyVisit + HbA1c + Sex + 
                  age_years + income_three + carb_inputs_per_day_b,
...
bolus_day_mod <- lme(boluses_per_day ~ Treatment*StudyVisit + HbA1c + Sex + 
                  age_years + income_three + boluses_per_day_b,random = ~1|Site/ID,
                  data = dat,na.action = na.omit)
```
...
```{r}
extreme_bolus_mod <- lme(extremeBG_without_carb_with_bolus_piu ~ 
