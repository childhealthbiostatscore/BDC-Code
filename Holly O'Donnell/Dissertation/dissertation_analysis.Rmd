...
full_mod_l <- lme(readings_per_day_change ~ Treatment*zDEPRESSION*StudyVisit + HbA1c +
                     Sex + age_years + T1D_dur_years + IncomeLevel + Insurance,random = ~1|ID,na.action = na.omit, 
                   data = dat) # Change score works without nested random effect
reduced_mod_l <- lme(readings_per_day_change ~ Treatment*zDEPRESSION*StudyVisit,random =~1|ID,na.action = na.omit, 
                   data = dat)
```
...
- Send Allison paper on change scores
