...
# Format dates
data_full[,c("date_death","date_of_tx","infxn_date","sens1_place","followup")] <- 
  lapply(data_full[,c("date_death","date_of_tx","infxn_date","sens1_place","followup")],
         lubridate::ymd)
```
...
for (r in 1:nrow(wear_plot_data)) {
  wear_plot_data$dates[r] <- 
    paste0(seq(as.Date(wear_plot_data$sens_place[r]),as.Date(wear_plot_data$sens_remove[r]),by = "days"),
           collapse = ",")
}
...
# Get earliest infection for each participant and censoring info
...
# Truncate CGM date at infection
cgm <- left_join(cgm,events[,c("study_id","event","sex")],by = "study_id")
cgm <- cgm %>% filter(Time < event)
# Time from Tx and first CGM value
cgm <- cgm %>% group_by(study_id) %>% 
  mutate(time_from_tx = difftime(Time,Tx_date,units = "days"),
         time_from_cgm_start = difftime(Time,Time[1],units = "days"))
events <- events %>%
  mutate(time_to_event_from_tx = difftime(event,date_of_tx,units = "days"),
         time_to_event_from_cgm_start = difftime(event,first_cgm,units = "days"),
         time_first_cgm_to_tx = difftime(date_of_tx,first_cgm,units = "days"))
# Average glucose by hour
cgm$date_hour <- lubridate::date(cgm$Time)
cgm <- cgm %>% group_by(study_id,date_hour) %>%
  summarise(sg = mean(Historic_Glucose,na.rm = T)) %>%
  mutate(time_from_cgm_start = difftime(date_hour,date_hour[1],units = "days"))
```

```{r infection joint models,echo=FALSE}
# Random intercept and random slope
mixfit <- lme(sg ~ time_from_cgm_start,
              random = ~1|study_id,data = cgm,method = "ML")
# Basic time to infection model
survfit <- coxph(Surv(time_to_event_from_cgm_start,infection)~1,
                   data = events,x=T)
# Joint fit
