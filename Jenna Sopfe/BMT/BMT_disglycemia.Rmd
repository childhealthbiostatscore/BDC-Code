...
cgm <- cgm %>% group_by(study_id) %>% 
  mutate(time_from_tx = difftime(Time,Tx_date,units = "mins"),
         time_from_cgm_start = difftime(Time,Time[1],units = "mins"))
events <- events %>%
  mutate(time_to_event_from_tx = difftime(event,date_of_tx,units = "mins"),
         time_to_event_from_cgm_start = difftime(event,first_cgm,units = "mins"),
         time_first_cgm_to_tx = difftime(date_of_tx,first_cgm,units = "mins"))
# Scale time to days
cgm$time_from_cgm_start <- cgm$time_from_cgm_start / 60
events$time_to_event_from_cgm_start <- events$time_to_event_from_cgm_start / 60
```

```{r infection joint models,echo=FALSE}
# Random intercept and random slope
mixfit <- lme(Historic_Glucose ~ time_from_cgm_start + time_from_cgm_start:sex,
              random = ~time_from_cgm_start|study_id,data = cgm)
# Basic time to infection model
survfit <- coxph(Surv(time_to_event_from_cgm_start,infection)~sex,
                   data = events,x=T)
# Joint fit
jointFit <- jointModel(mixfit, survfit, timeVar = "time_from_cgm_start",
                       method = "spline-PH-aGH",iter.EM = 200)
```