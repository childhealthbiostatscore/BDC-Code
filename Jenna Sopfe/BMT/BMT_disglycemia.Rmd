...
```{r echo=FALSE}
temp <- data_full %>% filter(!is.na(date_of_tx)) %>%
  select(study_id,date_of_tx,sens1_place,sens1_remove,
         sens2_place,sens2_remove,sens3_place,sens3_remove,sens4_place,
         sens4_remove,sens5_place,sens5_remove,sens6_place,sens6_remove)
l <- apply(temp[,3:ncol(temp)],1,function(x){
  return(x)
})
```