...
# Survival part
survfit <- coxph(Surv(time_to_event_from_cgm_start,infection)~sex + Race,
                   data = events,x=T)
# Simplest model
# Joint fit
...
Time-Dependent Slopes Results

```{r echo=FALSE}
kable(summary(jointFit_deriv)$`CoefTable-Long`,caption = "Longitudinal Process")
```

```{r echo=FALSE}
kable(summary(jointFit_deriv)$`CoefTable-Event`,caption = "Event Process")
```

### Polynomial Time Model

```{r infection jm poly,echo=FALSE}
# Polynomial model
mixfit_poly <- lme(sg ~ poly(time_from_cgm_start,3) + age,
                   data = cgm,method = "ML",
              random = ~time_from_cgm_start|study_id)
# Survival part
survfit <- coxph(Surv(time_to_event_from_cgm_start,infection)~sex + Race + AgeTx,
                   data = events,x=T)
# Joint fit
jointFit <- jointModel(mixfit_poly, survfit, timeVar = "time_from_cgm_start",
                       method = "piecewise-PH-GH")
# Joint fit with time-dependent slopes
# dForm = list(fixed = ~ I(2*time_from_cgm_start) + 
#                I(3*time_from_cgm_start^2),indFixed = 2:4,
#              random = ~1,indRandom = 2)
# jointFit_deriv <- jointModel(mixfit_poly, survfit, timeVar = "time_from_cgm_start",
#                        method = "piecewise-PH-GH",parameterization = "both",
#                        derivForm = dForm)
```
...
Unable to get this to converge.
