...
gluc_table = tableby(time_cat ~ avg + cv + kwt(perc_70,"median","q1q3") + 
                       ...
                       kwt(num_exc_140,"median","q1q3"),data = cgm_summ)
...
```{r infection avg,echo=FALSE}
# ROC curves
#************************************* MAKE THIS A FUNCTION********************
pre_roc = roc(Infection ~ avg,data = pre_tx,quiet = T)
imm_post_roc = roc(Infection ~ avg,data = imm_post_tx,quiet = T)
post_roc = roc(Infection ~ avg,data = post_tx,quiet = T)
# Plot
plot(pre_roc, col = "blue",main = "Average SG (mg/dL)")
plot(imm_post_roc,col = "dark green",add = T)
plot(post_roc,col = "red",add = T)
legend(1.32, 0.95, legend=levels(cgm_summ$time_cat),
       col=c("blue","dark green","red"), lty=1)
ret = c("threshold","specificity","sensitivity")
results = bind_rows(coords(pre_roc,"best",transpose = F),
                    coords(imm_post_roc,"best",transpose = F),
                    coords(post_roc,"best",transpose = F))
rownames(results) = levels(cgm_summ$time_cat)
results$auc = as.numeric(c(pre_roc$auc,imm_post_roc$auc,post_roc$auc))
kable(round(results,2),row.names = T)
```