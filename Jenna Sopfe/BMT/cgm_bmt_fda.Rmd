...
traj = function(df,n_class=NULL){
  ...
  new_df = data.frame("time_from_tx" = seq(min(df$time_from_tx,na.rm = T),max(df$time_from_tx,na.rm = t)))
  # Base model (one class)
  m1 = hlme(Historic_Glucose ~ time_from_tx,subject = 'SubjectID',
            data=df,ng = 1,verbose = FALSE)
  # Increase number of classes
  if(is.null(n_class)){n_class = length(unique(df$SubjectID))}
  for (c in 2:n_class) {
    m = hlme(Historic_Glucose ~ time_from_tx,mixture = ~time_from_tx,
             subject = 'SubjectID',data=df, ng = c,B = m1,verbose = FALSE)
    assign(paste0("m",c),m,envir = globalenv())
    assign(paste0("p",c),predictY(m,new_df,var.time = "time_from_tx"),envir = globalenv())
  }
  # Print model metrics
  do.call(summaryplot,mget(paste0("m",1:n_class)))
}
plot_traj = function(pred_obj,n_class=5){
  plot_df = data.frame(pred_obj$times,pred_obj$pred)
  plot_df = plot_df %>% pivot_longer(-time_from_tx)
  plot_df$name = sub("Ypred_","",plot_df$name)
  ggplot(plot_df,aes(x = time_from_tx,y = value,color = name)) + 
    geom_line() + 
...
```{r}
traj(cgm[cgm$time_from_tx >=0 & cgm$time_from_tx <= 1440,])
```
