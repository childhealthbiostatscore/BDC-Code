...
fda_df = cgm %>% group_by(SubjectID) %>% fill(tx_type) %>% ungroup() %>%
  filter(time_from_engraft >= 0 & time_from_engraft <= (days*24*60)) %>% 
...
```{r warning=FALSE}
fpca = rfr_fpca(Y = "Historic_Glucose", data = fda_df, pve = 0.95)
fda_df %>% 
  modelr::add_predictions(fpca, var = "gluc_fits") %>% 
  ggplot(aes(y = gluc_fits)) + 
  geom_spaghetti(alpha = .4) + 
  geom_spaghetti(aes(y = Historic_Glucose), color = "blue", alpha = .3)
```

```{r}
vars = paste0("Historic_Glucose_",1:(days*24*60))
fda_wide = cgm_wide %>% select(all_of(vars)) %>% data.matrix(.)
fit.fpca = fpca.sc(fda_wide)
plot_shiny(fit.fpca)
```
