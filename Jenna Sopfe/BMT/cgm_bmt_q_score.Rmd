...
q_scores <- lapply(cgm_list, function(df) {
  ...
  mbg <- mean(df$mmol, na.rm = T)
  r <- abs(diff(range(df$mmol, na.rm = T)))
  df$round_time <- lubridate::round_date(df$Time, unit = "5 minutes")
  df$round_time <- sapply(strsplit(as.character(df$round_time), " "), "[[", 2)
  modd <- df %>%
    group_by(round_time) %>%
...
q_scores_pre_post <- lapply(cgm_list, function(df) {
  ...
  s = sd(df$Historic_Glucose,na.rm = T)
  cv = s/mbg
  return(c(id = id,"time"=time,"q"=q,"mean"=mbg,"cv"=cv))
})
...
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank")
...
```{r results='asis'}
labels = list("q" = "Q score","mean" = "Mean Glucose","cv" = "Glucose CV")
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank",strata = tx_type)
summary(p,pfootnote = T,labelTranslations = labels)
```
...
```{r results='asis'}
labels = list("q" = "Q score","mean" = "Mean Glucose","cv" = "Glucose CV")
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank",strata = primary_dx_cat)
summary(p,pfootnote = T,labelTranslations = labels)
```
...
```{r results='asis'}
labels = list("q" = "Q score","mean" = "Mean Glucose","cv" = "Glucose CV")
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank",strata = prep_regimen)
summary(p,pfootnote = T,labelTranslations = labels)
```
...
```{r results='asis'}
labels = list("q" = "Q score","mean" = "Mean Glucose","cv" = "Glucose CV")
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank",strata = any_icu)
summary(p,pfootnote = T,labelTranslations = labels)
```
...
```{r results='asis'}
labels = list("q" = "Q score","mean" = "Mean Glucose","cv" = "Glucose CV")
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank",strata = infections_yn)
summary(p,pfootnote = T,labelTranslations = labels)
```
...
```{r results='asis'}
labels = list("q" = "Q score","mean" = "Mean Glucose","cv" = "Glucose CV")
p = paired(time~q+mean+cv,data = pre_post,id = id,numeric.test = "signed.rank",strata = post_tx_tpn)
summary(p,pfootnote = T,labelTranslations = labels)
```
