...
t1 <- as.data.frame(print(t1, nonnormal = c("AgeTx","bmi_percentile"),printToggle = F))
kable(t1)
```
...
# Add AE information
ae <- data_full %>% select(study_id,adv_event:ae_attrib) %>% filter(!is.na(adv_event)) %>%
  group_by(study_id) %>%
...
t2_by_patient <- as.data.frame(print(t2_by_patient, missing = T,printToggle = F,
                       nonnormal = c("num_cgms","total_days_worn","total_days_post_tx",
                                     "last_cgm_removal_post_tx")))
colnames(t2_by_patient) <- c("Overall","% Missing")
rownames(t2_by_patient) <- c("n","Number of CGMs (median [IQR])",
...
### Reasons for Removal, All CGMs

```{r echo=FALSE}
t2_all <- CreateTableOne("sens_reason",data = cgm_data)
t2_all <- as.data.frame(print(t2_all,printToggle = F))
row.names(t2_all)[2] <- "Sensor Removal Reason (%)"
kable(t2_all)
```
