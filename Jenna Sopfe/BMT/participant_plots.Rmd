...
events <- data_full %>% group_by(study_id) %>%
  select(study_id,date_of_tx,infxn_date,time_engraft,date_death,time_gf,
         acute_gvhd_date,ips_date,prim_icu_dates) %>%
...
events[,c("icu_start","icu_end")] <- 
  lapply(events[,c("icu_start","icu_end")],lubridate::mdy)
...
plot_events <- function(id,cgm_data,event_data){
  # Filter data
  ...
  p <- ggplot(cgm,aes(x=mins_from_tx,y=Historic_Glucose)) + 
    ylim(40,400) + 
    # xlim(min(cgm$mins_from_tx),max(cgm$mins_from_tx)) + 
...
```{r plot loop,echo=FALSE}
invisible(lapply(unique(cgm_data$id), function(x){plot_events(x,cgm_data,events)}))
```