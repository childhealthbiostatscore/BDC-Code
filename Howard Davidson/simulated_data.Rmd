...
t = lapply(sims, function(s){
  ...
  # Agreement
  return(list("elastic" = jaccard(elastic,s$true_metabs),
              "difacto" = jaccard(difacto,s$true_metabs)))
})
stopCluster(cl)
```

# Questions
1. What measure of agreement to use? Cohen's Kappa? Do we need to account for accidentally picking the correct measure?
  - Or do we want some sort of continuous measure like RMSE for the model re-fit with selected variables?
