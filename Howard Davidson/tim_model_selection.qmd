...
# Base recipe (no transform)
small_subset_recipe <- 
  recipe(y ~ ., data = small_subset) %>%
  step_nzv(all_predictors()) %>%
  step_normalize(all_predictors())
# EDA for small subset without Yeo-Johnson
eda = prep(small_subset_recipe,small_subset)
eda = bake(eda,new_data = small_subset)
eda %>%
    create_report(
        output_file = "EDA_small_subset_no_transform.html",
        output_dir = "~/Documents/Work/BDC/Howard Davidson/TEDDY data/Reports",
        report_title = "EDA: Small Subset (No Transform)",
        y = "y"
    )
# EDA for small subset with Yeo-Johnson
small_subset_recipe = small_subset_recipe %>%
  step_YeoJohnson(all_predictors()) 
eda = prep(small_subset_recipe,small_subset)
eda = bake(eda,new_data = small_subset)
eda %>%
    create_report(
        output_file = "EDA_small_subset.html",
        output_dir = "~/Documents/Work/BDC/Howard Davidson/TEDDY data/Reports",
        report_title = "EDA: Small Subset",
        y = "y"
    )
```
...
Using a missingness cutoff of \< 50% results in `r nrow(small_subset)` observations of `r ncol(small_subset)-1` variables. Increasing this threshold to 80% results in `r nrow(large_subset)` observations of `r ncol(large_subset)-1` variables.
...
Unfortunately, ElasticNet does not seem to produce a particularly helpful model in this case.
...
sPLS-DA does a better job of distinguishing classes, with an AUC of approximately 0.71.
...
1.  What is going on with the missing data threshold? Is this due to the transcriptomics data and should we exclude that? Is there something we're missing with these data?
