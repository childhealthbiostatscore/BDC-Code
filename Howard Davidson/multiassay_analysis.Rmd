...
# Demographics
demographics = read.csv("./Data_Raw/TEDDY_MP193_Data/MP193_DEMOGRAPHICS_2017-04-25_17-44-49.csv")
# Combine
df = left_join(demographics,X$carotenoids,by = c("mask_id" = "id")) %>% 
  rename(id = mask_id)
df = left_join(df,X$ascorbic_acid,by = c("id" = "ID","time","y"))
df = left_join(df,X$cholesterol,by = c("id" = "ID","time","y"))
df = left_join(df,X$fatty_acids,by = c("id", "y", "time"))
df = left_join(df,X$metabolomics,by = c("y", "id", "time"))
df = left_join(df,X$growth,by = c("y", "id", "time"))
df = left_join(df,X$negative_lipidomics,by = c("y", "id", "time"))
df = left_join(df,X$positive_lipidomics)
df = left_join(df,X$tocoperol,by = c("id", "y", "time"))
df = left_join(df,X$vitamin_d,by = c("id" = "ID","time","y"))
df = left_join(df,snps,by = c("id", "y", "time"))
```
...
corr_cluster = function(df,parallel_ratio = 0.5,folds,
...
This model resulted in a CV accuracy of `r round(mod$results$Accuracy[as.numeric(rownames(mod$bestTune))],3)` (accuracy SD `r round(mod$results$AccuracySD[as.numeric(rownames(mod$bestTune))],3)`) and was fit on `r nobs(mod$finalModel)` observations. 
...
elastic = function(df,folds,outcome = "y",id_cols = c("id","ID","time")){
  # Outcome 
  df[,outcome] = factor(df[,outcome])
  # Predictors
  pred = setdiff(colnames(df),c(id_cols,outcome))
...
1. Variables with >60% missing data were excluded to maximize the number of complete case observations.
