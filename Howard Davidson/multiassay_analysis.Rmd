...
corr_cluster = function(df,parallel_ratio = 0.5,
  ...
  model_perf = lapply(rs, function(r){
    ...
    if(length(best_cor)>0){
      ...
      return(rmse(mod,normalized = T))
    } else {return(NA)}
  })
  opt_r = rs[which.min(model_perf)]
  # final groups
  # Correlation cutoff
...
```{r eval=FALSE}
transcript = data_preprocces(X$transcriptomics,cv_cutoff = 0.2)
transcript_best = corr_cluster(transcript,heatmap=F,network=F) 
kable(transcript_best,col.names = "Selected variables")
```

The optimal $\rho$ cutoff was `r transcript_best$r_cutoff` based on normalized RMSE.
...
lasso_vars = c("ascorbic_acid",carotenoids_best$best,"cholesterol",
               fa_best$best,metab_best$best,transcript_best,
...
mod = glmnet(x = data.matrix(lasso_df[,lasso_vars]), 
             y = factor(lasso_df[,"y"]), 
             family = "binomial",lambda = cv_fit$lambda[20])
kable(rownames(coef(mod))[which(coef(mod) != 0)],col.names = "Selected variables")
```

The lasso is most likely picking variables that are highly correlated. Variable inflation factors for the final model are:

```{r}
pred = rownames(coef(mod))[which(coef(mod) != 0)]
pred = pred[pred != "(Intercept)"]
form = as.formula(paste0("y~",paste0(pred,collapse = "+")))
mod = glm(form,data = df,family = "binomial")
vif(mod)
```
...
```{r}
# Make single dataset
elastic_df = X$ascorbic_acid
elastic_df = full_join(elastic_df,X$carotenoids,by = c("ID" = "id","y", "time"))
elastic_df = full_join(elastic_df,X$cholesterol,by = c("y", "ID", "time"))
elastic_df = full_join(elastic_df,X$fatty_acids,by = c("ID" = "id","y", "time"))
elastic_df = full_join(elastic_df,X$metabolomics,by = c("ID" = "id","y", "time"))
elastic_df = full_join(elastic_df,X$growth,by = c("ID" = "id","y", "time"))
elastic_df = full_join(elastic_df,X$negative_lipidomics,by = c("ID" = "id","y", "time"))
elastic_df = full_join(elastic_df,X$positive_lipidomics,
              by = c("ID" = "id","y", "time", "id_pc__p_32_0__or_pc__o_32_1__5_", "id_pc__p_34_2__or_pc__o_34_3__5_", 
                     "id_pc__p_36_1__or_pc__o_36_2__6_", "id_pc__p_36_2__or_pc__o_36_3__5_", 
                     "id_pc__p_36_3__or_pc__o_36_4__5_", "id_pc__p_40_1__or_pc__o_40_2__6_", 
                     "id_pc__p_40_3__or_pc__o_40_4__6_", "id_pc__p_42_4__or_pc__o_42_5__6_", 
                     "id_pc__p_44_4__or_pc__o_44_5__7_", "id_pe__p_34_1__or_pe__o_34_2__6_", 
                     "id_pe__p_36_1__or_pe__o_36_2__6_", "id_pe__p_36_2__or_pe__o_36_3__6_", 
                     "id_pe__p_36_4__or_pe__o_36_5__5_", "id_pe__p_38_4__or_pe__o_38_5__6_", 
                     "id_pe__p_38_5__or_pe__o_38_6__5_", "id_pe__p_40_5__or_pe__o_40_6__6_"))
elastic_df = full_join(elastic_df,X$tocoperol,by = c("ID" = "id","y", "time"))
elastic_df = full_join(elastic_df,X$vitamin_d,by = c("y", "ID", "time"))
elastic_df = full_join(elastic_df,X$transcriptomics,by = c("ID" = "id","y", "time"))
# Data appear to be in almost two different sets - some variables are missing in rows 227 and above, some are only in those rows

# Process
# elastic_df = data_preprocces(elastic_df,id_cols = c("ID","time"))
# Complete cases
# X = elastic_df[,setdiff(colnames(elastic_df),c("y","ID","time"))]
# elastic_df = elastic_df[complete.cases(elastic_df),]
```
...
form = as.formula(paste0("y~",
                         paste0(setdiff(colnames(elastic_df),c("y","ID","time")),
                                collapse = "+")))
# Use caret for elasticnet (better with missing data)
cv = trainControl(method = "cv", number = 10)
hit_elnet = train(form, data = elastic_df,method = "glmnet",trControl = cv)
