...
corr_cluster = function(df,parallel_ratio = 0.5,
                        ...
                        optimize_corr = T, corr_cutoff = 0.7,
...
carotenoids_best = corr_cluster(carotenoids,corr)
...
transcript_best = corr_cluster(transcript,heatmap=F,network=F,
                               optimize_corr = F,corr_cutoff = 0.7)
...
# lasso_vars = c(lasso_vars,transcript_best)
...
elastic = function(df,folds = 10,
  ...
  # Biggest model 
  # l = cv$lambda[max(which(cv$cvm <= min(cv$cvm) + sd(cv$cvm)))]
