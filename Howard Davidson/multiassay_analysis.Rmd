...
corr_cluster = function(df,corr_cutoff = 0.7,parallel_ratio = 0.5,
  ...
  mod = glmnet(x=data.matrix(lasso_df[,best_cor]),
               y=data.matrix(lasso_df[,outcome]),
               family="binomial",lambda=cv_fit$lambda.min)
  best_cor = rownames(coef(mod))[which(coef(mod)!=0)]
  best_cor = best_cor[best_cor != "(Intercept)"]
  # Plots
  if(heatmap){
...
lasso_vars = c("ascorbic_acid",carotenoids_best$best,"cholesterol",
               ...
               "agemos",negative_best$best,positive_best$best,
