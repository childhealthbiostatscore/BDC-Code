...
##### ADD IN BMI, GENDER, and AGE

```

```{r enet a1cslope, include = FALSE}

```

# Methods

Due to sample size restrictions, an elastic net could not be performed on the entire dataset with converging models.

Outcomes for variable selection were defined as the linear rate of change over time for IDDA1c (Est Daily Dose, A1c, Est Cpep also derived).

Percent expressions were calculated for each cluster by donor; about 500 genes from each cluster with the highest percent expression among all donors were used for initial variable selection. 

Univariate regressions were performed on the IDDA1C slopes for each gene in the initial variable selection list. Genes associated with the IDDA1C slope at the 0.1 significance level were included in the elasticnet variable selection.

Leave One Out cross validation was performed for elasticnet variable selection methods. Models within 1 SE were examined, with the best model being chosen by MSE. 

Genes selected were included in a mixed model with random intercept for subject, adjusting for gender, age, bmi, time (defined as the average of visits in 6 month increments) and baseline outcome values. Model summaries were produced, and model-predicted values were compared to observed values in the figures below.
...
- selected genes:  `r idda1c_slope_en_small`