...
expressions_df_idda1c_mixed = expressions_df_idda1c_mixed %>% dplyr::select(IDDA1C_binned,idda1c_bl,donor,visit_binned, all_of(idda1c_slope_en_small))
...
idda1c_mixed_mod = lmer(IDDA1C_binned ~ idda1c_bl + visit_binned + RPL35A_c_5 + RPS3_c_7 + RPS6_c_8 + HNRNPDL_c_10 + RPS10_c_12 + HSP90AA1_c_12 + RPL3_c_12 + (1|donor),
            data = expressions_df_idda1c_mixed)
# summary(idda1c_mixed_mod)

# plot predicted values of idda1c
expressions_df_idda1c_mixed%>% na.omit() %>% dplyr::mutate(pred_idda1c = predict(idda1c_mixed_mod, newdata=.,re.form=~(1|donor))) %>%
  ggplot(aes(x = visit_binned, y = IDDA1C_binned)) + 
  geom_point()+
  geom_smooth(aes(y = pred_idda1c), method = "lm",color = "black")
