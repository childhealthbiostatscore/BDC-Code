...
idda1c_coefs = lapply(mods, tidy, simplify = F)
...
# IDDa1c
expressions_df_idda1c = expressions_df_idda1c %>% select(IDDA1C_binned:donor, all_of(idda1c_new_genes))
...
# formula
idda1c_coefs_lasso = colnames(transformed_analysis_idda1c %>% select(-donor)) 
testfrom = as.formula(v2frm(idda1c_coefs_lasso, y = 1L))

testfit = glmmLasso(fix = testfrom,
                    rnd = list(donor = ~1),
                    data = na.omit(transformed_analysis_idda1c),
                    lambda = 83,final.re = T,
...
### set seed
set.seed(1017)
N<-dim(transformed_analysis_idda1c)[1]
ind<-sample(N,N)
...
PQL<-glmmPQL(IDDA1C_binned~1,random = ~1|donor,family="gaussian",data=transformed_analysis_idda1c)
