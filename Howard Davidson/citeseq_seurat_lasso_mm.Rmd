...
expressions_df_idda1c = expressions_df_idda1c %>% dplyr::select(IDDA1C_binned,idda1c_bl,donor,visit_binned, all_of(idda1c_new_genes))
expressions_df_idda1c = expressions_df_idda1c %>% filter(!is.na(donor))
...
recipe_idda1c = recipe(expressions_df_idda1c) %>% update_role(everything()) %>% update_role(IDDA1C_binned, new_role = "outcome")
recipe_idda1c = recipe_idda1c %>%
  step_zv(all_predictors(),-c(visit_binned, donor)) %>%
  step_nzv(all_predictors(),-c( visit_binned, donor)) %>%
  step_YeoJohnson(all_predictors(),-c(visit_binned, donor)) %>%
  step_normalize(all_predictors(),-c( visit_binned, donor)) %>%
  prep()
transformed_analysis_idda1c <- bake(recipe_idda1c,expressions_df_idda1c)
...
full_rank_cols = colnames(as.data.frame(fullRank(expressions_df_idda1c2%>%dplyr::select(SSR2_c_0:MIF_c_12))))
expressions_df_idda1c2 = expressions_df_idda1c2 %>% select(IDDA1C_binned:donor, full_rank_cols)
idda1c_coefs_lasso2 = colnames(expressions_df_idda1c2 %>% dplyr::select(-donor)) 
testform2 = as.formula(v2frm(idda1c_coefs_lasso2, y = 1L))
# SWITCHING BETWEEN USING testform and expressions and testform2 and expressions2, and testform and expressions_naomit

### set seed
set.seed(1017)
N<-dim(expressions_df_idda1c2)[1]
ind<-sample(N,N)
...
# first fit good starting model
 ...
 # ## first fit good starting model
 PQL<-glmmPQL(IDDA1C_binned~1,random = ~1|donor,family="gaussian",data=expressions_df_idda1c2)
 ...
 glm1 <- try(glmmLasso(fix = testform2,
                       rnd = list(donor=~1),
                       data = expressions_df_idda1c2, lambda=lambda[j],switch.NR=FALSE,final.re=TRUE,
 ...
 glm1_final <- glmmLasso(fix = testform2,
                         rnd = list(donor=~1),
                         data = expressions_df_idda1c2, lambda=lambda[opt],switch.NR=FALSE,final.re=TRUE)
...
# # Using CV to determine the optimal tuning parameter lambda
# ## set number of folds
# kk<-5
# nk <- floor(N/kk)
# 
...
#     }
# 
# idda1c.train<-expressions_df_idda1c2[-indi,]
# idda1c.test<-expressions_df_idda1c2[indi,]
# 
# glm2 <- try(glmmLasso(fix = testform2,
#                       rnd = list(donor=~1),
#                       data = idda1c.train, lambda=lambda[j],switch.NR=FALSE,final.re=TRUE,
#         control=list(start=Delta.start,q_start=Q.start))
#         ,silent=TRUE)
# 
...
# 
# 
# glm2_final <- glmmLasso(fix = testform2,
#                         rnd = list(donor=~1),
#                         data = expressions_df_idda1c2, lambda=lambda[opt2],switch.NR=FALSE,
#                         control=list(start=Delta.start,q_start=Q.start),
#                         final.re=TRUE)
# 
# 
...
# USING THE EXPRESSIONS DATA
expressions_df_idda1c_base = expressions_df_idda1c2 %>% select(IDDA1C_binned:donor) %>% na.omit()
idda1c_coefs_lasso_base = colnames(expressions_df_idda1c_base %>% dplyr::select(-donor)) 
testform_base = as.formula(v2frm(idda1c_coefs_lasso_base, y = 1L))

### set seed
set.seed(1017)
N<-dim(expressions_df_idda1c_base)[1]
ind<-sample(N,N)
lambda <- seq(500,0,by=-2)
BIC_vec<-rep(Inf,length(lambda))
AIC_vec<-rep(Inf, length(lambda))
# ## first fit good starting model
PQL<-glmmPQL(IDDA1C_binned~1,random = ~1|donor,family="gaussian",data=expressions_df_idda1c_base)
Delta.start<-c(as.numeric(PQL$coef$fixed),rep(0,6),as.numeric(t(PQL$coef$random$donor)))
Q.start<-as.numeric(VarCorr(PQL)[1,1])
...
glm1 <- try(glmmLasso(fix = testform_base,
                      rnd = list(donor=~1),
                      data = expressions_df_idda1c_base, lambda=lambda[j],switch.NR=FALSE,final.re=TRUE,
        control=list(start=Delta.start,q_start=Q.start))
        ,silent=TRUE)
...
glm1_final <- glmmLasso(fix = testform_base,
                        rnd = list(donor=~1),
                        data = expressions_df_idda1c_base, lambda=lambda[opt],switch.NR=FALSE,final.re=TRUE)
