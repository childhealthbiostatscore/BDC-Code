...
idda1c_coefs_lasso = colnames(expressions_df_idda1c %>% dplyr::select(-donor)) 
...
# SWITCHING BETWEEN USING testform and expressions and testform2 and expressions2
...
glm1 <- try(glmmLasso(fix = testform,
                      rnd = list(donor=~1),
                      data = expressions_df_idda1c2, lambda=lambda[j],switch.NR=FALSE,final.re=FALSE,
...
glm1_final <- glmmLasso(fix = testform,
                        rnd = list(donor=~1),
                        data = expressions_df_idda1c2, lambda=lambda[opt],switch.NR=FALSE,final.re=FALSE)
