...
# wide to long
data_long <- reshape(data = data, direction = "long", varying = colnames(data[,1:13264]), v.names = "length")
...
data_long$sex <- ifelse(str_detect(data_long$id, "F") == TRUE, "F", "M")
data_long$tx <- ifelse(str_detect(data_long$id, "S"), "Sham", 
                       ifelse(str_detect(data_long$id, "4"), "PABw4", "PABw7"))
data_long$log_length <- log(data_long$length)
```

```{r data,echo=FALSE}
emm_options(pbkrtest.limit = 217060)
lme_model_int <- lmer(log_length ~ tx * sex + (1 | id), data = data_long)
#mod_means_int <- emmeans(lme_model_int,c("tx","sex"), lmerTest.limit = 300000)
#mod_pairs <-  pairs(mod_means_int,adjust="tukey", simple="each")

lme_model_noint <- lmer(log_length ~ tx + (1 | id), data = data_long)
