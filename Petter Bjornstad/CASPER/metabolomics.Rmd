...
# Petter's data
# Check data quality - all good!
#gs = goodSamplesGenes(X)
# Sample tree
sampleT = hclust(dist(X), method = "average")
plot(sampleT, main = "Sample clustering")
# Convert traits to a color representation: white means low, red means high, grey means missing entry
traits = as.data.frame(lapply(df[,2:13],as.numeric))
traitnames = names(traits)
rownames(traits) = df$ID
traits = as.matrix(traits)
traitColors = numbers2colors(traits, signed = FALSE);
# Plot the sample dendrogram and the colors underneath.
plotDendroAndColors(sampleT, traitColors,
                    groupLabels = traitnames,
                    main = "Sample dendrogram and trait heatmap")
```

RH-10 appears to be an outlier based on this dendrogram. Should we exclude that sample?
...
Choose the lowest power for which the scale-free topology fit index curve flattens out upon reaching a high value. 5 seems appropriate here. 

```{r echo=TRUE,cache=TRUE}
# Make simple network
net = blockwiseModules(X, power = 5,saveTOMs = TRUE,
                       saveTOMFileBase = "casperTOM",
                       maxBlockSize = ncol(X))
...
There are a lot of options in the function above. Is it okay to use the defaults for this analysis? Do we want to pick a p value ratio threshold for assigning metabolites to specific modules (default is 0.000001)?
...
MEs = orderMEs(moduleEigengenes(X[,1:5000],
                              labels2colors(net$colors))$eigengenes)
moduleTraitCor = cor(MEs, traits, use = "pairwise.complete.obs")
moduleTraitPvalue = corPvalueStudent(moduleTraitCor, nrow(X))
# Heatmap
labeledHeatmap(Matrix = moduleTraitCor,xLabels = traitnames,
               yLabels = names(MEs),
               ySymbols = names(MEs),
