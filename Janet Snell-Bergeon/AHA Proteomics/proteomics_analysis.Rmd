...
anova_results <- lapply(proteins, function(x){
  form <- as.formula(paste0(x,"~factor(abundance$GROUP)"))
  mod <- aov(form,abundance)
...
anova_results$p.adj <- p.adjust(anova_results$p.value,"fdr")
