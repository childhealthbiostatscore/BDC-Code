...
clinical <- read.csv("./Data_Cleaned/clinical_info.csv")
samples <- read.csv("./Data_Cleaned/sample_list.csv")
# Match sample number to subject ID
clinical$SampleID <- samples$Injection[match(clinical$StudyID,samples$SampleID)]
# Select columns, filter out missing
clinical <- clinical %>% 
  dplyr::select(SampleID,gender,age,sex,race,CACGROUP) %>% 
  filter(!is.na(SampleID)) %>% arrange(SampleID)
```
...
colnames(X) <- cols
# Try PCA 
trans.pca <- pca(X,center = F,scale = F,ncomp=10)
plot(trans.pca)
# Sample plot
plotIndiv(trans.pca,comp=c(1,2),group = clinical$CACGROUP,legend = T,
