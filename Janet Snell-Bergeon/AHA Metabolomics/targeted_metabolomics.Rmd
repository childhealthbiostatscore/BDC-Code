...
for (var in metabs) {
  ...
  cat("## ", var, "\n") 
  cat('\n')
  a <- lm(as.formula(paste0(var, "~ age")), data = targeted)
  ares <- as.data.frame(summary(a)$coefficients)
  print(kable(ares,caption = paste0("Association between age and ",var)))
  b <- multinom(as.formula(paste0("factor(GROUP) ~ age +",var)), 
                data = targeted,trace = F)
  coeffs <- as.data.frame(summary(b)$coefficients)[[var]]
  ses <- as.data.frame(summary(b)$standard.errors)[[var]]
  z <- coeffs/ses
  ps <- (1 - pnorm(abs(z), 0, 1))*2
  ...
  print(kable(bres,
        caption = paste0("Association between CAC group and age, adjusted for ",var)))
}
