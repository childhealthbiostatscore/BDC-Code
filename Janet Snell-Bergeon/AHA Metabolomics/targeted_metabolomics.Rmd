...
```{r echo=FALSE}
# Import pilot data
pilot <- read.csv("./Data_Cleaned/pilot_matched.csv")
...
targeted$CACp <- ifelse(targeted$c3 - targeted$c1 >= 2.5,1,0)
```
...
logits <- lapply(colnames(targeted)[which(colnames(targeted) == "Betaine"):which(colnames(targeted)=="linoleic.acid")], 
                 function(var){
                   #print(var)
                   formula <- as.formula(paste0("CACp ~ ",var,"+ strata(MATCHID)"))
                   mod <- clogit(formula, data = targeted, method = "exact") 
...
metab_logits <- metab_logits %>% filter(metabolite != "(Intercept)") %>% 
  select(metabolite,coef,`se(coef)`,`Pr(>|z|)`)
# Rename metabolites
