...
df <- raw_data %>% 
  ...
  mutate(feature_index = n(), # number the features and get m/z (per Qibin's email about MetaboAnalyst)
         `quant m/z` = Neutral.Mass..Da.+1.0078) %>% 
  select(feature_index,compounds,`quant m/z`,HMDB.No.)
# Sample information
...
sample_info["sampleID",] <- 1:ncol(samples)
