...
# Remove QC
X <- X %>% select(GlobalSampleID,age:Batch,everything()) %>% 
...
logged <- X
logged <- lapply(logged[,8:ncol(logged)], log)
# skim(logged)
```
...
# Full model results
fullModelResults <- mxtrmod(ynames=Y,
                            mxtrModel=fullModel,data=X)
# Reduced model results
reducedModelResults<-mxtrmod(ynames=Y,
