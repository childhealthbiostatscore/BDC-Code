...
plot(pca)
# Sample plot
plotIndiv(pca,group = df$GROUP,legend = T,ind.names = F, style='3d',
          title = "PCA comp 1-2 by CAC Group",pch = "sphere")
```
...
# Plot
plotIndiv(plsda,style='3d',pch = "sphere",
...
averaged <- raw_metab %>% 
  ...
  summarise_all(mean)
# Numeric matrix
X <- averaged %>% select(IsoMS_results_QC01_190618171010_converted.csv:
                         IsoMS_results_S227_Batch07_1400_RA8_converted.csv) %>%
  as.data.frame()
rownames(X) <- averaged$compound
X <- t(X)
X <- as.matrix(X)
class(X) <- "numeric"
# Try PCA 
