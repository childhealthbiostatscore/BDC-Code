...
df <- df %>% select(GlobalSampleID,gender,age,sex,race,CACp,GROUP,everything())
# As numeric
metabolites <- setdiff(names(df),c("GlobalSampleID","gender","age","sex","race","CACp","GROUP"))
...
#           title = "PCA comp 1-2 by CAC Group",pch = "sphere",style = "3d")
```
...
```{r anova qc,echo=FALSE}
anova_results <- lapply(metabolites, function(x){
  form <- as.formula(paste0(x,"~factor(GROUP)"))
  mod <- aov(form,df)
...
```{r anova qc adjusted,echo=FALSE}
...
anova_results <- lapply(metabolites, function(x){
  form <- as.formula(paste0(x,"~factor(GROUP)"))
  mod <- aov(form,df[which(df$GROUP != "QC"),])
  results <- broom::tidy(mod)
  results[1,1] <- x
  as.data.frame(results[1,])
}) 
