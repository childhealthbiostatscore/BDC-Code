...
Sex is significantly associated with PC1 and age is significantly associated with PC2 (regardless of whether or not demographic variables are included in the PCA).

```{r echo=FALSE}
# PCA
temp = data.matrix(X)
scaled = scale(X[,8:ncol(X)])
pca.res = pca(scaled,scale = T,center = T) 
loadings = as.data.frame(pca.res$loadings)
# Print top 20 "most important" variables based on loadings
#rownames(loadings)[order(abs(loadings$X.PC1))[1:20]]
# PC association with demographic variables
mod = lm(pca.res$x[,1] ~ X$age + X$sex + X$race + X$GROUP)
kable(summary(mod)$coefficients,caption = "PC1 ~ age + sex + race + group") # PC1 associated with sex
mod = lm(pca.res$x[,2] ~ X$age + X$sex + X$race + X$GROUP)
kable(summary(mod)$coefficients,caption = "PC2 ~ age + sex + race + group") # PC2 associated with age
# Clustered heatmap
# Correlation matrix
cormat = round(cor(scaled,use = "pairwise.complete.obs"),3)
cols.cor = cor(scaled,use = "pairwise.complete.obs")
rows.cor = cor(t(scaled),use = "pairwise.complete.obs")
pheatmap(cormat,show_rownames = F,show_colnames = F,cluster_cols = F,cluster_rows = F)
```