...
# As numeric
metabolites <- setdiff(names(df),c("GlobalSampleID","gender","age","sex","race","CACp","GROUP","sex_and_group"))
...
```{r}
X <- df %>% select(-GlobalSampleID,-gender,-age,-sex,-race,-CACp,-GROUP,-sex_and_group)
X <- as.matrix(X)
class(X) <- "numeric"
# Try PCA 
...
```{r sex and group plsda}
X <- df %>% filter(GROUP != "QC") %>%
  select(-GlobalSampleID,-gender,-age,-sex,-race,-CACp,-GROUP,-sex_and_group)
X <- as.matrix(X)
# Outcome
Y <- factor(df$gender[df$GROUP != "QC"])
# PLSDA
plsda <- plsda(X, Y, ncomp = 10,scale = T)
# Evaluate
perf.plsda <- perf(plsda, validation = "loo", folds = 5, 
...
plot(perf.plsda, sd = TRUE, legend.position = "horizontal")
# Plots
plotIndiv(plsda,group = df$gender[df$GROUP != "QC"], ind.names = FALSE,pch = 20,
