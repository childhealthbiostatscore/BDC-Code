...
library(gt)
knitr::opts_knit$set(
    root.dir = "/Users/timvigers/Library/CloudStorage/OneDrive-TheUniversityofColoradoDenver/Vigers/BDC/Janet Snell-Bergeon/MERIT"
)
```
...
tir_plot <- function(df = cgm, range = c(70, 180), by = "menstrual_phase") {
    p <- df %>%
        ...
        summarise(
            sensorglucose = mean(
                sensorglucose >= range[1] & sensorglucose <= range[2],
                na.rm = T
            ) *
                100,
            .groups = "drop"
...
invisible(lapply(unique(cgm$participant_id), function(id) {
    ...
    df %>%
        ...
        summarise(
            ...
            `TIR (70-180)` = scales::percent(
                mean(sensorglucose >= 70 & sensorglucose <= 180, na.rm = T),
                accuracy = 0.01
            ),
            `TBR (< 70)` = scales::percent(
                mean(sensorglucose < 70, na.rm = T),
                accuracy = 0.01
            ),
            `TBR (< 54)` = scales::percent(
                mean(sensorglucose < 54, na.rm = T),
                accuracy = 0.01
            ),
            `TAR (> 180)` = scales::percent(
                mean(sensorglucose > 180, na.rm = T),
                accuracy = 0.01
            ),
            `TAR (> 200)` = scales::percent(
                mean(sensorglucose > 200, na.rm = T),
                accuracy = 0.01
            ),
