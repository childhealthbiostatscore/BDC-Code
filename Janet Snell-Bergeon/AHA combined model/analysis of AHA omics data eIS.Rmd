...
df <- df %>% mutate(ins_tertile=ntile(df$eISV1,3))
df <- df %>% filter(!ins_tertile==2)
```
...
ins_contrast <- ifelse(df$ins_tertile==3,1,0)
ins_contrast <- cbind(rep(1,nrow(df)),ins_contrast)
...
saveWorkbook(wb,"~/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/AHA collaborative grant/Reports/AHA_omics_ttests_baseline_eIS.xlsx",overwrite = TRUE)
