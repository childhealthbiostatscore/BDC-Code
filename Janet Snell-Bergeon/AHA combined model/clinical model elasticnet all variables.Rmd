...
clin_mod = coxph(formula = f_death,data = df,id = StudyID,model = T)
kable(tidy(clin_mod),digits = 3)
```
...
```{r}
lpnew <- predict(clin_mod)
Surv.rsp <- Surv(time = df$PersonYrsDeath, event = df$Deceased)
times <- c(5,10)
AUC_hc <- AUC.hc(Surv.rsp,Surv.rsp, lpnew, times)

survivalROC(Stime = df$PersonYrsDeath,status = df$Deceased,predict.time = c(5,10))

x = data.matrix(clin_mod$model[,2])
y = clin_mod$model$`Surv(time = df$PersonYrsDeath, event = df$Deceased)`[,1]
d = clin_mod$model$`Surv(time = df$PersonYrsDeath, event = df$Deceased)`[,2]

td = tdROC(X = x,Y = y,delta = d,tau = 10)
```
