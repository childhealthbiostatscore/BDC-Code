...
```{r}
vis_miss(df[,c("CACprogV3",cac_prog_select[[1]])])
na_pattern = md.pattern(df[,c("CACprogV3",cac_prog_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("CACprogV3",cac_prog_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("CACprogV3",cac_prog_select[[1]],limited_omics)],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("cac_change_per_yr",cac_change_select[[1]])])
na_pattern = md.pattern(df[,c("cac_change_per_yr",cac_change_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("cac_change_per_yr",cac_change_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("cac_change_per_yr",cac_change_select[[1]],limited_omics)],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsDeath","Deceased",death_select[[1]])])
na_pattern = md.pattern(df[,c("PersonYrsDeath","Deceased",death_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsDeath","Deceased",death_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("PersonYrsDeath","Deceased",death_select[[1]],limited_omics)],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsCAD","CAD",cad_select[[1]])])
na_pattern = md.pattern(df[,c("PersonYrsCAD","CAD",cad_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsCAD","CAD",cad_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("PersonYrsCAD","CAD",cad_select[[1]],limited_omics)],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsHardCAD","HardCAD",hard_cad_select[[1]])])
na_pattern = md.pattern(df[,c("PersonYrsHardCAD","HardCAD",hard_cad_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsHardCAD","HardCAD",hard_cad_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("PersonYrsHardCAD","HardCAD",hard_cad_select[[1]],limited_omics)],
                        plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsCVD","CVD",cvd_select[[1]])])
na_pattern = md.pattern(df[,c("PersonYrsCVD","CVD",cvd_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsCVD","CVD",cvd_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("PersonYrsCVD","CVD",cvd_select[[1]],limited_omics)],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsHardCVD","HardCVD",hard_cvd_select[[1]])])
na_pattern = md.pattern(df[,c("PersonYrsHardCVD","HardCVD",hard_cvd_select[[1]])],plot = F)
kable(na_pattern)
```
...
```{r}
vis_miss(df[,c("PersonYrsHardCVD","HardCVD",hard_cvd_select[[1]],limited_omics)])
na_pattern = md.pattern(df[,c("PersonYrsHardCVD","HardCVD",hard_cvd_select[[1]],limited_omics)],
                        plot = F)
kable(na_pattern)
```
...
fit = with(imp,{
  environment(f_cac_prog) = environment()
  glm(formula = update(f_cac_prog,paste0("~ . +",paste0(limited_omics,collapse = "+"))),
               family = "binomial")
})
kable(summary(pool(fit)),digits = 3)
```
...
fit = with(imp,{
  environment(f_cac_change) = environment()
  lm(formula = update(f_cac_change,paste0("~ . +",paste0(limited_omics,collapse = "+"))))
})
kable(summary(pool(fit)),digits = 3)
```
...
fit = with(imp,{
  environment(f_death) = environment()
  coxph(formula = update(f_death,paste0("~ . +",paste0(limited_omics,collapse = "+"))),id = StudyID)
})
kable(summary(pool(fit)),digits = 3)
```
...
fit = with(imp,{
  environment(f_cad) = environment()
  coxph(formula = update(f_cad,paste0("~ . +",paste0(limited_omics,collapse = "+"))),id = StudyID)
})
kable(summary(pool(fit)),digits = 3)
```
...
fit = with(imp,{
  environment(f_hard_cad) = environment()
  coxph(formula = update(f_hard_cad,paste0("~ . +",paste0(limited_omics,collapse = "+"))),id = StudyID)
})
kable(summary(pool(fit)),digits = 3)
```
...
fit = with(imp,{
  environment(f_cvd) = environment()
  coxph(formula = update(f_cvd,paste0("~ . +",paste0(limited_omics,collapse = "+"))),id = StudyID)
})
kable(summary(pool(fit)),digits = 3)
```
...
fit = with(imp,{
  environment(f_hard_cvd) = environment()
  coxph(formula = update(f_hard_cvd,paste0("~ . +",paste0(limited_omics,collapse = "+"))),id = StudyID)
})
kable(summary(pool(fit)),digits = 3)
```
