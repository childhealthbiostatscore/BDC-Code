...
# variable for lean vs. obese
df$obese <- ifelse(df$bmiV1>=30,1,0)
```

```{r moderated ttests,echo=FALSE, message=FALSE, results="hide"}
# CAC progression
#bmi_contrast <- ifelse(df$CACp=="Yes",1,0)
#bmi_contrast <- cbind(rep(1,nrow(df)),bmi_contrast)

# insulin tertiles
# code the contrast so that high insulin sensitivity=1 and low=0
bmi_contrast <- ifelse(df$obese==1,1,0)
bmi_contrast <- cbind(rep(1,nrow(df)),bmi_contrast)

# untargeted metabolites
ymat <- df[,names(df) %in% untargeted_metabs]
ymat <- t(ymat)
fit <- lmFit(ymat,bmi_contrast)
fit <- eBayes(fit)
...
resT_untargeted_metabs<-mt.maxT(ymat,bmi_contrast[,2])
resP_untargeted_metabs<-mt.minP(ymat,bmi_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
#resT_targeted_metabs<-mt.maxT(ymat,bmi_contrast[,2])
#resP_targeted_metabs<-mt.minP(ymat,bmi_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
resT_global_proteins<-mt.maxT(ymat,bmi_contrast[,2])
resP_global_proteins<-mt.minP(ymat,bmi_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
resT_glycated_proteins<-mt.maxT(ymat,bmi_contrast[,2])
resP_glycated_proteins<-mt.minP(ymat,bmi_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
resT_lipids<-mt.maxT(ymat,bmi_contrast[,2])
resP_lipids<-mt.minP(ymat,bmi_contrast[,2])
ymat <- NULL
fit <- NULL
...
saveWorkbook(wb,"~/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/AHA collaborative grant/AHA_omics_ttests_baseline_bmi.xlsx",overwrite = TRUE)
