...
# ElasticNet on limited clinical variables only
...
f_cac_prog_any = as.formula(paste0("cac_prog_last_vis~",paste0(cac_prog_any_select,
                                                               collapse = "+")))
mod = glm(formula = f_cac_prog_any,data = df,family = "binomial")
kable(tidy(mod),digits = 3)
```
...
```{r}
mod = glm(formula = update(f_cac_prog_any,paste0("~ . +",paste0(limited_omics,collapse = "+"))),
          data = df,family = "binomial")
kable(tidy(mod),digits = 3)
```
