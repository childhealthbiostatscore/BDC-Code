...
if(Sys.info()["sysname"] == "Windows"){
  home_dir = "B:/Projects/Janet Snell-Bergeon/AHA collaborative grant"
} else if (Sys.info()["sysname"] == "Linux"){
  home_dir = "~/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/AHA collaborative grant"
...
source("/home/laura/Documents/Github/Helper-Functions/glinternet.R")
...
1) Tests of proportional hazards and diagnostic plots look better but not perfect
2) Included duration^2 in models but when refitting the models selected by LASSO, no parameter estimates provided for the squared terms.  For models without interactions, I forced the quadratic polynomial term (linear and squared) into the model using (poly,2).  But for models with interaction, what should we do for interactions with the squared term?
...
test$sex_0m1f <- as.factor(test$sex_0m1f)
test$CACprogV3_num <- as.factor(test$CACprogV3_num)
form_test <- easy_glinternet(df=test,outcome = "CACprogV3_num",id_vars = "StudyID",fam ="binomial",cv_type = "loo",cores = 24)
```
...
test$smknum <- as.factor(test$smknum)
test$sex_0m1f <- as.factor(test$sex_0m1f)
form_test <- easy_glinternet(df=test,outcome = "cac_change_per_yr",id_vars = "StudyID",fam="gaussian",cv_type = "loo",cores = 24)
```
