...
snp_data <- df[,c(snps,"StudyID")]

plsda_data <- df[,c("StudyID","gly_P02671.7","gly_P02671.5","gly_P02671.4","gly_P02671.3","AAA01201000179.0807263.4","gly_P02679",
...
limma_data <- df[,c("StudyID","AAA01201000179.0807263.4","gly_P02671.3","gly_P02671.7")]

omics_data <- merge(snp_data,plsda_data,by="StudyID",all.x = T,all.y = T)

id_check <- as.data.frame(snp_data[,c("StudyID")])
id_check$in_snp <- 1
colnames(id_check) <- c("StudyID","in_snp")
plsda_id <- as.data.frame(plsda_data[,c("StudyID")])
plsda_id$in_plsda <- 1
colnames(plsda_id) <- c("StudyID","in_plsda")
id_check <- merge(id_check,plsda_id,by="StudyID",all.x = T,all.y = T)
limma_id <- as.data.frame(limma_data[,c("StudyID")])
limma_id$in_limma <- 1
colnames(limma_id) <- c("StudyID","in_limma")
id_check <- merge(id_check,limma_id,by="StudyID",all.x = T, all.y = T)
```
