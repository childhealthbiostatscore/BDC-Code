...
load(file="/Volumes/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/AHA collaborative grant/aha_master_data.Rdata")
...
# create new var for renal complications
# AER>20 or AC>30 if AER is missing
df$renal_comp <- ifelse(!is.na(df$aerV1) & df$aerV1>20,1,
                        ifelse(!is.na(df$aerV1) & df$aerV1<=20,0,
                               ifelse(is.na(df$aerV1) & (!is.na(df$acV1) & df$acV1>30),1,
                                      ifelse(is.na(df$aerV1) & (!is.na(df$acV1) & df$acV1<=30),0,NA))))

# delete records with missing renal information
df <- df[!is.na(df$renal_comp),]
...
renal_contrast <- ifelse(df$renal_comp==1,1,0)
renal_contrast <- cbind(rep(1,nrow(df)),renal_contrast)

# untargeted metabolites
ymat <- df[,names(df) %in% untargeted_metabs]
ymat <- t(ymat)
fit <- lmFit(ymat,renal_contrast)
fit <- eBayes(fit)
...
resT_untargeted_metabs<-mt.maxT(ymat,renal_contrast[,2])
resP_untargeted_metabs<-mt.minP(ymat,renal_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
#resT_targeted_metabs<-mt.maxT(ymat,renal_contrast[,2])
#resP_targeted_metabs<-mt.minP(ymat,renal_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
resT_global_proteins<-mt.maxT(ymat,renal_contrast[,2])
resP_global_proteins<-mt.minP(ymat,renal_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
resT_glycated_proteins<-mt.maxT(ymat,renal_contrast[,2])
resP_glycated_proteins<-mt.minP(ymat,renal_contrast[,2])
ymat <- NULL
fit <- NULL
...
# trying permutation test based adjustment
resT_lipids<-mt.maxT(ymat,renal_contrast[,2])
resP_lipids<-mt.minP(ymat,renal_contrast[,2])
ymat <- NULL
fit <- NULL
...
saveWorkbook(wb,"~/UCD/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/AHA collaborative grant/Reports/AHA_omics_ttests_baseline_renal.xlsx",overwrite = TRUE)
