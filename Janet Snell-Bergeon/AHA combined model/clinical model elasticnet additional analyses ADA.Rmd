...
sample_info <- sample_info[,c("StudyID","hba1c","glu","egdr","eis","eis_nf","eis_exA")]
sample_info$StudyID <- as.factor(as.character(sample_info$StudyID))
sample_info_var <- sample_info %>% group_by(StudyID) %>% summarise(across(.cols = where(is.numeric),.fns = list(SD=sd), na.rm=T, .names="{col}_{fn}"))
# merge in omics data
