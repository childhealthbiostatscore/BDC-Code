...
selected = unique(c(cac_prog_select[[1]],cac_change_select[[1]],death_select[[1]],
                  cad_select[[1]],hard_cad_select[[1]],
                  cvd_select[[1]],hard_cvd_select[[1]]))
imp = mice(df[,c(selected,omics)], seed = 1017, m =3,
           print = FALSE,method = "rf")
```

Five imputed datasets were created using random forest imputation based on all selected cinical variables and omics variables selected by moderated t test and PLS-DA. Two models were fit to the imputed data: one with only the selected clinical variables, and the other with selected clinical and omics variables. Models were compared using the multivariate Wald test (see [this page](https://stefvanbuuren.name/fimd/sec-multiparameter.html#sec:likelihoodratio) for additional details) and pooled results of the omics models are reported below.
...
```{r eval=FALSE}
clinical_fit = imp %>% complete("all") %>%
  lapply(glm, formula = f_cac_prog,family = "binomial")
fit = imp %>% complete("all") %>%
  lapply(glm,formula = update(f_cac_prog,paste0("~ . +",paste0(omics,collapse = "+"))),
               family = "binomial")
...
```{r eval=FALSE}
clinical_fit = imp %>% complete("all") %>%
  lapply(lm, formula = f_cac_change)
fit = imp %>% complete("all") %>%
  lapply(lm,formula = update(f_cac_change,paste0("~ . +",paste0(omics,collapse = "+"))))
kable(summary(pool(fit)),digits = 3)
```
...
c = with(data = imp, expr = coxph(formula = as.formula(f_death),id = StudyID))

clinical_fit = imp %>% complete("all") %>%
  lapply(coxph, formula = formula(f_death),id = StudyID)
fit = imp %>% complete("all") %>%
  lapply(coxph,formula = update(f_death,paste0("~ . +",paste0(omics,collapse = "+"))),
         id = StudyID)


clinical_fit = with(imp,coxph(formula = f_death,id = StudyID))
fit = glm.mids(formula = update(f_cac_change,paste0("~ . +",paste0(omics,collapse = "+"))),
               data = imp)
