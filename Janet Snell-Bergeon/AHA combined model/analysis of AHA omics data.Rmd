...
plsrx <- as.matrix(df[,names(df) %in% untargeted_metabs])
plsr_utm <- plsr(df$delta_cac_v4 ~ plsrx, ncomp=20, validation="LOO")
ncomp_utm <- selectNcomp(plsr_utm, method="randomization", plot=TRUE)
# selects 0 components (error keeps increasing!)
plsr_utm <- plsr(df$delta_cac_v4 ~ plsrx, ncomp=2, validation="LOO")
```
...
## PLS regression of targeted metabolites
plsrx <- as.matrix(df[,colnames(df) %in% targeted_metabs])
plsr_tm <- plsr(df$delta_cac_v4 ~ plsrx, ncomp=20, validation="LOO")
ncomp_tm <- selectNcomp(plsr_tm, method="randomization", plot=TRUE)
...
#plsrx <- df[,names(df) %in% c("delta_cac_v4",global_proteins)]
#plsrx <- plsrx[!is.na(plsrx$delta_cac_v4),]
#plsry <- plsrx$delta_cac_v4
#plsrx <- as.matrix(select(plsrx,-delta_cac_v4))

plsrx <- as.matrix(df[,colnames(df) %in% global_proteins])
plsr_globalp <- plsr(df$delta_cac_v4 ~ plsrx, validation="LOO", na.rm=TRUE)
ncomp_globalp <- selectNcomp(plsr_globalp, method="randomization", plot=TRUE)
# why are there variables with every value missing? e.g. P06753-2
```
