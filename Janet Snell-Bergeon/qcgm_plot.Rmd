...
# Read in data
filename <- paste0(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/670g CGM/QCGM Plot/Data_Raw/QCGM_All.txt")
...
baseplot <- ggplot(plot_data, aes(x = Time,y = Glucose.Value)) +
  ylab("Glucose Value")
plot <- baseplot + 
  geom_smooth(aes(group = randomization_group, color = randomization_group),se = F)
plot2 <- baseplot + 
  geom_smooth(aes(group = randomization_group, color = randomization_group),se = F) +
  ylim(0,401)
plot3 <- baseplot +
