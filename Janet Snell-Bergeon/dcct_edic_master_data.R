library(tidyverse)
library(readxl)
setwd("/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Janet Snell-Bergeon/CACTI DCCT EDIC omics R01")
# Sample info (cleaned for metabolomics)
clinical = read_excel("./Data_Raw/EDIC_WP_DDA_60SPD_StudyInformation.xlsx")
# To numeric
...
sample_info$cac_prog_last_vis = apply(sample_info,1,function(r){
  ...
  d = cs[last] - cs[1]
  if(d >= 2.5){return("Progression")} else {return("No Progression")}
})
...
aha_outcomes = c('CACprogV3','cac_prog_last_vis','cac_change_per_yr','Deceased',
...
save(untargeted_metabs,targeted_metabs,global_proteins,glycated_proteins,lipids,
     snps,df,clinical_predictors,aha_outcomes,file = "./aha_master_data.Rdata")
