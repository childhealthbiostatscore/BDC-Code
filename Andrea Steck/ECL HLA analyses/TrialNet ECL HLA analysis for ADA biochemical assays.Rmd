...
f7 <- ggplot(dr3, aes(x=ab_cat,y=perc)) + geom_bar(stat="identity",position = position_dodge(width=2), width=0.5) + labs(x="",y="%") + annotate("text", x=1, y=45, label="p=0.002")

f8_tbl <- table(keep$DR4.Present,keep$ab_cat)
f8_p <- round(chisq.test(f8_tbl)$p.value,3)
f8 <- ggplot(dr4, aes(x=ab_cat,y=perc)) + geom_bar(stat="identity",position = position_dodge(width=2), width=0.5) + labs(x="",y="%") + annotate("text", x=1, y=65, label="p=0.094")
```
