...
cgm_lmm$pred = predict(lme_fit_mean)
ggplot(cgm_lmm, aes(x = Age, y = pred, color = ID)) +
  geom_line() +
  theme(legend.position = "none")
cox_fit <- coxph(
  Surv(Ages, event) ~ sex +
...
ggtraceplot(joint_fit_mean, "alphas")
...
ggtraceplot(joint_fit_sd, "alphas")
...
```{r}
lme_fit_mean <- lme(
  mean_glucose ~ TimeFromFirstCGM + Age,
