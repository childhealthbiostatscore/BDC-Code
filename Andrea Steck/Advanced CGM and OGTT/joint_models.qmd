...
ggplot(plot_df, aes(x = Age, color = ID)) +
  ...
  theme(legend.position = "none") +
  geom_line(aes(y = pred))
lme_fit_mean_rs_age2 <- lme(
  mean_glucose ~ poly(Age, 2),
  random = ~ poly(Age, 2) | ID,
  data = cgm_lmm,
  control = lmeControl(opt = 'optim')
)
plot_df = lme_fit_mean_rs_age2$data
plot_df$pred = predict(lme_fit_mean_rs_age2)
ggplot(plot_df, aes(x = Age, color = ID)) +
  geom_point(aes(y = mean_glucose), alpha = 0.2) +
  theme(legend.position = "none") +
  geom_line(aes(y = pred))
...
```{r}
load("./Data_Clean/final_model.RData")
newdat = full_join(cgm_surv, cgm_lmm)
# Dynamic predictions
t0 <- 15
ND <- newdat[newdat$ID == "00783-0", ]
ND <- ND[ND$Age < t0, ]
ND$event <- 0
ND$AgeEndpoint <- t0
predLong1 <- predict(
