...
cgm$sensordisplaytime <- as.POSIXct(cgm$sensordisplaytime, origin = "1960-01-01")
cgm$sensordisplaytime <- round_date(cgm$sensordisplaytime, unit = "5 minutes")
cgm <- cgm %>% arrange(ID, DOVISIT, sensordisplaytime)
...
df <- df %>%
  ...
  filter(rowSums(!is.na(.)) > 0) %>% # Remove rows that are entirely missing
  distinct() %>%
  rename(`Date of Diagnosis` = mdy(Date.of.T1D.dx))
df$Group <- factor(!is.na(df$`Date of Diagnosis`),
  levels = c(F, T), labels = c("Non-Progressor", "Progressor")
)
```
