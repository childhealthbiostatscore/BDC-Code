...
cgm <- cgm %>% select(ID, Date, Time, SensorValue)
...
cgm$DaysFromVisit <- cut(cgm$Days,
...
```{r}
ggplot(cgm, aes(x = Time, y = SensorValue, colour = Group)) +
  geom_smooth() +
  facet_wrap(~DaysFromVisit)
```
...
```{r}
#| include: false
ps <- lapply(split.data.frame(cgm, cgm$DaysFromVisit), function(d) {
  # Time period
  tp <- as.character(d$DaysFromVisit[1])
