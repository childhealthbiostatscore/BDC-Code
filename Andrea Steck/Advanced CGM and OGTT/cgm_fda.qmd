...
home_dir <- switch(
  Sys.info()["sysname"],
  "Darwin" = "/Users/tim/Library/CloudStorage/OneDrive-UW/UWMDI/Andrea Steck/Advanced CGM and OGTT",
  "Windows" = "C:/Users/Tim/OneDrive-UW/UWMDI/Andrea Steck/Advanced CGM and OGTT",
  "Linux" = "/home/tim/OneDrive-UW/UWMDI/Andrea Steck/Advanced CGM and OGTT"
)
github_dir <- switch(
  Sys.info()["sysname"],
...
```{r}
# It takes forever to do this sparse FDA on this big a dataset, so this was done
# on Hyak using fpca_fit.R and fpca_fit.slurm. See those files for details.
# Just import the FPCA object here.
setwd(home_dir)
load(file = "./Data_Clean/face_fit_sparse_cgm.RData")
# Plot the raw data and mean function
m <- data.frame(cbind(
  argvals.new = face_fit$argvals.new,
  mu.new = face_fit$mu.new,
  sigma.new = sqrt(diag(face_fit$Chat.new))
))
m$ll <- m$mu.new + 1.96 * m$sigma.new
m$ul <- m$mu.new - 1.96 * m$sigma.new
ggplot(sparse_cgm, aes(x = argvals, y = y, group = subj)) +
  geom_line(alpha = 0.5) +
  geom_line(
    data = m,
    aes(x = argvals.new, y = mu.new),
    inherit.aes = F,
    color = "red",
    linewidth = 2
  ) +
  geom_ribbon(
    data = m,
    aes(x = argvals.new, y = mu.new, ymax = ul, ymin = ll),
    inherit.aes = F,
    alpha = 0.2
  )
# Plot the covariance estimator
ggplot(m, aes(x = argvals.new, y = sigma.new)) +
  geom_line()
# Heatmap of the smoothed estimator of the correlation function
plot_ly(x = m$argvals.new, y = m$argvals.new, z = ~ face_fit$Cor.new) %>%
  add_surface(
    contours = list(
      z = list(
        show = TRUE,
...
cgm_filtered <- cgm %>%
  filter(
    Group == "Non-Progressor" |
      (Group == "Progressor" &
        DaysFromEndpoint >= -270 &
        DaysFromEndpoint <= -180)
  )
cgm_filtered %>%
  group_by(ID) %>%
  mutate(
