...
- Differences in AUC were compared using the bootstrap with 5000 replicates.
...
cgm %>%
  ...
  ungroup() %>%
  select(
    nClinicVisits, nCGMValues, `age at clinic`, A1C, `FDR status`, HLAGRP, SEX, Race_Ethn2,
    Group
  ) %>%
  tbl_summary(Group) %>%
  add_overall() %>%
...
ps <-
  lapply(split.data.frame(cgm_filtered, cgm_filtered$TimePeriod), function(d) {
    ...
    # Toot it and boot it
    reps <- boot(data = d, statistic = auc_diff_func, R = 5000, strata = d$Group)
...
p3_69
p4_69
p_collected <- (p1_69 + p2_69) / (p3_69 + p4_69) +
...
p3_36
p4_36
p_collected <- (p1_36 + p2_36) / (p3_36 + p4_36) +
  plot_layout(guides = "collect", axis_titles = "collect") +
  plot_annotation(
    title = "Function on Scalar Regression of CGM Data",
    theme = theme(plot.title = element_text(hjust = 0.5))
...
p_collected <- (p1_30 + p2_30) / (p3_30 + p4_30) +
  plot_layout(guides = "collect", axis_titles = "collect") +
  plot_annotation(
    title = "Function on Scalar Regression of CGM Data",
    theme = theme(plot.title = element_text(hjust = 0.5))
...
p3_0
p4_0
p_collected <- (p1_0 + p2_0) / (p3_0 + p4_0) +
  plot_layout(guides = "collect", axis_titles = "collect") +
  plot_annotation(
    title = "Function on Scalar Regression of CGM Data",
    theme = theme(plot.title = element_text(hjust = 0.5))
...
p1 <- ggplot(p1_df, aes(
  x = Time, y = Glucose, group = interaction(Group, Period),
  colour = Group, linetype = Period
)) +
  geom_line() +
  theme_classic() +
  ylim(c(90, 190)) +
