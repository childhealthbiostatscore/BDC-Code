...
if (user == "laurapyle") {
  ...
  home_dir <- "/Users/lpyle/Library/CloudStorage/OneDrive-TheUniversityofColoradoDenver/Vigers/BDC/Andrea Steck/Advanced CGM and OGTT"
} else {
...
# spaghetti plot
p <- ggplot(mean_cgm, aes(x = numtime, y = Glucose, group = numID, color = Group)) +
  geom_line() +
  labs(title = "Spaghetti Plot Color-Coded by Group",
       x = "Time Point",
       y = "Measurement Value") +
  theme_minimal()
```
...
lmm_2group <- gridsearch(hlme(Glucose ~ numtime,
  random = ~numtime, ng = 2, mixture = ~numtime,
  subject = "numID", data = mean_cgm, maxiter = 500, verbose = TRUE
), rep = 50, maxiter = 500, minit = lmm)
summary(lmm_2group)

lmm_quad <- hlme(Glucose ~ numtime + I(numtime^2),
  random = ~numtime, ng = 1, 
  subject = "numID", data = mean_cgm, maxiter = 50, verbose = TRUE)
summary(lmm_quad)

lmm_quad_2group <- gridsearch(hlme(Glucose ~ numtime + I(numtime^2),
  random = ~numtime , ng = 2, mixture = ~numtime,
  subject = "numID", data = mean_cgm, maxiter = 50, verbose = TRUE
), rep = 50, maxiter = 50, minit = lmm)
summary(lmm_quad_2group)
