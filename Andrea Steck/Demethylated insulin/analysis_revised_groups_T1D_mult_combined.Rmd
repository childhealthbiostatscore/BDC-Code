...
corr_bmi <- rcorr(as.matrix(allpro[, c("BMI","age")]),type = "spearman")
corr_bmi <- flattenCorrMatrix(corr_bmi$r, corr_bmi$P)

# correlation of BMI-Z and age
corr_bmiz <- rcorr(as.matrix(allpro[, c("bmiz","age")]),type = "spearman")
corr_bmiz <- flattenCorrMatrix(corr_bmiz$r, corr_bmiz$P)
...
t1 <- final_table(tab1data,c("age","Gender","race_eth","DPT1Risk","DR3","DR4","DR3DR4","HLA","HbA1c","BMI", "bmiz","followup"),group=as.factor(tab1data$group2),margin=2,single=0,ron=1)
...
# ECL pos vs neg
mix_year_adj_ecl <- lme(Pro_pep~ year_floor + as.factor(ecl_overall) + year_floor:as.factor(ecl_overall) + age + Gender + BMI, 
               random = ~1 | Random_ID, data=long, na.action = na.omit)
# type III tests
...
Table 11a shows the correlation of BMI-Z and age.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(corr_bmi,caption="Table 11a.  Correlation between BMI-Z and age.")
```
<br>

Table 11b shows the correlation of BMI and age.

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(corr_bmi,caption="Table 11b.  Correlation between BMI and age.")
```
<br>
...
Table 13 shows the results of the mixed model testing whether the ECL positive and negative groups differ in the trajectories of proinsulin ratios over time, after adjustment for sex, age, and BMI.  The trajectories in the two groups were not significantly different (p=`r mix_year_adj_ecl_t[7,4]`).

```{r, results='asis',tidy=TRUE, echo=FALSE}
kable(mix_year_adj_ecl_t,caption="Table 13.  ANOVA table for mixed effects model comparing ECL positive and negative, adjusting for sex, age, and BMI.")
```
<br>
