...
find_lambda = function(df,outcome,metric = "bic",lambdas = seq(0,200),cores = 8){
  # Make model formula
...
lambda = find_lambda(df = bolus_morning,outcome = "boluses",lambdas = seq(0,200))
mod = glmmLasso(lambda[[2]], rnd = list(record_id=~1),data = bolus_morning,
                lambda=lambda[[1]],control = list(method = "REML"))
names(which(s$coefficients[,"Estimate"] != 0))
```
