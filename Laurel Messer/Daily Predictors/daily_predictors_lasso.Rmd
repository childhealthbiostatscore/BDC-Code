...
lambda_cv = function(df,outcome,metric = "bic",folds = 5,
  ...
  metrics = parLapply(cl,lambdas, function(l){
    ...
    cv = lapply(sample, function(s){
      ...
      # Test set
      test <- df[-s, ]
      # Fit - different results with REML vs. EM, so stick with REML for now (much slower)
      mod <- try(suppressWarnings(glmmLasso(form, rnd = list(record_id=~1),data = train,lambda=l,
                                            control = list(method = "REML",start = start_vals))),
                 silent = T)
...
lambda_perf = lambda_cv(df = bolus_morning,outcome = "boluses",lambdas = seq(0,5))
```
