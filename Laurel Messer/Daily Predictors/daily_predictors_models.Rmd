...
```{r results='asis'}
preds = c("tir","smg","boluses","combined","gsq")
...
for (v in preds) {
  ...
  cat("\n")
  # Fit simple model
  f = as.formula(paste0("dm_caretime~",v,"+(1|record_id)"))
  m = lmer(f,data = data)
  r = tidy(m,"fixed")
  r$effect = NULL
  print(kable(r,digits = 3))
...
for (v in preds) {
  ...
  cat("\n")
  # Fit simple model
  f = as.formula(paste0("dm_thinktime~",v,"+(1|record_id)"))
  m = lmer(f,data = data)
  r = tidy(m,"fixed")
  r$effect = NULL
  print(kable(r,digits = 3))
...
for (v in preds) {
  ...
  r$effect = NULL
  cat("\n")
  cat(paste0("The mean daily change was ",round(r$estimate[1],2),
             " +/- ",round(r$std.error[1],2)))
  cat("\n")
}
```
...
```{r results='asis'}
m = glmer(boluses ~ scale(tir) + (1|record_id),data = data,family = "poisson")
r = tidy(m,"fixed",exponentiate = T)
r$effect = NULL
kable(r,digits = 3)
```

Boluses were modeled using a Poisson distribution because they are count data. TIR had to be centered and scaled for the model to work, so the interpretation of the "scale(tir)" coefficient is: "for a one SD change in TIR above the mean, the rate is expected to change by rate * `r round(r$estimate[2],3)`."

## SMG 

```{r results='asis'}
m = glmer(boluses ~ scale(smg) + (1|record_id),data = data,family = "poisson")
r = tidy(m,"fixed",exponentiate = T)
r$effect = NULL
kable(r,digits = 3)
```

Boluses were modeled using a Poisson distribution because they are count data. TIR had to be centered and scaled for the model to work, so the interpretation of the "scale(smg)" coefficient is: "for a one SD change in SMG above the mean, the rate is expected to change by rate * `r round(r$estimate[2],3)`."
