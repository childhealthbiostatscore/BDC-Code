...
The p-value for "Diagnosis" in the table of type 3 tests of fixed effects tests whether there was a significant difference groups averaged over time points.
...
```{r, echo=FALSE, message=FALSE}
mod <- lme(X..Templates ~ Diagnosis,random=~1|Patient,data = dats7_sag,na.action = na.omit)
mod_anova <- anova.lme(mod, type="marginal")
mod_means <- emmeans(mod,~Diagnosis)
mod_pairs <-  pairs(mod_means,adjust="tukey",simple="Diagnosis")
...
```{r, echo=FALSE, message=FALSE}
mod <- lme(X..Unique.CDR3s ~ Diagnosis,random=~1|Patient,data = dats7_sig,na.action = na.omit)
mod_anova <- anova.lme(mod, type="marginal")
mod_means <- emmeans(mod,~Diagnosis)
mod_pairs <-  pairs(mod_means,adjust="tukey",simple="Diagnosis")
