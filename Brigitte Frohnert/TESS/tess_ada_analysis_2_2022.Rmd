...
```{r results='asis'}
# - CGM (% time above 140, average sensor glucose, sensor SD, number of days worn)
# - HbA1c
# - BMI z-score
# - Age at visits (give Date of birth AND Date of visits)
df = df %>% group_by(cgm_tess_id) %>%
  summarise(age_baseline = Age[Visit == "Baseline"],
            age_3_months = Age[Visit == "Baseline"],
            BMIz = BMIz[Visit == "Baseline"],
            HbA1c = A1C[Visit == "Baseline"],
            cgm_hyper_diff = cgm_hyper[Visit != "Baseline"] - cgm_hyper[Visit == "Baseline"],
            cgm_average_diff = cgm_average[Visit != "Baseline"] - cgm_average[Visit == "Baseline"],
            cgm_std_dev_diff = cgm_std_dev[Visit != "Baseline"] - cgm_std_dev[Visit == "Baseline"],
            cgm_num_days_diff = cgm_num_days[Visit != "Baseline"] - 
              cgm_num_days[Visit == "Baseline"],
            .groups = "drop")
# Table
vars = colnames(df)[-1]
f = as.formula(paste0("~",paste0("wt(",vars,",'median','q1q3')",collapse = "+")))
...
df = full_join(df,family,by = c("cgm_tess_id" = "pf1_tess_id","Visit"))
# Add parent report
...
df = full_join(df,parent,by = c("cgm_tess_id" = "pp1_tess_id","Visit"))
# Sort
df = df %>% arrange(cgm_tess_id,Visit)
# Calculate change
