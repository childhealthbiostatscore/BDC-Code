...
```{r results='asis'}
f = as.formula(paste0("FDR ~ Age + BMIz + A1C + ",
                      paste0(c(cgm_vars,ogtt_vars),collapse = "+")))
t1 = tableby(f,data = df)
summary(t1,labelTranslations = list(cgm_num_days = "CGM Days",
                                    cgm_peaks_gt_200 = "CGM Peaks > 200",
                                    cgm_hyper = "CGM Time in Hyperglycemia",
                                    cgm_hypo = "CGM Time in Hypoglycemia",
                                    cgm_target = "CGM Time in Target Range",
                                    cgm_mean = "CGM Mean",cgm_std_dev = "CGM SD",
                                    cgm_cv = "CGM CV"))
```
