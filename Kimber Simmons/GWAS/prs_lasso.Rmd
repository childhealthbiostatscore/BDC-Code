...
pheno$Sex = NULL
write.table(pheno,"./pheno.txt",quote = F,sep = "\t",row.names = F)
```

# Traditional PGRS Approach

```{r cache=TRUE}
pgrs_nums = read.delim("./prsice_test.prsice")
ggplot(pgrs_nums[1:50,],aes(x=Threshold,y=Num_SNP)) + geom_point()
```

```{r cache=TRUE,include=FALSE}
# Summary statistics from plink logistic regression
ss <- read.delim(paste0("logistic_",pc_num,"pcs.PHENO1.glm.logistic.hybrid"))
ss <- ss[ss$TEST=="ADD",]
