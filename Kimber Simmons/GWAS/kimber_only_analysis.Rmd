...
# Get genes
genes = getBM(attributes = c('refsnp_id',"associated_gene"), 
                   filters = c('snp_filter'),values = res$ID, mart = snp_mart)
genes$row = paste0("col",1:nrow(genes))
genes = genes %>% pivot_wider(id_cols = refsnp_id,names_from = row,values_from = associated_gene)
genes[genes==""] = NA
genes = genes %>% unite(g,col1:col266,na.rm = T,sep = ",")
genes$g = unlist(lapply(genes$g,function(c){
  l = unique(strsplit(as.character(c),",")[[1]])
  paste0(l,collapse = ", ")
}))
res$Genes = genes$g[match(res$ID,genes$refsnp_id)]
# Print
kable(res)
```
...
genes = genes %>% pivot_wider(id_cols = refsnp_id,names_from = row,values_from = associated_gene)
genes[genes==""] = NA
genes = genes %>% unite(g,col1:col63,na.rm = T,sep = ",")
genes$g = unlist(lapply(genes$g,function(c){
  l = unique(strsplit(as.character(c),",")[[1]])
  paste0(l,collapse = ", ")
}))
