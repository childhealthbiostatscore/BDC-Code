...
```{bash plink filter missing, cache=TRUE}
...
```{bash plink call rate,cache=TRUE}
...
```{bash plink sex check pre, cache=TRUE}
...
```{bash plink sex check post, cache=TRUE}
...
```{bash plink maf check pre, cache=TRUE} 
...
```{bash plink maf check post, cache=TRUE}
...
```{bash plink hw check pre, cache=TRUE}
...
```{bash plink hw check post, cache=TRUE}
# Filter for controls
...
```{bash plink heterozyg check pre, cache=TRUE}
...
```{bash plink heterozyg check post, cache=TRUE}
...
```{bash plink related check pre, cache=TRUE}
...
temp <- read.table("./Simmons_combined_7.fam")
```
