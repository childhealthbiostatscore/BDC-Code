...
# Plots 
hist(indmiss[,5],main="Histogram proportion missing SNPs per individual",
...
# Delete SNPs
plink2 --bfile simmons --geno 0.02 --make-bed --out simmons_1
# Delete individuals
plink2 --bfile simmons_1 --mind 0.02 --make-bed --out simmons_1
# Check missing post-deletion
plink2 --bfile simmons_1 --missing --out miss_post_del
```

```{r r missing plots post-deletion,echo=FALSE}
# Post-deletion
indmiss_check <-
  read.table(file="./miss_post_del.smiss")
snpmiss_check <-
  read.table(file="./miss_post_del.vmiss")
# Plots
hist(indmiss_check[,5],main="Histogram proportion missing SNPs per individual",
     xlab = "Proportion Missing")
...
indmiss_check$id <- gsub(".*_","",indmiss_check$V2)
keep <- indmiss_check %>% group_by(id) %>% slice(which.min(V5))
...
plink2 --bfile simmons_1 --keep call_rate_keep.txt --make-bed --out simmons_2
```

```{r echo=FALSE}
# Read in deleted 
post_repeated_subs_del <- read.table("./simmons_2.fam") 
```
...
plink --bfile simmons_2 --check-sex
```
...
sex_fam <- read.table("./simmons_2.fam")
sex_fam$V5 <- 
...
plink2 --bfile simmons_2 --remove sex_discrepancy.txt --make-bed --out simmons_3
# Check sex discrepancy again
plink --bfile simmons_3 --check-sex --out sex_post_del
```
...
plink2 --bfile simmons_3 --snps-only --make-bed --out simmons_4
# Check minor allele frequency
plink2 --bfile simmons_4 --freq --out MAF_check
```

```{r r maf check pre, echo=FALSE}
simmons <- read.table("./simmons_3.bim")
# MAF file
...
plink2 --bfile simmons_4 --maf 0.01 --make-bed --out simmons_5
# Check minor allele frequency again
plink2 --bfile simmons_5 --freq --out MAF_check_post
```

```{r r maf check post, echo=FALSE}
# MAF file
maf_freq <- read.table("./MAF_check_post.afreq", header =TRUE, as.is=T)
hist(maf_freq[,5],main = "MAF distribution", xlab = "MAF")
```
...
# Filter for controls
plink2 --bfile simmons_5 --hwe 1e-6 --make-bed --out simmons_6
# Filter for cases
plink2 --bfile simmons_6 --hwe 1e-12 --hwe-all --make-bed --out simmons_6
# Check again
plink2 --bfile simmons_6 --hardy --out post_del
# Select SNPs with HWE p-value below 0.00001
...
plink --bfile simmons_6 --indep-pairwise 50 5 0.2 --out indepSNP
plink --bfile simmons_6 --extract indepSNP.prune.in --het --out R_check
```
...
plink2 --bfile simmons_6 --remove het_fail_ind.txt --make-bed --out simmons_passed_qc
...
```{r}
simmons <- read.table("./simmons_pruned.bim")
