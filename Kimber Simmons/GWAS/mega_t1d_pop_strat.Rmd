...
# Reference races
race<- read.csv(file="/Volumes/Tim/kimber/population_stratification/race_1kG.csv",stringsAsFactors = F)
colnames(race) <- c("FID","V2","race")
race <- full_join(race,fam[,c("V2","race")],by = "V2")
race <- race %>% mutate(race = coalesce(race.x,race.y))
colnames(race)[2] <- "IID"

data<- read.table(file="/Volumes/Tim/kimber/population_stratification/MDS_merge2.mds",header=TRUE)

datafile<- left_join(data,race[,c("IID","FID","race")],by=c("IID"))
# Plot
