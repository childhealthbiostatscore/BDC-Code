...
knitr::opts_knit$set(root.dir = "/Users/timvigers/Documents/simmons")
library(knitr)
library(qqman)
library(tidyverse)
```
...
A first attempt to merge the datasets  revealed 231,289 variants with three or more alleles. After flipping these variants, there remained 1,125 positions with different variant IDs in the two datasets. Variant IDs, position, and alleles were checked using the programming language R version 4.0.2 (https://www.r-project.org/). 524 variants appeared to be genuinely different between the two datasets, so these SNPs were excluded. The remaining variants were re-named in the Biobank dataset to match the Simmons data naming conventions.
