...
```{bash combine files,cache=TRUE,results = 'hide'}
# Combine files
plink --bfile Simmons_redo --merge-list files_to_combine.txt --make-bed --out Simmons_combined
```

```{r check combine,echo=FALSE,cache=TRUE}
combined <- read.table("Simmons_combined.fam")
combined$V6 <- 2
write.table(combined, file = "./Simmons_combined.fam",row.names = F,sep = "\t",col.names = F,quote = F)
...
```{bash plink missing,cache=TRUE,results = 'hide'}
# Remove IDs with NA
...
```{r r missing plots pre-deletion, echo=FALSE, cache=TRUE}
...
```{bash plink filter missing, cache=TRUE,results = 'hide'}
...
```{r r missing plots post-deletion, echo=FALSE, cache=TRUE}
# Post-deletion
indmiss_check <-
...
```{bash plink call rate,cache=TRUE,results = 'hide'}
...
```{bash plink sex check pre, cache=TRUE,results = 'hide'}
...
```{bash plink sex check post, cache=TRUE,results = 'hide'}
...
```{bash plink maf check pre, cache=TRUE,results = 'hide'} 
...
```{bash plink maf check post, cache=TRUE,results = 'hide'}
...
```{bash plink hw check pre, cache=TRUE,results = 'hide'}
...
```{bash plink hw check post, cache=TRUE,results = 'hide'}
# Filter for controls
...
```{bash plink heterozyg check pre, cache=TRUE,results = 'hide'}
...
```{bash plink heterozyg check post, cache=TRUE,results = 'hide'}
...
```{bash plink related check pre, cache=TRUE,results = 'hide'}
