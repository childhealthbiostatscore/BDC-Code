...
# PRSice - requires the covar.txt and pheno.txt files generated by prs_lasso.Rmd
# --lower is the starting p value threshold
# --interval is the p value interval
# --no-regress shows all p-value threshold results
PRSice_linux \
  -b plink2.PHENO1.glm.logistic.hybrid \
  -t merged_imputed_qc \
  --snp ID \
  --binary-target T \
  --stat OR \
  ...
  --thread 8 \
  --out prsice_test
# Based on the PRSice results 1e-20 is the best cutoff (PRS around 50 SNPs)
# Fit with plink
awk '{print $3,$13}' logistic_4pcs.PHENO1.glm.logistic.hybrid > SNP.pvalue
echo "1e-20 0 1e-20" > range_list 
plink \
  --bfile merged_imputed_qc \
  --score logistic_4pcs.PHENO1.glm.logistic.hybrid 3 4 12 header \
