...
pathstart <- os.check()
source(paste0(pathstart,'som/PEDS/RI Biostatistics Core/Shared/Laura Tim projects/Shideh Majidi/High Risk Program/Data_Cleaned/HighRiskProgramClean_R_2018-11-20_1104.r'))
data$pt_status <- NULL
library(lubridate)
library(zoo)
library(data.table)
...
dates <- read.csv(paste0(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Laura Tim projects/Shideh Majidi/High Risk Program/Data_Cleaned/StartEndDates.csv"))
dates <- dates[,c("record_id","program_start_date","program_end_date","Gender")]
data <- merge(data,dates,all.x = T,by = "record_id")
# Fill down ID columns
idcols <- c("record_id","name","mrn","control","case_id","dob")
data[,idcols] <- na.locf(data[,idcols])
# Re-format columns
data$Gender <- as.factor(data$Gender)
data$a1c <- as.numeric(data$a1c)
data$visitdate <- ymd(as.character(data$visitdate))
data$dob <- ymd(as.character(data$dob))
data$program_start_date <- mdy(as.character(data$program_start_date))
data$program_end_date <- mdy(as.character(data$program_end_date))
data$case_id <- as.factor(data$case_id)
data$control <- as.factor(data$control)
data$sw.factor <- as.character(data$sw.factor)
...
# Set incorrect TIR values to missing.
incorrect.bgs <- read.csv(paste0(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Laura Tim projects/Shideh Majidi/High Risk Program/Data_Cleaned/RoundingBGs.csv"))
incorrect.bgs <- semi_join(data,incorrect.bgs,by = c("record_id","redcap_event_name"))
rows <- which(!is.na(row.match(data,incorrect.bgs)))
data$highbg[rows] <- NA
data$targetbg[rows] <- NA
data$lowbg[rows] <- NA
