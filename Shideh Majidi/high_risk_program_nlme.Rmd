...
dates <- read.csv(paste0(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Shideh Majidi/High Risk Program/dat_Cleaned/StartEndDates.csv"))
...
incorrect.bgs <- read.csv(paste0(pathstart,"som/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Shideh Majidi/High Risk Program/dat_Cleaned/NotVerifiedBG.csv"))
...
dat$timepoint <- ifelse(dat$visitdate <= dat$program_start_date,"Before",
                         ifelse(dat$visitdate > dat$program_start_date & dat$visitdate <= dat$program_end_date,"During","After"))
...
a1c_mod <- lme(a1c ~ timepoint * control,random=~1+case_id|record_id,dat = mixed,na.action = na.omit) # Not enough observations
