...
data <- data %>% 
  ...
  mutate(Age = round(difftime(visitdate,dob,units = "days")/365.2425,3)) %>%
  mutate(T1Dyears = round(difftime(dx,dob,units = "days")/365.2425,3)) %>%
  mutate(base_a1c = a1c[2])
```

```{r echo=FALSE}
# Match each case to closest control (out of 3)
match <- data %>%
  group_by(record_id) %>%
  filter(row_number()==1) %>%
  arrange(case_id)
match$treat <- ifelse(match$control == "No",1,0)
matched <- matchit(treat ~ T1Dyears,match,ratio =1)
```
