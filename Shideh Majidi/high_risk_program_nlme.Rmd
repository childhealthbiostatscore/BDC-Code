...
# Record ID as factor
data$record_id <- as.factor(as.numeric(data$record_id))
```
...
data$timepoint <- ifelse(data$visitdate <= data$program_start_date,"Before",
                         ifelse(data$visitdate > data$program_start_date & data$visitdate <= data$program_end_date,"During","After"))
data$timepoint <- factor(data$timepoint,levels = c("Before","During","After"))
# Filter out rows with just demographic information
mixed <- data %>% filter(!is.na(visitdate))
plot_mixed <- mixed %>%
...
a1c_mod <- lme(a1c ~ timepoint * control,random=~1|record_id/case_id,data = mixed,na.action = na.omit)
a1c_mod_2 <- lme(a1c ~ timepoint * control,random=~1|case_id/record_id,data = mixed,na.action = na.omit)
```
