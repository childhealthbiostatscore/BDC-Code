...
Aim 1a: In Control patients only, the A1c will be different post-RTC depending on whether the patient participated in <4 versus 4+ RTC visits.  A linear mixed model was fit to control subjects only with the outcome was A1c.  The main predictors were number of RTC visits (<4 versus 4+), time period (RTC versus post-RTC), and an interaction between number of RTC visits and time period.  The model was also adjusted for A1c at baseline, days from first visit, gender, insurance (public vs private), and Race/Ethnicity (Non-hispanic white versus other).  If the interaction term was significant, this would indicate that A1c trajectory in the post-RTC period differed depending on the number of RTC visits. 
...
After adjusting for all other variables in the model, baseline A1c was associated with average A1c (higher baseline = higher average), there was a significant time trend (p=`r lmm_1a_sum[3,3]`), and hte average A1c in the Post-RTC period was higher (`r lmm_1a_sum[7,3]`). There was a significant difference in A1c between RTC and Post-RTC time periods depending on whether the patient participated in 4+ RTC visits (p=`r lmm_1a_sum[9,3]`). More detail on this interaction is found below:
...
Control patients that had 4+ RTC visits did not have a significant change in A1c between RTC and post-RTC time periods (p=`r contrasts_1a[1,4]`).  Those with 2-3 visits had a significant increase in A1c between RTC and post-RTC time periods (p=`r contrasts_1a[2,4]`).
...
After adjusting for all other variables in the model, baseline A1c was associated with average A1c (higher baseline = higher average), there was a significant time trend (p=`r lmm_1b_sum[3,3]`), and the average A1c in the CTC period was higher (`r lmm_1b_sum[5,3]`). There was a significant difference in A1c between RTC and CTC time periods depending on whether the patient participated in 4+ RTC visits (p=`r lmm_1b_sum[7,3]`). More detail on this interaction is found below:

```{r,echo=FALSE,message=FALSE,results='asis',warning=FALSE}
kable(contrasts_1b, align = c("l", "l"))
```

CTC patients that had 4+ RTC visits did not have a significant change in A1c between pre and post time periods (p=`r contrasts_1b[1,4]`).  Those with 2-3 visits had a significant increase in A1c between RTC and CTC time periods (p=`r contrasts_1b[2,4]`).
...
A1c did not differ signifiacntly post-RTC for those in CTC group compared to the control group (p=`r lmm_2_sum[5,3]`).
