...
# write.csv(a1cs_wide,file = "/Users/timvigers/Desktop/a1cs_wide.csv",
#           row.names = F,na="")
# write.csv(a1cs,file = "/Users/timvigers/Desktop/a1cs.csv",
#           row.names = F,na="")
```

```{r traj model,echo=FALSE,message=FALSE,include=FALSE,cache=TRUE}
# Basic trajectory model:
# Dependent variable = HbA1c
# Independent = time
a1cs = as.data.frame(a1cs)
m1 = hlme(OrderValue ~ time,subject="ID",data=a1cs,ng = 1)
mod1 = 
  gridsearch(hlme(OrderValue ~ time,mixture = ~time,subject="ID",data=a1cs,ng = 3),
...
```{r plot trajectories,echo=FALSE}
plot_time = as.data.frame(seq(0,max(as.numeric(a1cs$time))))
colnames(plot_time) = "time"
pred <- predictY(mod1, plot_time, var.time = "time")
```

```{r assign groups,echo=FALSE,results='asis'}
# Predicted groups based on model
groups = as.data.frame(mod1$pprob)
# Add to demographics
demographics = left_join(demographics,groups[,c("ID","class")],by = "ID")
gtable = tableby(RiskCategory_coded ~ factor(class),
                     data = demographics)
summary(gtable,digits = 2,
        labelTranslations = list(`factor(class)` = "HbA1c Trajectory Class"))
# Pairwise tests
t = table(demographics$class,demographics$RiskCategory_coded)
...
pair_chi$`p value` = format.pval(pair_chi$`p value`,eps = 0.001,digits = 3)
# Kable
kable(pair_chi,row.names = T,caption = "Pairwise Risk Comparisons")
```
