...
write.csv(a1cs_wide,file = "/Users/timvigers/a1cs_wide.csv",row.names = F,na="")
write.csv(a1cs,file = "/Users/timvigers/a1cs.csv",row.names = F,na="")
```

```{r traj plot,echo=FALSE,message=FALSE}
ggplot(a1cs,aes(x = as.numeric(time), y = OrderValue, color = RiskCategory_coded)) + 
  geom_point(pch = ".",alpha = 0.5) +
  geom_smooth() + theme_bw()
```

```{r traj model,echo=FALSE,message=FALSE,include=FALSE}
# Basic trajectory model:
# Dependent variable = HbA1c
# Independent = time
a1cs = as.data.frame(a1cs)
a1cs = a1cs[a1cs$visit <= 17,]
mod1 = hlme(OrderValue ~ time,mixture = ~time,subject="ID",data=a1cs,ng = 3)
# Summary
s = summary(mod1)
```
