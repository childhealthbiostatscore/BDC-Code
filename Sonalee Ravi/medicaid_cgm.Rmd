...
```{r echo=FALSE}
# Check A1c at limit of detection
length(which(delta$HbA1c > 14))
# Only 15, probably okay?
# Table 1 by CGM = "Yes" vs CGM = "No"
vars <- c("HbA1c","PumpUse")
nonnormal <- norm.check(delta,c("HbA1c","Age_AtEncounterDate"))
f2l_comps <- CreateTableOne(vars,strata = "CGMUse",delta,testNonNormal = wilcox.test,argsNonNormal = list(paired = T))
print(f2l_comps,nonnormal = nonnormal)
```