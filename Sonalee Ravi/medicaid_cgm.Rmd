...
tir_descriptives <- print(tir_descriptives,nonnormal = "CGMLowPercent",printToggle = F)
...
levels(dat$mod_race) <- c("Other","Other","Hispanic","Other","Other",
                          ...
                          "Other")
# Can't get this to converge, but works fine without random effects. It will with pconv = 0.1. Scaling continuous variables doesn't help.
write.csv(dat,file = "/Users/timvigers/Desktop/cgm_medicaid.csv",
          row.names = F,na = "")
```

```{r echo=FALSE}
# Results require running "glmer in SAS.sas" code
t3 <- read_excel("/Users/timvigers/Desktop/type3.xlsx")
t3$FValue <- round(t3$FValue,3)
t3$ProbF <- round(t3$ProbF,3)
results <- read_excel("/Users/timvigers/Desktop/predict_cgm.xlsx",na = ".")
results[,5:ncol(results)] <- lapply(results[,5:ncol(results)],function(x) round(x,3))
sigt3 <- which(t3$ProbF < 0.05)
kable(t3,caption = "Overall Effects",na = "")
kable(results,caption = "Results",na = "")
```

Results above presented on the log scale.

Overall, age and race predicted CGM use >= 85% (p = `r t3$ProbF[sigt3[1]]` and p = `r t3$ProbF[sigt3[2]]` respectively). On average, each year increase in age resulted in a `r round((1 - exp(results$Estimate[3]))*100,2)`% decrease in the odds of successful CGM use (p = `r as.numeric(results$Probt[3])`). Also, Hispanic participants were `r round((1 - exp(results$Estimate[7]))*100,2)`% less likely to successfully use CGM compared to non-Hispanic whites (p = `r as.numeric(results$Probt[7])`). Days since last visit was not associated with successful CGM use (p = `r as.numeric(results$Probt[2])`).
