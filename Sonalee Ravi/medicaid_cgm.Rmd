...
cgm_use_mod <- lme(CGM_Perc ~ days_since_last + Age_AtEncounterDate + DiabetesDuration + PumpUse + Race.Ethnicity,
               random=~1|EPICMRN,data = dat, na.action = na.omit)
```
...
use_pred <- glm(factor(cgm_binary) ~ Age_AtEncounterDate + DiabetesDuration + 
                    PumpUse + Race.Ethnicity,
                  dat[dat$CGMUse == "Yes",],family = "binomial")
car::vif(use_pred)
...
use_pred <- glmer(factor(cgm_binary) ~ scale(Age_AtEncounterDate) + scale(DiabetesDuration) + 
                    PumpUse + Race.Ethnicity + (1|EPICMRN),
                  dat[dat$CGMUse == "Yes",],family = "binomial",
