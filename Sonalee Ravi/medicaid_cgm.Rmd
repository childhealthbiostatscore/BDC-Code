...
plot(residuals(mod_int)~fitted(mod_int),type="p",cex=0)
text(residuals(mod_int)~fitted(mod_int),labels = delta$HbA1c,cex=0.6)
hist(delta$HbA1c)
barplot(table(delta$cgm_pump))
```
...
plot(mod_cgm_ever)
hist(dat$HbA1c)
barplot(table(dat$cgm_ever_pump))
```
...
mod_adj_int_slp <- lme(HbA1c ~ CGM_Perc + Age_AtEncounterDate + DiabetesDuration + PumpUse + Race.Ethnicity-1,
...
plot(tir_mod)
hist(dat$HbA1c)
hist(dat$CGMOKPercent)
```

# To Do
1. CHECK RANDOM SLOPE IN LME4
2. Tobit's regression for censured values