...
# Replace pump use NA with "No" as it's unlikely they would have a pump and not 
# have data in this field (per Sonalee)
dat$PumpUse[which(is.na(dat$PumpUse))] <- "No"
...
rownames(on_off_results) <- c("Intercept","cgmNo:pumpYes","cgmYes:pumpNo","cgmYes:pumpYes")
...
never_some_comp <- CreateTableOne(c("mean(HbA1c)",vars),strata = "CGMUse",never_some)
never_some_comp <- print(never_some_comp,nonnormal = c("mean(HbA1c)",nonnormal))
```
...
The above model effectively treats CGM and pump status as a treatment group, and examines the effect on HbA1c. The reference group is no pump and no CGM. So, on average those on a pump without CGM (cgmNo:pumpYes) had an HbA1c 1.528 percentage points lower than those on neither pump or CGM (p=0.0001298). The type 3 test of fixed effects indicates that combined CGM and pump status was significant overall.

### Table 5: Mean HbA1c and descriptive statistics for individuals with some CGM time vs. those never on CGM
```{r echo=FALSE}
kable(never_some_comp[,1:3])
```
