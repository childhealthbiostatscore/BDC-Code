...
```{r echo=FALSE}
kable(t1_full,caption = "Table 1: Descriptive Statistics for Full Cohort")
```
...
```{r echo=FALSE}
kable(f2l_comps[,1:4],caption ="HbA1c Comparisons Stratified by CGMUse:PumpUse (Kruskal-Wallis Rank Sum Test)")
```

```{r echo=FALSE,include=FALSE}
delta <- delta %>% 
  unite(cgm_pump,CGMUse,PumpUse,remove = F)
# Use mixed model to test this
```