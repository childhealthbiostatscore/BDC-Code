...
# Tableone
never_some_comp <- CreateTableOne(c("mean(HbA1c)",vars,"AI_AN","Asian","Hispanic","more_than_one","NativeH_OPI","NH_Black","NH_White","Other","Unknown"),strata = "CGMUse",never_some)
never_some_comp <- print(never_some_comp,nonnormal = c("mean(HbA1c)",nonnormal),exact = c("AI_AN","Asian","Hispanic","more_than_one","NativeH_OPI","NH_Black","NH_White","Other","Unknown"))
```
...
### Table 5b: Mixed model comparing HbA1c never on CGM to HbA1c off CGM, adjusting for pump status
...
mod_cgm_ever <- lme(HbA1c ~ cgm_ever_pump,random=~1|EPICMRN,data = dat,na.action = na.omit)
...
# Means
mod_cgm_ever_mean <- lme(HbA1c ~ cgm_ever_pump-1,random=~1|EPICMRN,data = dat,na.action = na.omit)
cgm_ever_results_mean <- as.data.frame(summary(mod_cgm_ever_mean)$tTable)
rownames(cgm_ever_results_mean) <- c("cgmNever:pumpNo","cgmNever:pumpYes","cgmSome:pumpNo","cgmSome:pumpYes")
```

```{r echo=FALSE}
kable(cgm_ever_results, caption = "Fixed Effects")
kable(anova.lme(mod_cgm_ever), caption = "Type 3 Tests of Fixed Effects")
kable(cgm_ever_results_mean, caption = "Group Means")
```

CGM uptake = $\frac{177}{892}=19.84\text{ %}$

Pump use for never on CGM:
...
pumpuse <- left_join(never_some,pumpuse,by = "EPICMRN")
kable(pumpuse %>% filter(CGMUse == "Never") %>% count(Pump))
```

Pump use for those with some CGM:

```{r echo=FALSE}
kable(pumpuse %>% filter(CGMUse == "Some") %>% count(Pump))
```
...
mm_plot <- ggplot(data = dat, aes(x = CGM_Perc, y = HbA1c,group = EPICMRN)) +
  ...
  geom_smooth(aes(x = CGM_Perc, y = HbA1c),method='lm',inherit.aes = F,size=0.5) + 
  xlab("% CGM Use") + 
  ylab("HbA1c (%)") + 
...
tir_t1 <- print(tir_t1,nonnormal = nonnormal)
```
...
tir_plot <- ggplot(data = tir, aes(x = CGMOKPercent, y = HbA1c,group = EPICMRN)) +
  geom_line(size = 0.2,aes(color = EPICMRN)) +
  geom_smooth(aes(x = CGMOKPercent, y = HbA1c),method='lm',inherit.aes = F,size=0.5) + 
  xlab("% Time in Range") + 
  ylab("HbA1c (%)") + 
  theme(legend.position = "none")
tir_plot
