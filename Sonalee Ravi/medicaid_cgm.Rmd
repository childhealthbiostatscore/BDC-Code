...
delta <- rbind(yes,no)
# Calculate time difference in days between no and yes
delta <- delta %>% arrange(EPICMRN,EncounterDate) %>%
  group_by(EPICMRN) %>%
  mutate(diff = ifelse(CGMUse[1] == "No",difftime(EncounterDate,EncounterDate[1],units = "days"),NA))
