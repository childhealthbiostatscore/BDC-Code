...
n = number of observations (not number of participants)
...
kable(tir_descriptives)
```

## The Effect of Time since last visit on CGM Use

```{r echo=FALSE}
dat <- dat %>% arrange(EPICMRN,EncounterDate) %>% group_by(EPICMRN) %>%
  mutate(days_since_last = as.numeric(difftime(EncounterDate,EncounterDate[1],units = "days")))
cgm_use_mod <- lme(CGM_Perc ~ days_since_last + Age_AtEncounterDate + 
                     DiabetesDuration + PumpUse + Race.Ethnicity,
               random=~1|EPICMRN,data = dat, na.action = na.omit)
```
...
```{r echo=FALSE,warning=FALSE,dpi=600}
visnums <- dat %>% 
  filter(!is.na(CGMOKPercent)) %>%
  mutate(visitnum = row_number())
tir_mod <- lme(CGMOKPercent ~ factor(visitnum) + Age_AtEncounterDate + DiabetesDuration + PumpUse + Race.Ethnicity,
               random=~1|EPICMRN,data = visnums, na.action = na.omit)
ggplot(visnums,aes(x = visitnum,y = CGMOKPercent,group = EPICMRN)) + geom_line() + theme_bw()
format_nlme_out(tir_mod)
```
...
dat$mod_race <- ifelse(dat$Race.Ethnicity == "Non-Hispanic White","Non-Hispanic White",
                       ifelse(dat$Race.Ethnicity == "Hispanic","Hispanic","Other"))
dat$mod_race[is.na(dat$Race.Ethnicity)] <- NA
...
use_pred <- glmmPQL(factor(cgm_binary) ~ Age_AtEncounterDate + DiabetesDuration + 
                    PumpUse + mod_race, random =~1|EPICMRN,
                  dat,family = "binomial") 
```
