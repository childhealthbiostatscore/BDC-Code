...
### Figure 4: % CGM Use Over Time From First Visit With CGM 
...
### Figure 5: Time to First % CGM Use < 85%

```{r echo=FALSE,dpi=600}
survplot_dat <- km_plot_dat %>%
  select(EPICMRN,CGM_Perc,Days)
survplot_dat$discontinued <- ifelse(survplot_dat$CGM_Perc<85,1,0)
survplot_dat$discontinued[is.na(survplot_dat$discontinued)] <- 0
survplot_dat$discontinued[survplot_dat$Days == 0] <- 0
surv <- survfit(Surv(Days,event=discontinued) ~ 1, data = survplot_dat)
plot(surv,xlab = "Days",ylab = "Overall continuation probability")
```