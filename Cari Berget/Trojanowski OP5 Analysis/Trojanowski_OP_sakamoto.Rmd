...
baseline = analysis %>% filter(timepoint==0)
...
# fit model fun
fit_mod = function(outcome="paid_score",df=analysis, parent = F){
if(parent == F){
    # Fit model
  f = as.formula(paste0(outcome,"~","inspire_score*timepoint + ","demographics_sex + demographics_t1d_duration + hba1c + baseline_paid"))
  mod = lme(f,
            random = ~1|record_id,
            data = df,
            na.action = na.omit)
}else{
   f = as.formula(paste0("p_paid_score","~","p_inspire_score*timepoint + ","demographics_sex + demographics_t1d_duration + hba1c + baseline_p_paid"))
  mod = lme(f,
            random = ~1|record_id,
            data = df,
            na.action = na.omit)
...
# problems with a1c and baseline paid for soem reason when fitting the models

# mod_test = lme(paid_score ~ inspire_score*timepoint + demographics_sex + demographics_t1d_duration + baseline_paid,
#             random = ~1|record_id,
#             data = analysis,
#             na.action = na.omit)
# mod_anova_test = anova.lme(mod_test)
# mod_means_test = emmeans(mod_test, specs=~timepoint, adjust="none")
# kable(mod_anova_test)
# kable(mod_means_test,digits = 3,caption = "Timepoint Means")
```
# Background
...
## Child Model
```{r m1}
analysis2 = analysis %>% filter(timepoint %in% c("Baseline", "1 Month", "3 Months"))
fit_mod(df = analysis2)
```
