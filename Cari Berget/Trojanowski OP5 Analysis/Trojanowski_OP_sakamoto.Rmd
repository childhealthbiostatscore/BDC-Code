...
# test = analysis %>% filter(record_id == "102_op5") -- which one should be dropped/changed for 102op5


# test = analysis %>% filter(timepoint %in% c( "3 Months"))
# mo1_ppaid_ids = analysis %>% filter(timepoint %in% c("1 Month") & !is.na(p_paid_score)) %>% select(record_id)
# m3_ppaid_ids = analysis %>% filter(timepoint %in% c("3 Months") & record_id %in% mo1_ppaid_ids$record_id) 
# test = analysis %>% filter(timepoint %in% c("1 Month") & !is.na(p_inspire_score)) 148 ciq subj with 1 month data (0 op5)


```

For 102_op5, the responses that correspond to the 12/15/2022 time point should be used as the 3 mo data, not the 10/10/2022. It looks like these were out of order and so close together that the 12/15 date better corresponds to 3 mo from their baseline.

For the interpretation of the main effects and writing results, is it accurate to say that a one point increase in INSPIRE scores at any time point was associated with an XX point drop in PAID scores at the same time point? I’m a little confused on the temporal relationship. How would you best write up the main finding? [it might make sense to meet again once the results are finalized to discuss this point!]
