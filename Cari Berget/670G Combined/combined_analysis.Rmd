...
t1 <- tableby(cohort ~ hba1c_baseline + 
                ...
                kwt(days_to_m18,"Nmiss","median","q1q3"),
              data = demographics)
# Print
...
```{r results='asis'}
hist(model_df$sensor_g250)
ar1_mod_func(outcome = "scale(sensor_g250)",data = model_df,diagnostics = T)
```
...
Cutoffs were determined by fitting a generalized linear mixed model (GzLMM) with a binary outcome (0 = TIR <70% and 1 = TIR >=70%) and random intercept for subject to account for repeated measures. Then sensitivity and specificity were calculated based on predicted values from the GzLMM, using the pROC package. Optimal cutoffs were determined by maximizing Youden's J statistic.
...
# Fit GzLMM
am_mod <- glmer(tir_outcome ~ time_am + (1|record_id),
                data = model_df,family = "binomial")
est <- summary(am_mod)$coefficients[,1]
# ROC using the predicted values
gzlmm_pred <- predict(am_mod,type = "response")
roc <- roc(am_mod@resp$y,gzlmm_pred)
# Threshold to maximize Youden
c<-coords(roc,"best",best.method = "youden",transpose = F)
# Find the participant closest to this threshold and use their TIR
w <- as.numeric(which.min(abs(gzlmm_pred-c$threshold)))
am_mod@frame$time_am[w]
```

```{r}
t <- df %>% filter(cohort == "Adult") %>% 
  select(record_id,timepoint,time_am) %>% 
  pivot_wider(id_cols = record_id,names_from = timepoint,
              values_from = time_am) %>%
  select(record_id,Baseline,M1,M3,M6,M9,M12)
t <- left_join(t,discontinuers[,c("record_id","discontinued")],
               by = "record_id") %>%
