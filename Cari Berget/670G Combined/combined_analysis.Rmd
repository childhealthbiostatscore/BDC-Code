...
ar1_mod_func <- function(outcome,data = df,diagnostics=F){
...
discontinuers <- df %>% filter(timepoint != "Baseline") %>% 
  group_by(record_id) %>% mutate(time_am_0 = ifelse(time_am > 0,1,0)) %>%
  summarise(
    discontinued = 
      ifelse(0 %in% diff(time_am_0) & 
               1 %in% diff(time_am_0),"Intermittent",
             ifelse(0 %in% time_am,"Discontinued","Continued"))) %>%
  ungroup()
kable(table(discontinuers$discontinued),col.names = c("","n"))
```
