...
```{r results='asis'}
hist(model_df$hba1c)
ar1_mod_func(outcome = "hba1c",data = model_df,diagnostics = T)
```

## % Time in AM

```{r results='asis'}
hist(model_df$time_am)
ar1_mod_func(outcome = "time_am",data = model_df,diagnostics = T)
```

## % CGM Wear

```{r results='asis'}
hist(model_df$sensor_wear)
ar1_mod_func(outcome = "sensor_wear",data = model_df,diagnostics = T)
```

## AM Exits per Day

```{r results='asis'}
hist(model_df$amexits_day)
ar1_mod_func(outcome = "amexits_day",data = model_df,diagnostics = T)
```
...
```{r results='asis'}
hist(model_df$bg_checks)
ar1_mod_func(outcome = "bg_checks",data = model_df,diagnostics = T)
```

## Sensor Mean

```{r results='asis'}
hist(model_df$sensor_mean)
ar1_mod_func(outcome = "sensor_mean",data = model_df,diagnostics = T)
```

## % TIR < 70 mg/dL

```{r results='asis'}
hist(model_df$sensor_70)
# Exclude 1408816
ar1_mod_func(outcome = "sensor_70",diagnostics = T)
```

## % TIR 70 - 180 mg/dL

```{r results='asis'}
hist(model_df$sensor_70_180)
ar1_mod_func(outcome = "sensor_70_180",data = model_df,diagnostics = T)
```
...
ar1_mod_func(outcome = "sensor_181_250",data = model_df,diagnostics = T)
```
...
```{r results='asis'}
hist(log(model_df$sensor_g250))
ar1_mod_func(outcome = "scale(sensor_g250)",data = model_df,
             diagnostics = T)
```
...
```{r results='asis'}
hist(model_df$tdd)
ar1_mod_func(outcome = "tdd",data = model_df,diagnostics = T)
```

## % TDD Delivered as Bolus

```{r results='asis'}
hist(model_df$tdd_bolus)
ar1_mod_func(outcome = "tdd_bolus",data = model_df,diagnostics = T)
```

## % TDD Delivered as Basal

```{r results='asis'}
hist(model_df$tdd_basal)
ar1_mod_func(outcome = "tdd_basal",data = model_df,diagnostics = T)
```

## Meal Boluses per Day

```{r results='asis'}
hist(model_df$dailymealbolus)
ar1_mod_func(outcome = "dailymealbolus",data = model_df,diagnostics = T)
```
...
A threshold of `r thresh`% automode use predicted TIR >= 70% at 12 months or later with sensitivity of `r c$sensitivity` and specificity of `r c$specificity`. Area under the ROC curve was `r as.numeric(roc$auc)`.
...
res = broom.mixed::tidy(thresh_mod,conf.int = T,exponentiate=T)
