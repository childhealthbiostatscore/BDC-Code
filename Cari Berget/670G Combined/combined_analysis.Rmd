...
knitr::opts_knit$set(root.dir = "/Volumes/PEDS/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Cari Berget/670G Combined")
library(Hmisc)
library(arsenal)
library(skimr)
library(nlme)
...
# Exclude 1408816
ar1_mod_func(outcome = "sensor_70",data = model_df,diagnostics = T)
```
...
ar1_mod_func(outcome = "sensor_g250",data = model_df,diagnostics = T)
```
...
# Test with average TIR
s = model_df %>% group_by(record_id) %>% 
  summarise(tir_outcome = tir_outcome[timepoint == "M12"],
            am = mean(time_am))
```
