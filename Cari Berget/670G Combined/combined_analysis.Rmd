...
```{r results='asis'}
hist(df$hba1c)
ar1_mod_func(outcome = "hba1c",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$time_am)
ar1_mod_func(outcome = "time_am",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$sensor_wear)
ar1_mod_func(outcome = "sensor_wear",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$amexits_day)
ar1_mod_func(outcome = "amexits_day",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$bg_checks)
ar1_mod_func(outcome = "bg_checks",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$sensor_mean)
ar1_mod_func(outcome = "sensor_mean",data = df,diagnostics = T)
```

## % TIR 70 - 180 mg/dL

```{r results='asis'}
hist(df$sensor_70_180)
ar1_mod_func(outcome = "sensor_70_180",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$sensor_181_250[df$cohort=="Peds"])
ar1_mod_func(outcome = "sensor_181_250",data = df,diagnostics = T)
```
...
```{r results='asis'}
hist(df$sensor_g250)
ar1_mod_func(outcome = "scale(sensor_g250)",data = df,diagnostics = T)
```
