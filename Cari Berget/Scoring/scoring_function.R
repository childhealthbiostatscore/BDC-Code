...
parent_scores = function(df){
  ...
  scores = t(apply(df,1,function(r){
    # PAID
    p_paid_b = 5 - as.numeric(r[grep("p_paid\\d{,2}$",names(r))])
    ...
    p_paid = list(p_paid_b,p_paid_t1,p_paid_t2,p_paid_t3,p_paid_t4)
    p_paid = sapply(p_paid,function(x){mean(x,na.rm = T)*25})
    names(p_paid) = c("p_paid_b","p_paid_t1","p_paid_t2","p_paid_t3","p_paid_t4")
    # HFS maintain
    ...
    p_hfs_maintain = c(p_hfs_maintain_b,p_hfs_maintain_t1,p_hfs_maintain_t2,
                         p_hfs_maintain_t3,p_hfs_maintain_t4)
    names(p_hfs_maintain) = c("p_hfs_maintain_b","p_hfs_maintain_t1",
    ...
    p_hfs_helpless = c(p_hfs_helpless_b,p_hfs_helpless_t1,p_hfs_helpless_t2,
                         p_hfs_helpless_t3,p_hfs_helpless_t4)
    names(p_hfs_helpless) = c("p_hfs_helpless_b","p_hfs_helpless_t1",
    ...
    p_hfs_worry = c(p_hfs_worry_b,p_hfs_worry_t1,p_hfs_worry_t2,
                         p_hfs_worry_t3,p_hfs_worry_t4)
    names(p_hfs_worry) = c("p_hfs_worry_b","p_hfs_worry_t1",
    ...
    # Return all
    return(c(p_paid,p_hfs_maintain,p_hfs_helpless,p_hfs_worry,p_inspire))
  }))
  colnames(scores) = gsub("_b","_Baseline",colnames(scores))
  colnames(scores) = gsub("_t1","_T1",colnames(scores))
  colnames(scores) = gsub("_t2","_T2",colnames(scores))
  colnames(scores) = gsub("_t3","_T3",colnames(scores))
  colnames(scores) = gsub("_t4","_T4",colnames(scores))
  scores[is.nan(scores)] = NA
  scores = data.frame(scores)
  # Bind
  df = cbind(df,scores)
