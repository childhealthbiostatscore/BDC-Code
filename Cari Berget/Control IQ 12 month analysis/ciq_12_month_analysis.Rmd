...
```{r}
panelResult <- censReg(sensor_g250_trans ~ numeric_time, data = cens)
...
```{r}
panelResult <- censReg(sensor_g250_trans ~ numeric_time*hba1c_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(sensor_g250_trans ~ numeric_time*age_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(sensor_under_70_trans ~ numeric_time, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(sensor_under_70_trans ~ numeric_time*hba1c_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(sensor_under_70_trans ~ numeric_time*age_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
Participant 266 had a TDD of 789.70 at 9 Months, which was excluded for these models.
...
```{r}
panelResult <- censReg(sleep ~ numeric_time, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(sleep ~ numeric_time*hba1c_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(sleep ~ numeric_time*age_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(exercise_trans ~ numeric_time, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(exercise_trans ~ numeric_time*hba1c_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
...
```{r}
panelResult <- censReg(exercise_trans ~ numeric_time*age_group, data = cens)
s = summary(panelResult)$estimate
s = data.frame(s[1:(nrow(s)-2),])
kable(s,digits = 3)
```
