...
Time was treated as numeric because timepoints are not evenly spaced, and the interpretation of interactions between a large number of categories is difficult. The final model structure was selected using mean sensor glucose as the outcome, and the same model structure was used for all following models for consistency. We compared a model with random intercept for each participant to a model with both random intercept and random slope, and the random intercept and random slope model was the best by AIC and BIC. 
...
```{r model function}
fit_mod = function(df = mod_data,outcome,family = "gaussian",diagnostics = F){
  # Formula
  f = as.formula(paste0(outcome,"~numeric_time*hba1c_group + (numeric_time|record_id)"))
  # Fit
  mod = lmer(f,data = df)
...
```{r}
fit_mod(outcome = "sensor_mean",diagnostics = T)
```
