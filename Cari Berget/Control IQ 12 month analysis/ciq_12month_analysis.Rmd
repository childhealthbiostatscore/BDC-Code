...
data = data %>% filter(!record_id %in% discontinuers) %>%
  ...
  select(record_id,gyl_timepoint.factor,date,demographics_dob,
         age,age_group,hba1c,hba1c_group,sex,
...
pca = prcomp(pca_data)
new = as.matrix(pca$x) %*% pca$rotation[,1]

new_x = pca$center[1] + new
new_y = pca$center[2] + new
new_z = pca$center[3] + new

fig = 
  plot_ly(x=pca_data$sensor_mean, y=pca_data$hba1c,z=pca_data$sensor_70_180, 
        type="scatter3d",mode="markers",size = 2) %>%
  add_trace(x = new_x,y = new_y,z = new_z,type = 'scatter3d', mode = 'lines') %>%
  layout(scene = list(xaxis=list(title = "Sensor Mean"),
                      yaxis=list(title = "HbA1c"),zaxis=list(title = "TIR")))
```
