...
```{r test}
a = analysis
b = analysis %>% filter(record_id == 3 & redcap_event_name == "Month 1") %>% select(inspire_score:seeds_score)

analysis_m3[3,24:29] = b
analysis_bl = analysis %>% filter(redcap_event_name == "Baseline") %>% select(record_id, gly_time_am) 

analysis_m3 = analysis %>% filter(redcap_event_name == "Month 3")

analysis_missing = analysis %>% filter(record_id %in% c(3,102,103))
```
o	Groups: 
o	Full cohort
