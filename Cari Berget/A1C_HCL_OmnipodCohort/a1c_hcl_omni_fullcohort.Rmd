...
use_plot_df = plot_df %>% select(a1c_group, redcap_event_name, gly_sensorwear, gly_time_am) %>% 
  pivot_longer(cols = gly_sensorwear:gly_time_am, names_to = "Measure", values_to = "Percent") %>% filter(!is.na(Percent))
use_plot = ggplot(data = use_plot_df, aes(x = redcap_event_name, y = Percent, shape = a1c_group, linetype = Measure, group = interaction(a1c_group, Measure))) + xlab("")+
  stat_summary(geom='line', fun = mean) +
  stat_summary(geom='point', fun = mean) + theme_classic()


# b)	A1c
a1c_plot_df = plot_df %>% select(a1c_group, redcap_event_name, gly_a1c) 
a1c_plot = ggplot(data = a1c_plot_df, aes(x = redcap_event_name, y = gly_a1c, shape = a1c_group, group = a1c_group)) + xlab("")+
  stat_summary(geom='line', fun = mean) +
  stat_summary(geom='point', fun = mean) + theme_classic() + theme(legend.position="none")
...
tir_plot = ggplot(data = tir_plot_df, aes(x = redcap_event_name, y = Sensor, shape = a1c_group, linetype = Measure, group = interaction(a1c_group, Measure))) + xlab("")+
  stat_summary(geom='line', fun = mean) +
  stat_summary(geom='point', fun = mean) + theme_classic()

# d)	Number of boluses/day 
bol_plot_df = plot_df %>% select(a1c_group, redcap_event_name, gly_dailybolus) 
bol_plot = ggplot(data = bol_plot_df, aes(x = redcap_event_name, y = gly_dailybolus, shape = a1c_group, group = a1c_group)) + xlab("")+
  stat_summary(geom='line', fun = mean) +
  stat_summary(geom='point', fun = mean) + theme_classic() + theme(legend.position="none")
