...
use_plot = ggplot(data = use_plot_df, aes(x = redcap_event_name, y = Percent, color = a1c_group, linetype = Measure, group = interaction(a1c_group, Measure))) + 
  stat_summary(geom='line', fun = mean) + theme_classic()


# b)	A1c
a1c_plot_df = plot_df %>% select(a1c_group, redcap_event_name, gly_a1c) 
a1c_plot = ggplot(data = a1c_plot_df, aes(x = redcap_event_name, y = gly_a1c, color = a1c_group, group = a1c_group)) + 
  stat_summary(geom='line', fun = mean) + theme_classic() + theme(legend.position="none")
...
tir_plot = ggplot(data = tir_plot_df, aes(x = redcap_event_name, y = Sensor, color = a1c_group, linetype = Measure, group = interaction(a1c_group, Measure))) + 
  stat_summary(geom='line', fun = mean) + theme_classic()

# d)	Number of boluses/day 
bol_plot_df = plot_df %>% select(a1c_group, redcap_event_name, gly_dailybolus) 
bol_plot = ggplot(data = bol_plot_df, aes(x = redcap_event_name, y = gly_dailybolus, color = a1c_group, group = a1c_group)) + 
  stat_summary(geom='line', fun = mean) + theme_classic() + theme(legend.position="none")
