...
library(emmeans)
library(lubridate)
#analysis_raw2 <- read_csv("S:/Laura/BDC/Projects/Cari Berget/a1c_hcl_omnipod/DATA/HCLClinicalObservati_DATA_2023-06-14_1345.csv")
analysis_raw <- read_csv("S:/Laura/BDC/Projects/Cari Berget/a1c_hcl_omnipod/DATA/HCLClinicalObservati_DATA_2.csv")
...
# age
analysis = analysis %>% filter(demo_c_age < 6)

# APPENDED August 2024

# 113 and 53 no gly data; to remove from analysis
# updated data has 113 data
# test_bl = analysis  %>% filter(redcap_event_name == "Baseline") %>% select(record_id)
# test_m1 = analysis %>% filter(redcap_event_name != "Baseline") %>% select(record_id)
analysis = analysis %>% filter(!(record_id %in% c(53)))

# table(analysis_raw$record_id);table(analysis$record_id) 
# length(table(analysis_raw$record_id));length(table(analysis$record_id))
```
...
```{r t1, echo = FALSE}
# id list
table(unique(analysis$record_id))
