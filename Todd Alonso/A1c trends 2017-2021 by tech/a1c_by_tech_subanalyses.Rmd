...
The average A1c increased by 0.41 (95% CI: 0.14, 0.68) in the Pump Only groups (p = 0.01), and increased by 0.81 (95% CI: 0.55, 1.07) in the No Tech group (p < 0.0001). There was no significant difference in the time periods within the HCL, CGM+Pump, or CGM only group (p = 0.98, 0.99, 1.00 respectively) 
...
The average A1c increased by 0.83 (95% CI: 0.53, 1.13) in the No Tech group (p < 0.0001). There was no significant difference in the time periods within the HCL (p = 0.96), CGM + Pump (p = 0.49), CGM only group (p = 0.997) or the pump only group (p = 0.43) 
...
There was a significant difference in A1c for patients who went from the CGM + Pump to HCL (decreased by 0.463, p = 0.003)
There was a significant difference in A1c for patients who went from the CGM + Pump to Pump only (increased by 0.980, p = 0.014)
There was a significant difference in A1c for patients who went from Pump only to HCL (decreased by 0.913, p < 0.0001)
...
# both visits
# by group
a1c_predict %>% group_by(tech_group, time_group) %>% summarise(pred_mean = mean(grpmean))

# overall
a1c_predict %>% group_by(time_group) %>% summarise(pred_mean = mean(grpmean))
# p-value
...
# same group
# by grup
a1c_predict_2 %>% group_by(tech_group, time_group) %>% summarise(pred_mean = mean(grpmean))
# overall

a1c_predict_2 %>% group_by( time_group) %>% summarise(pred_mean = mean(grpmean))
...
# switch grp
# by group
a1c_predict_switch %>% filter(tech_group_change %in% c("CGM + Pump/HCL", "CGM + Pump/Pump Alone", "Pump Alone/HCL"))%>% group_by(tech_group_change, time_group) %>% summarise(pred_mean = mean(grpmean))
# all
a1c_predict_switch %>% group_by(time_group) %>% summarise(pred_mean = mean(grpmean))
```