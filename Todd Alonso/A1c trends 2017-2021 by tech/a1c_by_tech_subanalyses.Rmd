...
```{r subanaly 2, include = FALSE}
# Fit random intercept model
a1c_mod_2 = lme(A1c_Value ~ Age_A1cResultDate + DiabetesDuration_A1cResultDate + tech_group + time_group + tech_group*time_group,
            random = ~1|EPICMRN,
            data = analysis_same_grp,
            na.action = na.omit)

a1c_mod2_2 = lme(A1c_Value ~ Age_A1cResultDate +  tech_group + time_group + tech_group*time_group,
            random = ~1|EPICMRN,
            data = analysis_same_grp,
            na.action = na.omit)
# Anova
...
# model
a1c_mod_switch = lme(A1c_Value ~ Age_A1cResultDate +  tech_group_change + time_group + tech_group_change*time_group,
            random = ~1|EPICMRN,
...
The average A1c decreased by 0.43 (95% CI : 0.24, 0.62) in the CGM + Pump group (p < 0.0001), increased by 0.34 (95% CI: 0.09, 0.53) in the Pump Only groups (p = 0.041), and increased by 0.85 (95% CI: 0.58, 1.11) in the No Tech group (p < 0.0001). There was no significant difference in the time periods within the CGM only group (p = 1.00) 
```{r both windows}
# .429+c(-1,1)*1.96*.097
...
The average A1c decreased by 0.49 (95% CI : 0.27, 0.71) in the CGM + Pump group (p < 0.0001), and increased by 0.81 (95% CI: 0.50, 1.11) in the No Tech group (p < 0.0001). There was no significant difference in the time periods within the CGM only group (p = 1.00) and the pump only group (p = 0.491) 
...
Adjusting for age, there is a significant difference in A1c within the different tech groups across the 2016-2017 and 2020-2021 time periods (p < 0.0001).

There was a significant difference in A1c for patients who went from the CGM + Pump to Pump only (increased by 0.924, p = 0.038)
There was a significant difference in A1c for patients who went from Pump only to CGM + Pump (decreased by 0.502, p = 0.001)
...
ggplot(data = a1c_predict_switch,aes(x = time_group, y = a1cpred, group = tech_group_change, color = tech_group_change))+
  geom_smooth(method = lm, se = F,na.rm = T) + 
  xlab("Time Groups") + ylab("Predicted Mean A1c") + theme_classic() 
```