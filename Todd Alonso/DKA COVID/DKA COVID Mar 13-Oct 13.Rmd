...
alldata <- read.csv("T:\\Todd Alonso\\DKA during COVID\\Raw data\\Copy of New Data_ Laura_Version4_11202020.csv")
...
alldata$temp <- NA
alldata[alldata$OnsetDate >= dmy("13-Mar-2017") & alldata$OnsetDate <= dmy("13-Oct-2017"),]$temp <- "Pre1"
alldata[alldata$OnsetDate >= dmy("13-Mar-2018") & alldata$OnsetDate <= dmy("13-Oct-2018"),]$temp <- "Pre2"
alldata[alldata$OnsetDate >= dmy("13-Mar-2019") & alldata$OnsetDate <= dmy("13-Oct-2019"),]$temp <- "Pre3"
alldata[alldata$OnsetDate >= dmy("13-Mar-2020") & alldata$OnsetDate <= dmy("13-Oct-2020"),]$temp <- "Post"
...
# compare demographics in pre vs. post
demovars <- c("AgeAtOnset","Gender","RaceEthnicity","InsuranceType","PrimarySpokenLanguage",
              "Rural.Non.Rural")
demotab <- CreateTableOne(vars=demovars,strata="pre_post",data=alldata)
demotab <- print(demotab,printToggle = F,varLabels=TRUE,missing=T,showAllLevels = T)
...
Mar 13-Oct 13 (2017-2019) was used as the pre-pandemic period.  3/13/20-10/13/20 was used as the post-pandemic period.
...
A total of `r nsubj` participants were included in this analysis.  

The table below compares demographic variables in the pre- and post-pandemic periods.

```{r, echo=FALSE}
kable(demotab,caption = "Comparison of demographics in the pre- and post-pandemic periods.")
```
<br>

Table 1 compares the rate of DKA at diagnosis pre- and post-pandemic.
