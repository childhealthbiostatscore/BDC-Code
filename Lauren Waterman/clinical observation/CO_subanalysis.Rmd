...
#test_co_tp = ClinicalObservationC_DATA_2024 %>% select(record_id, gyl_timepoint, timepoint, redcap_event_name)
```
...
# check 1
clinicalc_f = ClinicalObservationC_DATA_2024 %>% filter(!grepl("baseline", redcap_event_name)) %>% select(record_id, timepoint,redcap_event_name, hba1c,
...
analysis2 = analysis %>% group_by(record_id, timepoint) %>% fill(hba1c:hfs_total, .direction = "updown") %>% select(-redcap_event_name) %>% unique()  %>% ungroup()

# keep = rowMeans(is.na(analysis2[,1:38])) < 0.23
# test = analysis2[keep,]
# test_count = test %>% group_by(record_id, cohort, hcl) %>% summarise(count = n())
# table(test_count$hcl, test_count$cohort) # numbers match up with list



id_count = analysis2 %>% group_by(record_id, hcl, cohort) %>% summarise(count = n())
...
baseline = analysis2 %>% filter(timepoint=="Baseline")
bl_count = baseline %>% group_by(record_id, cohort, hcl) %>% summarise(count = n())
table(bl_count$hcl, bl_count$cohort) # numbers match up with list
...
Demographics, glycemic data, and survey data were produced from a combined dataset of Control IQ and OP5 users, split into cohorts of age 13-17 and 18-30.
...
## Demographics/Tables
```{r demogs}
# sanity/data clean check
table(id_count$hcl, id_count$cohort) # numbers match up with list
...
Time in AM was significantly different for months 3,6,12 from Baseline (all p < 0.0001).

Month 3 on average had 66% (SE = 8.5) more use  than Baseline.

Month 6 on average had 59% (SE=8.4) more use than baseline.

Month 12 on average had 57% (SE = 8.5) more use than baseline.
...
There were no significant differences from baseline for % time CGM for Month 3 (P=0.56), Month 6 (p = 0.48), or Month 12(p = 0.80)
...
There were no significant differences from baseline for % time CGM for Months 3,6 or 12 (P=0.97, 0.06, 0.34)
...
There were no significant differences from baseline for daily meal bolus for Month 3,6,12 (P=0.78, .91, .24)
...
Sleep AM use was significantly different for months 3,6,12 from Baseline (all p < 0.0001).
...
Exercise AM use was significantly different for months 3,6,12 from Baseline (p = .01, .0002, 0.001).
...
TIR was significantly different for months 3,6 from Baseline, but not M 12 (p = .004, .04, .06).
...
Hba1c was not significantly different for months 3,6,12 from Baseline (p = .48, .49, .22).
...
Avg Glucose was not significantly different for months 3,6,12 from Baseline (p = .15,  .78, .54).
...
Time < 70 was not significantly different for months 3,6,12 from Baseline (p = .79, .42, .60).
...
Time < 54 was not significantly different for month 3 from Baseline (p =.53, .55, .22).
...
Time >180  was significantly different for months 3,12 from Baseline (p = 0.01, 0.04) but not month 6 (p = 0.29).
```{r gtt180tir}
...
Time >250  was not significantly different for months 3,6,12 from Baseline (p = .25, .82, .69).
```{r gtt250tir}
...
Month 6 and 12 significantly different from month 1 (p = 0.048, 0.035). Month 3 p = 0.23
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.67, .75 .49)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.33, .59 .87)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.26, .43 .88)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.87, .92 .62)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.12, .61 .40)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.16, .48 .56)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.40, .43 .20)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.15, .65 .45)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.34, .99 .11)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.88, .38 .50)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.19, .92 .32)
```{r}
...
Month 3, 6 and 12 not significantly different from month 1 (p = 0.17, .55 .46)
```{r}
...
Month 3 not significantly different from baseline(p = 0.08)
...
Month 3 not significantly different from baseline(p = 0.71)
...
Month 3 not significantly different from baseline(p = 0.23)

Month 12 not significantly different from baseline (p = 0.92)
...
There is a significant difference in change in Time in Auto Mode between the cohorts (p =0.003)

```{r}
fit_mod_inf4("time_am", analysis)
#test = analysis %>% filter(timepoint == "Baseline" & cohort == "13-17") note a poor distribution: all 0's and one 93 in this group
```

### Time CGM

There is not a significant difference in change in sensor wear between the cohorts (p =0.731)
...
There is not a significant difference in change in TDD between the cohorts (p =0.403)
...
There is a significant difference in change in daily meal bolus between the cohorts (p =0.037)
...
There is not a significant difference in change in sleep activity use between the cohorts (p =0.764)
...
There is not a significant difference in change in exercise activity use between the cohorts (p =0.953)
...
There is not a significant difference in change in tir between the cohorts (p =0.849)
...
There is not a significant difference in change in hba1c between the cohorts (p =0.875)
...
There is not a significant difference in change in mean glucose between the cohorts (p =0.507)
...
There is not a significant difference in change in time under 70 between the cohorts (p =0.275)
...
There is a significant difference in change in time under 54 between the cohorts (p =0.029)
...
There is not a significant difference in change in time above 180 between the cohorts (p =0.127)
...
There is a significant difference in change in time above 250 between the cohorts (p =0.240)
...
There is not a significant difference in change in paid score between the cohorts (p =0.199)
...
There is not a significant difference in change in inspire score between the cohorts (p =0.248)
