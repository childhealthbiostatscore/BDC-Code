...
```{r}
table1(~sex + race + ethnicity + diabetes_duration + insurance + pump_user|emerging_adult, data = analysis)
```

### Instate

```{r}
table1(~sex + race + ethnicity + diabetes_duration + insurance + pump_user|instate, data = analysis %>% filter(!is.na(instate)))
...
```{r}
table1(~sex + race + ethnicity + diabetes_duration + insurance + pump_user|rationing, data = analysis %>% filter(!is.na(rationing)))
...
Running out: emerging adult is associated with Running out p = 0.03**
