...
note: Adult-2 record id missing a bunch of questions
...
```{r}
hcp_cgm_long_pc = hcp_cgm_long %>% filter(Practice_Type == "Primary Care")
lapply(split(hcp_cgm_long_pc, hcp_cgm_long_pc$likert_q), function(df) t.test(Likert ~ eos_visit , data = df ))
```
...
```{r}
hcp_cgm_long_ae = hcp_cgm_long %>% filter(Practice_Type == "Adult Endocrinology")
#lapply(split(hcp_cgm_long_ae, hcp_cgm_long_ae$likert_q), function(df) t.test(Likert ~ eos_visit , data = df ))
```
...
```{r}
hcp_cgm_long_pe = hcp_cgm_long %>% filter(Practice_Type == "Pediatric Endocrinology")
# lapply(split(hcp_cgm_long_pe, hcp_cgm_long_pe$likert_q), function(df) t.test(Likert ~ eos_visit , data = df ))
```
