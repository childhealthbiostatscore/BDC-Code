...
a1credcap$Lab.Collection.Trimester <- a1credcap$Trimester
...
```{r, echo=FALSE, message=FALSE}
mod <- lme(percent_time_0_69 ~ time*randomization_group + clinic ,random=~1|pid,data = cgmdata,na.action = na.omit)
mod_anova <- anova.lme(mod, type="marginal")
mod_means <- emmeans(mod,c("time","randomization_group"))
mod_pairs <-  pairs(mod_means,adjust="tukey", simple="each")
