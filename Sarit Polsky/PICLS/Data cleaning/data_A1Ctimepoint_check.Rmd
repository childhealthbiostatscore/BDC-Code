...
venous_counts=venous_A1Cs_gestdates %>% 
  group_by(`Study ID`) %>% 
  summarize(Visit_4=sum(week>=6 & week<=8),
            Visit_5=sum(week>=8 & week<=12),
            Visit_6=sum(week>=14 & week<=18),
            Visit_7=sum(week>=18 & week<=20),
            Visit_8=sum(week>=20 & week<=24),
            Visit_9=sum(week>=24 & week<=28),
            Visit_10=sum(week>=30 & week<=32),
            Visits_11thru12=sum(week>=32 & week<=36),
            Visits_13thru14=sum(week>=36 & week<=40))

venous_counts %>% 
  summarize('Visit 4 (6-8 wks)'=sum(Visit_4 != 0),
            'Visit 5 (8-12 wks)'=sum(Visit_5 != 0),
            'Visit 6 (14-18 wks)'=sum(Visit_6 != 0),
            'Visit 7 (18-20 wks)'=sum(Visit_7 != 0),
            'Visit 8 (20-24 wks)'=sum(Visit_8 != 0),
            'Visit 9 (24-28 wks)'=sum(Visit_9 != 0),
            'Visit 10 (30-32 wks)'=sum(Visit_10 != 0),
            'Visits 11-12 (32-36 wks)'=sum(Visits_11thru12 != 0),
            'Visits 13-14 (36-40 wks)'=sum(Visits_13thru14 != 0)) %>% t() %>% 
...
* Example: Referencing the Venous only table, there are 7 out of 23 subjects who have at least one A1C value that falls within the 6-8 gestational week window (Visit 4).