...
pump_variables = function(indir,outdir,outname){
  ...
  for (f in 1:length(files)) {
    ...
    if (nrow(bg_time_df)>0){
      bg_time_df$bg_datetimes <- parse_date(bg_time_df$bg_datetimes,approx = F)
    ...
    for (r in which(!is.na(table$BWZ.Carb.Input..grams.))) {
      ...
      total_carbs_g = total_carbs_g + as.numeric(table$BWZ.Carb.Input..grams.[r])
      # Dates
    ...
    if (nrow(bolus_time_df)>0){
      bolus_time_df$bolus_datetimes <- parse_date(bolus_time_df$bolus_datetimes,approx = F)
  ...
  filename <- paste0(outdir,"/",outname,".csv")
