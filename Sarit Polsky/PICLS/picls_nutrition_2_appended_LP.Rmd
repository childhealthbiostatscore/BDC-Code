...
pp_weights <- merge(pp_weights, dq, by = "pid")
...
1) Weight gain per week was calculated as the final weight in each trimester minus the preconception weight, divided by the number of weeks in the trimester.
2) Review "last weights tri" file

# Methods - UPDATE!!

Subjects maternal weight gain was measured using the last pregnancy visit compared with the preconception weight. This was then averaged into a weekly weight gain over all trimesters.

Excessive weight gain(either by weekly weight gain or total weight gain) was derived for the entire pregnancy.
...
Generalized Linear Mixed Models with a binomial link function (Mixed Effects Logistic Regression) for Excessive Weight Gain were fit adjusting for enrollment maternal BMI. 
...
###  Logistic Regression Models with categorized diet variables
...
No participant had an overall high quality diet so this model could not be run.
...
## Maternal Weight Gain (continuous variable)

### Linear Models with weight gain as continuous outcome
