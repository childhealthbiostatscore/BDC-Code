...
```{r}
table1(~as.factor(excessive_weight_gain) + weight_gain_total_lbs + avg_weekly_weight_gain_lbs, data = analysis_aim1_tri)
```

### Post-partum weight retention

```{r}
table1(~pp_weight_retention, data = pp_weights)
```
...
In this section, the Analysis of Deviance table provides p-values for the test of each variable in the model.

The second table for each model provides odds ratios (labeled "estimate").
