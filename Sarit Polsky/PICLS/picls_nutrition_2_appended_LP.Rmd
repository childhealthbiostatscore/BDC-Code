...
Most recent date the data was pulled from BDC Shared drive: may 2024; new data added in march 2024.

The data seems to match the data we should have PRIOR to discrepancies ie) for 100A we have YNY for baseline, 2nd, 3rd tri; but the discrepancy column lists 2nd tri - Y.

Notes list this data as previously "Archived;" haven't checked every part of every file (hard to code with different naming conventions/duplicate filenames), but it looks like it should be able to match up to correct, or closer to correct numbers as listed in email ( 22 tri 1, 16 tri 2, 14 tri 3, 15 pp) .

Outstanding remarks: Wanted to chat about this in the meeting instead of over email to confirm what's what/ safe to trust etc within the "archive" folder
...
# discrepancy for 2nd tri so should have actual log 2nd tri as jul 19
testid = maternal_diet_subj_day %>% ungroup()%>%select(pid, tp, `Date of Intake`) %>% filter(pid == "100A")
print((testid))
```
