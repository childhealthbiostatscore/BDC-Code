...
standardized_tri_avg = standardized_groups %>% group_by(pid, tp)  %>% summarise(avg_daily_fiber_s = mean(s_fiber),
                                                                           ...
                                                                           avg_daily_animal_perc_s = mean(s_plant_perc))
...
analysis_m1 = analysis_m1_tri %>% ungroup() %>% group_by(pid) %>% select(avg_daily_fiber_s:avg_daily_animal_perc_s, avg_weekly_weight_gain_lbs) %>% 
  summarise(fiber_s = mean(avg_daily_fiber_s),
            ...
            plant_perc_s = mean(avg_daily_animal_perc_s),
            avg_wk_weight_gain_lbs = mean(avg_weekly_weight_gain_lbs, na.rm=T))
```

# Outstanding Data questions/remarks

- does weight loss make sense here? some subj have lower weights than pre-con in their last trimester measurements; ex 107A Tri2 had a 0.3kg lower weight than precon
- most pretty small differences except 119A with a -35kg difference tri 1 date:3/15/21. This seems like a typo, the other weights are all ~ -5ish? I have the precon weight as 103.5, but trimester weights all varying degree of lower
- daily calorie total was between 500-3000. does this seem like an appropriate distribution
