...
# cgm data
HCLSLEEP_CGM <- read.csv("S:/Laura/BDC/Projects/Erin Cobry/ADA 2022/HCL and Sleep/Data_Raw/HCLSLEEP_CGM.csv")

HCLSLEEP_CGM$vis = factor(HCLSLEEP_CGM$vis, labels = c("BL", "W2", "M3", "M6"), levels = c("BL", "W2", "M3", "M6"))
```
...
fitmod_glyc = function(outcome,df,diagnostics = F){

  # Fit random intercept model
  f = as.formula(paste0(outcome,"~gyl_timepoint+(1|record_id)"))
  mod = lmer(f,data = df)
  ...
  print(kable(res,digits = 3,caption = "Comparison to Baseline"))
  # Means
  means = data.frame(emmeans(mod,~gyl_timepoint))
  print(kable(means,digits = 3,caption = "Model Means"))
  # Number of obs
  cat("\n")
  cat(paste("This model was fit using",nobs(mod),"observations."))
...
added in analysis to make the glycemic data match the other segments

```{r results='asis'}
print("sensor > 251")
fitmod_glyc(outcome = "sensor_g251", df = glyc)

print("sensor 181-250")
fitmod_glyc(outcome = "sensor_181_250", df = glyc)

print("sensor 70 -180")
fitmod_glyc(outcome = "sensor_70_180", df = glyc)

print("sensor55-69")
fitmod_glyc(outcome = "sensor_55_69", df = glyc)

print("sensor < 54")
fitmod_glyc(outcome = "sensor_u54", df = glyc)

print("avg sensor glucose")
fitmod_glyc(outcome = "sensor_mean", df = glyc)

print("sensor sd")
fitmod_glyc(outcome = "sensor_sd", df = glyc)




print("night sensor > 251")
fitmod_glyc(outcome = "night_sensor_high", df = glyc)

print("night sensor 181-250")
fitmod_glyc(outcome = "night_sensor_above", df = glyc)

print("night sensor 70 -180")
fitmod_glyc(outcome = "night_sensor_target", df = glyc)

print("night sensor55-69")
fitmod_glyc(outcome = "night_sensor_below", df = glyc)

print("night sensor < 54")
fitmod_glyc(outcome = "night_sensor_low", df = glyc)

print("night avg sensor glucose")
fitmod_glyc(outcome = "night_sensor_mean", df = glyc)

print("night sensor sd")
fitmod_glyc(outcome = "night_sensor_sd", df = glyc)



print("%time in close loop")
fitmod_glyc(outcome = "time_am", df = glyc)

print("%time sensor wear")
fitmod_glyc(outcome = "sensor_wear", df = glyc)
...
# get n's for table
print("age group 3-5")
child_activity_35 %>% group_by(timepoint) %>% summarise(n())

print("age group 6-12")
child_activity_612 %>% group_by(timepoint) %>% summarise(n())

print("age group 13-17")
child_activity_1317 %>% group_by(timepoint) %>% summarise(n())

# parent actigraphy by age grp
parentgrps = child_activity %>% select(record_id, age_grp) %>% mutate(record_id2 = as.character(as.numeric(record_id)-100))
parent_activity$record_id = as.character(parent_activity$record_id)
parent_activity = full_join(parent_activity, parentgrps, by = c("record_id" = "record_id2")) %>% unique()
parent_activity$age_grp = factor(parent_activity$age_grp, levels = c("3-5", "6-12", "13-17"))

parent_activity %>% group_by(timepoint, age_grp) %>% summarise(n())
```
# appended as of may 2023
## parent actigraphy by age group
```{r results='asis'}
print("age group 3-5")
fit_mod(outcome = "sleepefficency_avg",df = parent_activity %>% filter(age_grp == "3-5"), plot = F)
fit_mod(outcome = "onsetlatency_avg",df = parent_activity %>% filter(age_grp == "3-5"), plot = F)
fit_mod(outcome = "totalsleep_avg",df = parent_activity %>% filter(age_grp == "3-5"), plot = F)
fit_mod(outcome = "waso_avg",df = parent_activity %>% filter(age_grp == "3-5"), plot = F)

print("age group 6-12")
fit_mod(outcome = "sleepefficency_avg",df = parent_activity %>% filter(age_grp == "6-12"), plot = F)
fit_mod(outcome = "onsetlatency_avg",df = parent_activity %>% filter(age_grp == "6-12"), plot = F)
fit_mod(outcome = "totalsleep_avg",df = parent_activity %>% filter(age_grp == "6-12"), plot = F)
fit_mod(outcome = "waso_avg",df = parent_activity %>% filter(age_grp == "6-12"), plot = F)


print("age group 13-17")
fit_mod(outcome = "sleepefficency_avg",df = parent_activity %>% filter(age_grp == "13-17"), plot = F)
fit_mod(outcome = "onsetlatency_avg",df = parent_activity %>% filter(age_grp == "13-17"), plot = F)
fit_mod(outcome = "totalsleep_avg",df = parent_activity %>% filter(age_grp == "13-17"), plot = F)
fit_mod(outcome = "waso_avg",df = parent_activity %>% filter(age_grp == "13-17"), plot = F)

# different results :( table1(~ sleepefficency_avg + onsetlatency_avg + totalsleep_avg + waso_avg | timepoint*age_grp.y , data = parent_activity)
```
...
kable(hfs_c_n)
#mod
fit_mod(outcome = "total_s", df = hfs_c)

fit_mod(outcome = "worry_s", df = hfs_c)
fit_mod(outcome = "worry_lowbg_s", df = hfs_c)
fit_mod(outcome = "worry_negsc_s", df = hfs_c)

fit_mod(outcome = "behave_s", df = hfs_c)
fit_mod(outcome = "maintain_high_s", df = hfs_c)
...
cshq = cshq %>% mutate_at(vars, as.numeric)
cshq = cshq %>% mutate(cshq_total = rowSums(.[vars]))
```
...
paid_pr = paid_pr %>% mutate_at(c(3:20), function(x){y = ((6-x)-1)*25;y})

# score 
paid_pr = paid_pr %>% mutate(paid_pr_total = (rowMeans(.[3:20])))
```
...
pedsql_pr_t = pedsql_pr_t %>% mutate_at(c(3:30), function(x){y = (6-x-1)*25;y})

# score 
pedsql_pr_t = pedsql_pr_t %>% mutate(pedsql_pr_t_total = (rowMeans(.[3:30])))
```
...
pedsql_pr_c = pedsql_pr_c %>% mutate_at(c(3:30), function(x){y = (6-x-1)*25;y})

# score 
pedsql_pr_c = pedsql_pr_c %>% mutate(pedsql_pr_c_total = (rowMeans(.[3:30])))
```
...
pedsql_pr_yc = pedsql_pr_yc %>% mutate_at(c(3:30), function(x){y = (6-x-1)*25;y})

# score 
pedsql_pr_yc = pedsql_pr_yc %>% mutate(pedsql_pr_yc_total = (rowMeans(.[3:30])))
```
...
pedsql_t = pedsql_t %>% mutate_at(c(3:30), function(x){y = (6-x-1)*25;y})
...
pedsql_c = pedsql_c %>% mutate_at(c(3:30), function(x){y = (6-x-1)*25;y})
...
pedsql_yc = pedsql_yc %>% mutate_at(c(3:30), function(x){y = (6-x-1)*25;y})
...
inspire_pb = inspire_pb %>% mutate_at(c(3:23), function(x)
  {x = recode(x,"Strongly Agree" = 4, "Agree" = 3, "Neither Agree nor Disagree" = 2, "Disagree" = 1,"Strongly Disagree" = 0, .default=NA_real_);x}
  )
inspire_pb = inspire_pb %>% mutate_at(c(3:23), as.integer)

inspire_pb = inspire_pb %>% mutate_at(c(3:23), function(x){y = (x)*25;y})
...
# set levels
inspire_pf = inspire_pf %>% mutate_at(c(3:23), function(x)
  {x = recode(x,"Strongly Agree" = 4, "Agree" = 3, "Neither Agree nor Disagree" = 2, "Disagree" = 1,"Strongly Disagree" = 0, .default=NA_real_);x}
  )
inspire_pf = inspire_pf %>% mutate_at(c(3:23), as.integer)

inspire_pf = inspire_pf %>% mutate_at(c(3:23), function(x){y = (x)*25;y})
...
# set levels
#inspire_yb[3:23] = lapply(inspire_yb[3:23], function(x) {levels(x) = c(4,3,2,1,0, -99);x } )
inspire_yb = inspire_yb %>% mutate_at(c(3:19), function(x)
  {x = recode(x,"Strongly Agree" = 4, "Agree" = 3, "Neither Agree nor Disagree" = 2, "Disagree" = 1,"Strongly Disagree" = 0, .default=NA_real_);x}
  )
inspire_yb = inspire_yb %>% mutate_at(c(3:19), as.integer)

inspire_yb = inspire_yb %>% mutate_at(c(3:19), function(x){y = (x)*25;y})
# score 
...
# set levels
inspire_yf = inspire_yf %>% mutate_at(c(3:19), function(x)
  {x = recode(x,"Strongly Agree" = 4, "Agree" = 3, "Neither Agree nor Disagree" = 2, "Disagree" = 1,"Strongly Disagree" = 0, .default=NA_real_);x}
  )
inspire_yf = inspire_yf %>% mutate_at(c(3:19), as.integer)

inspire_yf = inspire_yf %>% mutate_at(c(3:19), function(x){y = (x)*25;y})
...
#test = promis_peds %>% filter(timepoint == "baseline_arm_2") %>% filter(impairment > 60) # 18/33 dist 55 10/33 dist 60
```
