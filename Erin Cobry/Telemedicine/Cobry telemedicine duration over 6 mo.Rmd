...
code_dir = ifelse(.Platform$OS.type == "unix",
                  "",
                  "B:\\Projects\\Erin Cobry\\Telemedicine\\Data raw\\")
setwd(code_dir)

knitr::opts_chunk$set(echo = TRUE)

# read in general clinic data
allclinic <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\Erin Cobry\\Telemedicine\\Data raw\\General Clinic Master List_08-26-20 Rural.csv",na.strings = c(" ",""))
...
data <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\Erin Cobry\\Telemedicine\\Data raw\\DiabetesTelemedicine-Cobry2020ADAAbstract_DATA_rural_distance_11-5-2020.csv")
...
keep <- read.csv("S:\\Shared Projects\\Laura\\BDC\\Projects\\Erin Cobry\\Telemedicine\\Data raw\\Telemedicine Study Patient MRNs.csv")
...
ns = tele %>% group_by(techgroup) %>% filter(!is.na(a1c_fup_telemed)) %>%
  summarise(n = n())
ggplot(tele[!is.na(tele$a1c_fup_telemed),],aes(x=techgroup,y=a1c_fup_telemed)) +
  geom_boxplot() +
...
nchar <- rep("NA",6)
for (i in 1:length(plotN$n)) {
  nchar[i] <- paste0("N=",plotN$n[i])
}
nchar <- as.data.frame(nchar)

lab <- noquote(paste0(noquote("N="),noquote(plotN$n[1])))
p <- ggplot(comb, aes(x=techgroup, y=hba1c, fill=group)) + geom_boxplot() + xlab("") + ylab("HbA1c (%)") + scale_fill_grey() + theme_classic() + guides(fill=guide_legend(title="")) + geom_text_repel(data = plotN, aes(y = 0, label = n),direction = "x")
  
  geom_text(data = plotN, aes(y = 0, label = n),position = position_jitter(height=0))

# export for graphpad
gp <- comb[,c("techgroup","group","hba1c")]
write.csv(gp,"B:\\Projects\\Erin Cobry\\Telemedicine\\Data_clean\\export_for_graphpad.csv")

#p <- ggplot(data=comb, aes(x=techgroup, y=hba1c, fill=group)) + geom_boxplot() + xlab("") + ylab("HbA1c (%)") + scale_fill_grey() + #theme_classic() + guides(fill=guide_legend(title="")) + geom_text(data = nchar, aes(y = 0, label = nchar))
