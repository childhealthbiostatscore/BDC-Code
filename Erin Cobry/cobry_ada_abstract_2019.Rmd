...
For each timepoint, the "estimate" column in the table above represents the factor by which the rate of alarms changed from baseline, after adjusting for percent time in AM. All four timepoints had a significantly lower rates of alarms per day compared to baseline. Also, for each percentage point increase in time spent spent in AM, the rate of alarms per day was on average `r results$estimate[results$term == "% Time in AM"]` times higher (p = `r results$p.value[results$term == "% Time in AM"]`). So, a 10% increase in AM use would on average result in a `r (results$estimate[results$term == "% Time in AM"] - 1)*100*10`% increase in the rate of alarms.
