...
# Select columns
cgm <- cgm %>% select(id,timepoint,
                      daytime_avg_sensor_glucose,percent_time_70_180_day,
                      nighttime_avg_sens_glucose,percent_time_under_70_night)
# Merge
overnight_alarms$id <- as.character(overnight_alarms$id)
temp <- left_join(overnight_alarms,cgm,by = c("id","timepoint"))
```
...
mod_ri_car1 <- glmmPQL(num_alarms ~ timepoint + perc_time_in_AM + offset(log(num_nights)),
                       random = ~1|id,data = overnight_alarms,
                       family = "poisson",correlation = corCAR1())
