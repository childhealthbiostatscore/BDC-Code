...
model_and_plot = function(outcome_name,ylab,group_var,df,pois = F){
  ...
  print(kable(pairs(emmeans(mod,"Timepoint")),caption="Comparison of time points"))
}
```
...
```{r results='asis'}
tvc("average_glucose","exposure_sum",parent_cefis_child_diabetes[parent_cefis_child_diabetes$exposure_sum>0,])
```

## Impact and average glucose

```{r results='asis'}
tvc("average_glucose","impact_mean",parent_cefis_child_diabetes[parent_cefis_child_diabetes$exposure_sum>0,])
```

## Distress and average glucose

```{r results='asis'}
tvc("average_glucose","distress",parent_cefis_child_diabetes[parent_cefis_child_diabetes$exposure_sum>0,])
```
