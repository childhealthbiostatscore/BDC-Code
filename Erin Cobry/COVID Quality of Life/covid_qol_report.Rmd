...
model_and_plot = function(outcome_name,ylab,group_var,df,pois = F){
  ...
  # Fit a mixed model. Some of these are essentially count data and require a Poisson model
  if (pois == T) {
    mod = glmer(as.formula(paste0(outcome_name,"~Timepoint+(1|",group_var,")")),df,family = poisson())
  } else {
