...
# sleep measures
sleep_meas = sleep_disruptors %>% select(record_id,group,redcap_event_name,totalsleep_avg, onsetlatency_avg, sleepefficency_avg, waso_avg ) %>% mutate(totalsleep_m = hour(hm(totalsleep_avg))*60 + minute(hm(totalsleep_avg)))

sleep_meas = sleep_meas %>%  filter(redcap_event_name == "surveys_arm_1") %>% select(group, onsetlatency_avg:totalsleep_m)

sleep_t = print(CreateTableOne(vars = c("onsetlatency_avg", "sleepefficieny_avg","waso_avg","totalsleep_m"), strata = "group", data = sleep_meas))
...
# HCL use (for those on HCL, if RedCap value blank do not include)
# Sensor use (for those on sensor, if RedCap value blank do not include)
diab_meas = sleep_disruptors %>% filter(redcap_event_name == "surveys_arm_1") %>% select(record_id,group,redcap_event_name, sensor_u54, sensor_55_69, sensor_70_180, sensor_181_250, sensor_g251, sensor_wear, insulinmethod) %>% select(-redcap_event_name)

diab_meas = diab_meas %>% mutate(sensor_hypo = sensor_u54 + sensor_55_69,
                                 sensor_hyper = sensor_181_250 + sensor_g251)

diab_t = print(CreateTableOne(vars = c("sensor_70_180", "sensor_hypo", "sensor_hyper"), strata = "group", data = diab_meas))
```
...
# Sleep measures (RedCap wrist actigraphy variable)
note: for 1035 no visit collected, 1029 all missing
total sleep average measured in minutes
```{r Sleep Measures}
kable(sleep_t)
```

# diabetes measures
note: for 1035 no visit collected, 1029, 26 all missing values
```{r diabetes}
kable(diab_t)
```
