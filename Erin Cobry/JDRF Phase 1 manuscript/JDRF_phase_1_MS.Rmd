...
mesc <- mesc %>% mutate(mesc_type = case_when(
  !is.na(mesc_total) & mesc_total <= 20 ~ "E-type",
  !is.na(mesc_total) & mesc_total >= 21 & mesc_total <= 27 ~ "N-type",
  !is.na(mesc_total) & mesc_total >28 ~ "M-type",
  .default = NA
))
...
sufficient_sleep_df <- analysis %>% select(record_id, sufficient_sleep, mesc_type)
write.csv(sufficient_sleep_df, "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Erin Cobry/JDRF Career Development Grant/Phase 1 Manuscript/Data_clean/sufficient_sleep.csv", row.names = FALSE)
...
as.factor(fstage2) + as.factor(drstage) + as.factor(self_stage) + as.factor(mesc_type), data = demog_df)
```
