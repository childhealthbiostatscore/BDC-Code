...
#source("/Users/pylell/Documents/GitHub/BDC-Code/Erin Cobry/JDRF Phase 1 manuscript/write promis for checking.r")
#check <- extract_promis_values(sd_df)
#check <- check %>% filter(redcap_event_name %in% c("surveys_arm_1", "surveys_arm_2"))
#write.csv(check, "/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Erin Cobry/JDRF Career Development Grant/Phase 1 Manuscript/Data_clean/promis_data.csv", row.names = FALSE)

# merge in scaled scores
# are these t-scores? why are they called "raw score"?
sd_df <- left_join(sd_df, Depression_T, by = c("c_depressive_raw" = "Raw Score"))
sd_df <- left_join(sd_df, pp_dep_short, by = c("p_depressive_raw" = "Raw Score"))
sd_df <- left_join(sd_df, Anxiety_T, by = c("c_anxiety_raw" = "Raw Summed Score"))
sd_df <- left_join(sd_df, pp_anx_short, by = c("p_anxiety_raw" = "Raw Score"))
sd_df <- left_join(sd_df, Psychological_Stress_T, by = c("c_psychological_stress_raw" = "Raw Score"))
sd_df <- left_join(sd_df, pp_pse_short, by = c("p_psychological_stress_raw" = "Raw Score"))
sd_df <- left_join(sd_df, Sleep_Disturbance_T, by = c("c_sleep_disturbance_raw" = "Raw Summed Score"))
sd_df <- left_join(sd_df, Impairment_T, by = c("c_sleep_impairment_raw" = "Raw Summed Score"))
sd_df <- left_join(sd_df, peds_lifesatisfaction_short, by = c("c_life_satisfaction_raw" = "Raw Summed Score"))
sd_df <- left_join(sd_df, peds_physact_short, by = c("c_physical_raw" = "Raw Score"))
sd_df <- left_join(sd_df, pp_physact_short, by = c("p_physical_raw" = "Raw Score"))
sd_df <- left_join(sd_df, peds_posaff_short, by = c("c_positive_affect_raw" =  "Raw Score"))
sd_df <- left_join(sd_df, pp_posaff_short, by = c("p_positive_affect_raw" = "Raw Scored"))
sd_df <- left_join(sd_df, peds_anger_short, by = c("c_anger_raw" = "Raw Summed Score"))
sd_df <- left_join(sd_df, pp_anger_short, by = c("p_anger_raw" = "Raw Summed Score"))
sd_df <- left_join(sd_df, peds_fr_short, by = c("c_family_raw" = "Raw Score"))
sd_df <- left_join(sd_df, peds_pr_short, by = c("c_peer_raw" = "Raw Score"))

# this one doesn't look right - drop it
sd_df <- left_join(sd_df, pp_fr_short, by = c("p_family_raw" = "Raw Score"))
sd_df <- sd_df %>% select (-parent_fr_t)

# convert to t-scores
sd_df$c_depressive_tscore <- convert_to_tscore(raw_score = sd_df$c_depressive_raw, scale_name = "depressive", lookup_table =  Depression_T)

# read in correctly scored parent family relationships
temp <- read.csv('/Volumes/RI Biostatistics Core/Shared/Shared Projects/Laura/BDC/Projects/Erin Cobry/JDRF Career Development Grant/Phase 1 Manuscript/Data_raw/SleepDisruptorsInAdo-PROMISParentFamRelat_DATA_2025-03-18_0745.csv')
temp <- temp %>% filter(!is.na(promis_parent_proxy_sf_v10_fam_relationships_8a_tscore))
# still need to merge t-score into the dataset
...
mesc = survey_df %>% select(record_id, redcap_event_name, mesc_total)
...
srdb = survey_df %>% filter(redcap_event_name %in% c("surveys_arm_1", "surveys_arm_2")) %>% mutate(SRDB_score = rowMeans(across(snore_ht:interrupts), na.rm = T)) %>%
  select(record_id, redcap_event_name, SRDB_score) # SRDB Scale
...
hfs = survey_df %>% filter(redcap_event_name == "surveys_arm_1") %>% mutate(across(bedtime_snacks:having_low_sugar, ~ .x - 1),
                           hfs_total = rowSums(across(bedtime_snacks:having_low_sugar), na.rm=F)) %>% select(record_id, redcap_event_name, hfs_total)# Hypoglycemia Fear Survey child/Teen

surveys = full_join(srdb, hfs, by = c("record_id", "redcap_event_name")) %>% unique()
surveys = full_join(surveys, mesc) %>% unique()

analysis = left_join(analysis,surveys, by = c("record_id", "redcap_event_name")) %>% unique()
