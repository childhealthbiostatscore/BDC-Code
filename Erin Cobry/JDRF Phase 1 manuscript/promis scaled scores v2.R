...
#' @export
convert_to_tscore <- function(raw_score, scale_name, respondent, lookup_tables) {
  # Get the appropriate lookup table
  table_name <- paste0(scale_name, "_", respondent)
  ...
  # Convert raw scores to character for lookup
  raw_score_char <- as.character(raw_score)
  
  # Match raw scores to T-scores using lookup table
  t_scores <- lookup_table[raw_score_char]
...
convert_all_to_tscores <- function(scores, lookup_tables, respondent = "child") {
  ...
  for (col in raw_score_cols) {
    ...
    # Remove c_ or p_ prefix if present to get clean scale name for lookup
    clean_scale_name <- sub("^[cp]_", "", scale_name)
    
    # Skip life satisfaction for parent respondent
    if (clean_scale_name == "life_satisfaction" && respondent == "parent") {
      next
    }
    ...
    if (!is.null(lookup_table)) {
      ...
      t_scores <- convert_to_tscore(scores[[col]], clean_scale_name, respondent, lookup_tables)
      
      # Add T-scores to result data frame, preserving original prefix
      result[[paste0(scale_name, "_tscore")]] <- t_scores
    } else {
      warning(sprintf("No T-score conversion table available for %s scale (%s version)", 
                      clean_scale_name, respondent))
      # Keep raw score as is
      result[[paste0(scale_name, "_tscore")]] <- scores[[col]]
    }
  }
...
#
# # Convert life satisfaction scores
# life_sat_tscores <- convert_to_tscore(
#   raw_score = df$c_life_satisfaction_raw,
#   scale_name = "life_satisfaction",
#   respondent = "child",
