...
daytime_alarms = read.csv("./Erin Cobry/Nocturnal Alarms/Data_Cleaned/daytime_alarms.csv",
                          na.strings = c("","No Data"))
daytime_alarms = daytime_alarms[daytime_alarms$num_nights > 0,]
...
overnight_alarms = overnight_alarms %>%
  mutate(across(all_of(alarm_vars),.fns = ~./(num_nights*8),.names = "{col}_per_hour_night"))
daytime_alarms = daytime_alarms %>%
  mutate(across(all_of(alarm_vars),.fns = ~./((num_nights+1)*16),.names = "{col}_per_hour_day"))
# Combine nocturnal and daytime
overnight_alarms = overnight_alarms %>% 
  select(id:end_date,numeric_time,num_alarms_per_hour_night:other_alarms_per_hour_night)
daytime_alarms = daytime_alarms %>%
  select(id:end_date,numeric_time,num_alarms_per_hour_day:other_alarms_per_hour_day)
overnight_alarms = full_join(overnight_alarms,daytime_alarms,
                             by = c("id", "timepoint", "start_date", "end_date", 
                                    "numeric_time"))
...
All alarms were analyzed using a linear mixed model with random intercept for participant and continuous AR1 correlation structure.

```{r}
# Model function
fit_mod = function(outcome,df = overnight_alarms){
  # Random intercept (not enough data for a random slope)
  f = as.formula(paste0(outcome,"~ timepoint"))
  list2env(list(f=f,df=df),.GlobalEnv)
  mod_ri_car1 <- lme(f,random = ~1|id,na.action = na.exclude,
                     data = df,correlation = corCAR1(form = ~numeric_time))
...
fit_mod(outcome = "num_alarms_per_hour_night")
```
