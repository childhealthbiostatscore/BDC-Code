...
# Print
kable(results[,c("term","estimate","conf.low","conf.high","p.value")],digits = 3)
check_model(mod_ri_car1,verbose=F)
```
...
```{r echo=FALSE,message=FALSE}
# Random intercept (not enough data for a random slope)
mod_ri_car1 = lme(pump_alarms_per_night ~ timepoint,
                  random = ~1|id,na.action = na.omit,
                   data = overnight_alarms,
                  correlation = corCAR1(form = ~numeric_time))
results <- tidy(mod_ri_car1,effects = "fixed",conf.int = T)
...
```{r echo=FALSE,message=FALSE}
# Random intercept (not enough data for a random slope)
mod_ri_car1 = lme(pump_alarms_per_night ~ timepoint + time_am,
                  random = ~1|id,na.action = na.omit,
                   data = overnight_alarms,
                  correlation = corCAR1(form = ~numeric_time))
results <- tidy(mod_ri_car1,effects = "fixed",conf.int = T)
...
```{r echo=FALSE,message=FALSE}
# Random intercept (not enough data for a random slope)
mod_ri_car1 = lme(pump_alarms_per_night ~ timepoint + sensor_wear,
                  random = ~1|id,na.action = na.omit,
                   data = overnight_alarms,
                  correlation = corCAR1(form = ~numeric_time))
results <- tidy(mod_ri_car1,effects = "fixed",conf.int = T)
...
```{r echo=FALSE,message=FALSE}
# Random intercept (not enough data for a random slope)
mod_ri_car1 = lme(other_alarms_per_night ~ timepoint,
                  random = ~1|id,na.action = na.omit,
                   data = overnight_alarms,
                  correlation = corCAR1(form = ~numeric_time))
results <- tidy(mod_ri_car1,effects = "fixed",conf.int = T)
...
```{r echo=FALSE,message=FALSE}
# Random intercept (not enough data for a random slope)
mod_ri_car1 = lme(other_alarms_per_night ~ timepoint + time_am,
                  random = ~1|id,na.action = na.omit,
                   data = overnight_alarms,
                  correlation = corCAR1(form = ~numeric_time))
results <- tidy(mod_ri_car1,effects = "fixed",conf.int = T)
...
```{r echo=FALSE,message=FALSE,eval=FALSE}
# Random intercept (not enough data for a random slope)
mod_ri_car1 = lme(other_alarms_per_night ~ timepoint + sensor_wear,
                  random = ~1|id,na.action = na.omit,
                   data = overnight_alarms,
                  correlation = corCAR1(form = ~numeric_time))
results <- tidy(mod_ri_car1,effects = "fixed",conf.int = T)
