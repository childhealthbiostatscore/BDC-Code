...
iv_time <- read.csv("./Data raw/AccuracyOfTheDexcomG-IVInsulin_DATA_2023-02-24_2222.csv")
...
cgm_keep <- merge(cgm_keep,iv_time,by="record_id",all.x=F,all.y=T)
cgm_keep <- cgm_keep %>% filter(date_time>=gtt_start_daet)
cgm_keep <- cgm_keep %>% filter(date_time<=insulin_drip_stop)

glucose_keep <- merge(glucose_keep,iv_time,by="record_id",all.x=F,all.y=T)
...
demo <- patient %>% group_by(record_id) %>% slice(1)
...
-   For this analysis, only hospital glucose values during the insulin infusion were included.

-   For this analysis, only patients admitted for DKA were included.
