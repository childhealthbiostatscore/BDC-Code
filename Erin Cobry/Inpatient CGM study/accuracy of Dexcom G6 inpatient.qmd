...
patient$race <- as.factor(patient$race)
patient$ethnicity <- as.factor(patient$ethnicity)
patient_keep <- patient %>% select(record_id,locationfloor.factor)

# Read in csv files
cleandata(inputdirectory = "B:/Projects/Erin Cobry/Inpatient CGM study/Data raw/Dexcom CSVs", outputdirectory = "B:/Projects/Erin Cobry/Inpatient CGM study/Data clean/Clean CSVs", verbose = F, id_filename = T, gapfill = F, removegaps = F)
...
```{r include=TRUE, warning=FALSE, message=F}
test <- getClarkeZones(referenceVals = glucose_keep$bgvalue, testVals = glucose_keep$cgm_glucose)
table(test)
...
p + scale_fill_manual(values=c("#ca7dcc","#1b98e0","#353436","#02e302"))
...
```{r include=TRUE, warning=FALSE, message=F}
plotClarkeGrid(referenceVals = med_ed$bgvalue, testVals = med_ed$cgm_glucose,xlab="POC glucose (mg/dL)",ylab = "CGM glucose (mg/dL)" )
...
```{r include=TRUE, warning=FALSE, message=F}
plotClarkeGrid(referenceVals = icu$bgvalue, testVals = icu$cgm_glucose,xlab="POC glucose (mg/dL)",ylab = "CGM glucose (mg/dL)" )
